...
export const BUILTIN_GITHUB_TEMPLATES: TemplateSet = {
  ...
  "common/ut-tdd.mjs": [
    ...
    "// PLAN-L7-509 §2.3-10: consumer-local な解決先だけを見る。setup を実行したマシンの",
    "// Pack checkout 絶対パスは **埋め込まない** — PLAN-L6-101 §1.1 が「ローカル Pack checkout を",
    "// runtime discovery 入力にしてはならない」と定め、§2 が development repo / worktree /",
    "// Pack checkout 不在での導入再現を要求するため。解決できない場合は typed fail-close する",
    "// (実体を consumer 内へ materialize する方式は後続 PLAN が所有する)。",
    "// 2 ファイルを要求するのは、無関係な consumer の src/cli.ts を harness CLI と誤認しないため。",
    ...
    "const resolvedCli = repoLocalHarness ? repoLocalCli : null;",
    "if (!resolvedCli) {",
    '  console.error("[ut-tdd-wrapper] consumer_runtime_absent: no consumer-local UT-TDD entrypoint.");',
    '  console.error("[ut-tdd-wrapper] This wrapper resolves only <repo>/src/cli.ts inside this repository.");',
    '  console.error("[ut-tdd-wrapper] It never falls back to the machine that ran setup (PLAN-L6-101 §1.1).");',
    "  process.exit(127);",
    "}",
    ...
    "",
    "if (result.error) {",
    '  console.error("[ut-tdd-wrapper] Failed to launch the consumer-local ut-tdd entrypoint.");',
    '  console.error("[ut-tdd-wrapper] Verify `node src/cli.ts --help` inside this repository.");',
