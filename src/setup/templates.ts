...
export const BUILTIN_GITHUB_TEMPLATES: TemplateSet = {
  ...
  "common/ut-tdd.mjs": [
    ...
    "// exist (A-172 C-2). A consumer source tree is never treated as an executable CLI.",
    'const repoLocalCli = join(repoRoot, "dist", "ut-tdd.mjs");',
    "const repoLocalHarness = existsSync(repoLocalCli);",
    "const setupSourceCli = {{UT_TDD_SOURCE_CLI_JSON}};",
    "const sourceCli = repoLocalHarness ? repoLocalCli : setupSourceCli;",
    'const command = existsSync(localBin) ? localBin : existsSync(sourceCli) ? process.execPath : "ut-tdd";',
    "const args = command === process.execPath ? [sourceCli, ...process.argv.slice(2)] : process.argv.slice(2);",
