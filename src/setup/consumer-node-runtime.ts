...
type ConsumerNodeRuntimeInstallPhase =
  | "admission"
  | "staging"
  | "activation"
  | "reconcile"
...
export function buildConsumerNodeRuntimeBundle(
  ...
  if (
    historySequence > 0 &&
    (!DIGEST.test(priorBundleDigest) || !DIGEST.test(priorHistoryTipDigest))
  )
...
export function validateConsumerNodeRuntimeBundle(
  ...
  if (!within(bundle.identity.runtime_root, bundle.bundle_path))
    return "consumer_runtime_external_path";
  if (
    bundle.history_sequence === 0 &&
    (bundle.prior_bundle_digest !== GENESIS || bundle.prior_history_tip_digest !== GENESIS)
  )
    return "consumer_runtime_identity_mismatch";
  if (
    bundle.history_sequence > 0 &&
    (!DIGEST.test(bundle.prior_bundle_digest) || !DIGEST.test(bundle.prior_history_tip_digest))
  )
    return "consumer_runtime_identity_mismatch";
  return null;
}
  ...
  try {
    ...
    if (committed) {
      ...
      result ??= {
        ok: false,
        status: "failed",
        reason: "consumer_runtime_permission",
        phase,
        error: primaryError,
      };
    }
