 ...
 * accepted のいずれでもない PLAN について、その `generates` の **merged deliverable が実在
 * (existsSync)** なら「merged-but-unconfirmed」violation とする。draft でも deliverable が未 merge なら
 * (= 真に作業中) violation にしない。これにより「実装が merge されたら PLAN を confirm + review せよ」を
 * fail-close 強制する。
 *
...
/** deliverable を産出する kind (design/poc/reverse 等は出荷物を merge しないので status 正確性対象外)。 */
