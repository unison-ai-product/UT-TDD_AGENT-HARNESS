...
import { loadStrictCanonicalMemorySnapshot } from "../memory/service.ts";
import {
...
function loadState(repoRoot: string): FenceState | ProjectMemoryCompletionFailure {
  ...
  let corpus: Corpus;
  const snapshot = loadStrictCanonicalMemorySnapshot(resolved.canonicalProjectRoot);
  if (!snapshot.ok) return failure(snapshot.reason);
  ...
  corpus = {
    files,
    digest: snapshot.canonicalCorpusDigest,
    index: new Set(files.map((file) => `${file.memoryId}\0${file.digest}`)),
  };
