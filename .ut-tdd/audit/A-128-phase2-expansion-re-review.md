...
- **F-7/F-8 根本対処 (repo 外) — 解消 (2026-06-10 PM)**: PO の個別明示指示 (「2は消して」) により settings.json から ① env.PATH エントリ ② OPENAI_API_KEY 平文 2 エントリを削除 (JSON 妥当性検証済、env 反映は次 session から)。当初は classifier が「improve everything は明示授権でない」として 2 経路とも拒否 → 個別指示で通過 (境界設計どおり)。key は未使用と PO 確認済みだが、OpenAI ダッシュボード側の revoke 確認のみ PO 残タスク。
