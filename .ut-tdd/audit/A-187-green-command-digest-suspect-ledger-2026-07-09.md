...
- 2026-07-09 進捗: PLAN-L7-256 の placeholder digest 3 件を実再実行証跡で正規化し、`recoverable=0 / suspect=19 / already-anchored=776` へ改善。
- 2026-07-09 進捗: L6-48 / L7-393 / REVERSE-393 の `tests/vmodel-forward-freeze-contracts.test.ts` 3 件を合同再実行証跡で正規化。
- 2026-07-09 進捗: L7-405 / REVERSE-405 の `tests/spec-ir-projections.test.ts` 2 件を合同再実行証跡で正規化。
- 2026-07-09 進捗: L6-62 の `tests/secret-scan.test.ts` 1 件を再実行証跡で正規化。
- 2026-07-09 現在値: `recoverable=0 / suspect=13 / already-anchored=782`。
...
|---|---|---|
| PLAN-L6-48-vmodel-l2-freeze-l5-verification-design | tests/vmodel-forward-freeze-contracts.test.ts | resolved 2026-07-09: `bun run vitest run tests\vmodel-forward-freeze-contracts.test.ts tests\vmodel-pair.test.ts` 再実行 green、現在 file hash に置換し `anchor_commit=1afa132c...` を付与 |
...
| PLAN-L6-62-design-doc-secret-scan-gate | tests/secret-scan.test.ts | resolved 2026-07-09: `bun run vitest run tests\secret-scan.test.ts tests\doctor-rule-quality.test.ts tests\cli-surface.test.ts tests\relation-graph.test.ts` 再実行 green、現在 file hash に置換し `anchor_commit=1afa132c...` を付与 |
| PLAN-L7-256-model-id-ssot-drift-gate | tests/model-id-ssot-drift.test.ts | resolved 2026-07-09: targeted vitest 再実行 green、現在 file hash に置換し `anchor_commit=1afa132c...` を付与 |
| PLAN-L7-256-model-id-ssot-drift-gate | src/team/model-policy.ts | resolved 2026-07-09: `bun run typecheck` 再実行 green、現在 file hash に置換し `anchor_commit=1afa132c...` を付与 |
| PLAN-L7-256-model-id-ssot-drift-gate | src/setup/templates.ts | resolved 2026-07-09: `bun run lint` 再実行 green、現在 file hash に置換し `anchor_commit=1afa132c...` を付与 |
...
| PLAN-L7-393-vmodel-l2-freeze-l5-verification-gate | tests/vmodel-forward-freeze-contracts.test.ts | resolved 2026-07-09: L6-48/REVERSE-393 と同一テスト証跡として合同再実行 green、現在 file hash に置換 |
| PLAN-L7-405-spec-ir-detector-precision | tests/spec-ir-projections.test.ts | resolved 2026-07-09: `bun run vitest run tests\spec-ir-projections.test.ts` 再実行 green、現在 file hash に置換し `anchor_commit=1afa132c...` を付与 |
...
| PLAN-REVERSE-393-vmodel-l2-freeze-l5-verification-gate-backfill | tests/vmodel-forward-freeze-contracts.test.ts | resolved 2026-07-09: L6-48/L7-393 と同一テスト証跡として合同再実行 green、現在 file hash に置換 |
| PLAN-REVERSE-405-spec-ir-detector-precision-backfill | tests/spec-ir-projections.test.ts | resolved 2026-07-09: L7-405 と同一テスト証跡として合同再実行 green、現在 file hash に置換 |
