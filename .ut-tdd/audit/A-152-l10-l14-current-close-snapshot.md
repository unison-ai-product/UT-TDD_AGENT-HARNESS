...
- `bun src\cli.ts feedback list --emit`: `total=1803`, `gate=0`, `actionable=0`, `telemetry=1803`.
  - telemetry summary: `artifact_progress_yellow=749`, `missing-test-oracle-id=599`, `skill_acceptance_rate=225`, `skill_firing_rate=225`, `refactor_candidate:split-module=2`.
  - `bun src\cli.ts progress artifacts --color red --json`: `[]`; the prior A-155 red artifact was cleared by binding A-155 into PLAN-L3-05 and the L14 close required evidence set.
...
- GitHub PR #2 is the source-side CI surface; the latest pushed head must keep `harness-check` green after this evidence-binding update.
- `bun run test`: A-155 full local rerun passed 119 test files / 1231 tests.
...
- `bun src\cli.ts doctor --strict-green-command-digest`: green after A-155 rerun-bound digest rebind.
...
| HIGH | evidence integrity | Digest restamp must not be treated as green-command re-execution. | A-155 reran `typecheck`, `lint`, full `bun run test`, and `db rebuild`, then rebound 131 stale green-command entries across 57 PLAN files in the same packet. `doctor --strict-green-command-digest` is now green for current evidence. |
...
- **Evidence integrity**: green-command digest backlog is no longer open after A-155; future digest corrections must remain rerun-bound and must not be mechanical hash restamps.
