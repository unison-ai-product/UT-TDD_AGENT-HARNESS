...
| A150-03 | high | green evidence integrity | `green-command-digest` can be satisfied by restamping digest files; digest equality alone does not prove current-command rerun. | Partially remediated on 2026-06-30: normal `ut-tdd doctor` keeps digest mismatches as advisory, while `ut-tdd doctor --strict-green-command-digest` now fail-closes when any `output_digest` differs from the `evidence_path` hash. The stale digest backlog still requires rerun-bound correction before this can be claimed as closed; see A-153 for command-group classification. |
...
- `bun src\cli.ts doctor --strict-green-command-digest`: expected fail-close while stale digest evidence remains; this is the strict verification-cycle guard for green evidence integrity, not the normal local-close doctor mode.
