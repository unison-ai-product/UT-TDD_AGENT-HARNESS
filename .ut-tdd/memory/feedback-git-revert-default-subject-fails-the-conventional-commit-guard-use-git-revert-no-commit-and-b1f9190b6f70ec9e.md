...
updated_at: 2026-09-10T05:01:28.084Z
---
...
branch-type guard で決定的に赤になった。ローカルでは `node src/cli.ts github guard` で同じ判定 (exit 1) が再現する。
...
3. push 前に `node src/cli.ts github guard` を流し、直近の件名が全て guard を通ることを確認する。
