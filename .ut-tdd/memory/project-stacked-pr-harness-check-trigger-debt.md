...
2026-07-14にPR #56（base=設計PR #55）でharness-checkが起動しないことを実測。source workflowのpull_request branches:[main]が親子PRを除外する。concept §7.2/requirements §7.5の『全PRでharness-check』と不整合。PLAN-L7-197/221の後続訂正で、source/Packのtrigger contractとgithub-ci-policyの負例テストを追加し、stacked PRでも同一aggregate gateが走ることをGitHub receiptで固定する。branch protection等のproduction設定変更はPO承認後のみ。
