...
## 待ち行列 (この順。集合ではなく順序に意味がある)

1. **#146 の赤 2 件を修理して merge** (最安・即効)。
2. **#163 — plan lint に numeric-core 一意性の fail-close を追加**。3 より先である理由: 現状の衝突検出は
   偶然に依存しており (PR #116 では legacy inventory テストの副作用で偶然発覚)、検出器を入れずに改番
   だけ進めると並行レーンの同番号取り合いがそのまま再発する。**検出器 → 改番の順を崩さない**。
3. **#128 — PLAN-L7-421 の rekey**。#145 自身が「#128 を最初の proof slice にする」「27 組を一括改名
   しない」と定義しているため、#128 を 1 組目として手順 (予約 → append-only revision/rekey →
   alias event → 依存/工程表再投影) を確立し、残りへ適用する。
4. **train 2 の最初の一手 = doctor の check 別プロファイル取得** (修理ではなく計測)。

並行して、Claude が author した PR #172 / #173 / #174 のレビュー/マージが Codex レーンの担当
(PO ルール 2026-07-16、Claude は自 PR のレビューを自分で回さない)。
