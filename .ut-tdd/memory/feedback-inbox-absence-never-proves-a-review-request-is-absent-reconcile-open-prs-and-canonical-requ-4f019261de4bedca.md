...
updated_at: 2026-09-09T11:41:29.304Z
---
...
for(const f of fs.readdirSync(rq).filter(f=>f.endsWith(".json"))){const d=f.slice(0,-5);if(done.has(d))continue;
...
**注記 (この memory 自体の来歴):** 同じ規則を述べた先行 memory は 2 世代あり、いずれも配送から除外した。
...
(加えて requests 直下の `.md` を JSON.parse して落ちる潜在欠陥もあった)。
時点事実やコマンドを書くときは、その値・終了コードを両 OS で再現できる形で併記すること。
