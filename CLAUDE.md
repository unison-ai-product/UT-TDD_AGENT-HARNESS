...
複数 AI を使える場合は、判断と実行を分ける。Claude Code が作業主体なら設計レビューや R4/G2/G3/G4 判断は別 runtime の `frontier-reviewer` へ回し、実装・テスト追加・文書整形は `worker` に委譲する。単体 mode では self-review を許容するが、cross-agent review 不在は P1 warning として残す。
