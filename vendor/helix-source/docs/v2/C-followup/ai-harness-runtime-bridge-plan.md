# Phase H AI Harness Runtime Bridge 整理計画

最終更新: 2026-05-14  
作成: Research (Codex)  
対象: L1 `FR-AT01`〜`FR-AT05`  
位置づけ: Phase H の実装前に、agent/orchestration/cost/tool-contract/Codex/Claude/hook の散在を実行可能レベルで整理する計画

---

## 合流地点

本書は旧 Phase H AT 計画を改名した合流計画である。名称を `AI Harness Runtime Bridge` に広げる理由は、現行の課題が BaseAgent だけではなく、`helix-codex` / `helix-claude` / `helix team` / Claude Code subagent / PreToolUse・PostToolUse hook / Windows shim までまたがるためである。

合流先は以下で固定する。

- L1: `FR-AT01`〜`FR-AT05` を主軸にし、`FR-GR03`〜`FR-GR06` の hook feedback / stop と接続する。
- C-followup: `hooks-commands-sot-plan.md` の hooks / agents / commands SoT 整理を sibling plan として扱う。
- 実装: `cli/helix-codex`, `cli/helix-claude`, `cli/lib/team_runner.py`, `cli/lib/agent_policy_guard.py`, `.claude/hooks/`, `.claude/agents/`, `cli/roles/*.conf` を同一 harness boundary として扱う。
- 後続: PLAN-099/100 の runtime framework へ接続し、UserPromptSubmit / SessionStart / PreToolUse / PostToolUse の 5-layer runtime と衝突しないようにする。
