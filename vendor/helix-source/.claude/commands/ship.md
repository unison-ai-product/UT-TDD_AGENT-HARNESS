...
2. **`security-audit`** — Run a vulnerability and threat-model pass. Check OWASP Top 10, secrets handling, auth/authz, dependency CVEs. Output the standard audit report.
3. **`qa-test`** — Analyze test coverage for the change. Identify gaps in happy path, edge cases, error paths, and concurrency scenarios. Output the standard coverage analysis.
...
2. **Security** — Promote any Critical/High `security-audit` findings to launch blockers. Cross-reference with `code-reviewer`'s security axis.
