...
- `PLAN-L7-75-cost-tiered-provider-router`
  - commit: f48546d
  - commit: 601810b
- `PLAN-L7-76-review-remediation-reliability` (troubleshoot) — 本セッションの実完了
  PLAN。§1/§2 の機械 digest は active-plan ポインタが session 中 L7-76 へ切替らな
  かったため L7-75 を表示している (実作業は下記)。
  - commit: b168aa5 `fix(reliability): close verified L7 status-review findings`
  - pushed: `1af322e..b168aa5 codex/harden-automation-team-launch`
  - files: src/state-db/projection-writer.ts, src/runtime/agent-slots.ts,
    src/lint/change-impact.ts, src/doctor/index.ts, tests/{projection-writer,
    agent-slots,doctor}.test.ts, docs/test-design/harness/L7-unit-test-design.md,
    docs/plans/PLAN-L7-76-review-remediation-reliability.md

## §3 Next Action

外部ステータスレビュー (`ut-tdd-latest-fix-status-review.md`, 13 件) を**現ブランチ
実コードで全数裏取り**し、現コードで実在しガバナンス判断不要な信頼性欠陥 3 件を
test-first で修正・コミット・プッシュ済み。

1. **修正済 (Red→Green)**:
   - P0 `rebuildHarnessDb` 原子性 — truncate+全 project\* を `BEGIN IMMEDIATE`/
     `COMMIT`/`ROLLBACK` で包囲 (`U-DBPROJ-ATOMIC-01`、pre-fix で plan_registry
     188→0 破損を実証)。
   - P1 非Git `doctor` — `isGitRepository` 追加、change-impact /
     change-set-integrity を非Gitで skip(ok)。tracked-canonical /
     runtime-portability の既存非Git fail-open 慣行に整合。git在りで status壊れは
     fail-close 維持、CI不変 (`U-CHGIMPACT-NONGIT-01`)。
   - P1 `agent-slots` アトミック書込 — `nodeAgentSlotsDeps.writeText` を temp+
     `renameSync` 化 (`U-SLOT-009`)。
2. **次に着手可 (私側で実装可)**: #3 session digest の追記取りこぼし
   (`session-log.ts:230` の session_id 単位 fold)。event 単位 high-watermark
   idempotency へ。スキーマ/意味論変更を伴うので別 PLAN 推奨。
3. **README/GitHub 確認** (本セッション後続タスク、§下記参照)。

## §4 carry (未了・先送り)

PO ガバナンス/契約方向判断が要る指摘 (即興せず escalate、CLAUDE.md 外部API/契約
変更ルール):

- #5 MCP config `command`/`args` — 有効。正しい修正は profile 毎 `mcpCommand`/
  `mcpArgs` 契約 (外部 launcher 契約のスキーマ設計変更)。
- #7 requirements の current/future/carry CLI 分離。
- #8 `status --json` field 契約の一本化方向 (doc⇄実装)。
- #9 `skill suggest` I/O 契約方向 (`--plan` path/id、`--text`、3バケット)。
- #10 `repository-structure.md` の `src/` ツリー陳腐化 (doc 更新、要 src 突合)。
- #12 / #13 module-drift / builder catalog 文言整合。

既対応 (現コードで不成立): #6 README 実在 (343 行) / #11 PLAN ownership は doctor
`impl-plan-trace` OK (baseline 被覆)。

## §5 未了 PO 判断

- #7/#8/#9 の契約方向 (どちらを正本に一本化するか) は PO 指示待ち。
- #3 session digest と #5 MCP launch 契約を別 PLAN で着手するか。

## §6 壊さない / 再発させない

- **外部レビューを ZIP ハッシュ一致で「未修正」と推定するな**。指摘は必ず現ブランチ
  実コードで一件ずつ裏取りしてから直す/退ける ([[feedback_verify_carry_status_against_code]]
  の外部レビュー版)。13 件中 #6/#11 は既対応で、誤って「未修正」とされていた。
- `rebuildHarnessDb` の truncate+project\* は必ずトランザクション内に保て。途中失敗で
  harness.db を破壊する退行を戻すな。
- 非Git skip は「git 不在」限定。git 在りで status が壊れる実エラーは fail-close を
  維持しろ (skip に巻き込むな)。CI は常に git なので非Git skip は CI を緩めない。
- agent-slots は temp+rename のアトミック書込を維持。直書き `writeFileSync` に戻すな
  (torn-write 破損が再発する)。
