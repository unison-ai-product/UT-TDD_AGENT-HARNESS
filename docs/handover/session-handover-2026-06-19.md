...
本 session (Opus / PO goal=「残タスクの解消。PO 判断待ちは TL 相談で進める」) の実完了。
前 handover の §4 minor carry + §5 PO-pending を、判断系は **cross_agent TL(Codex/gpt-5.5) 相談 → 残差のみ PO** の
順で処理した (5 commit、全 green、未 push)。

1. **§4 model-id SSoT (`2ce6f29`)**: `TIER_TABLE` ⇔ `modelForProvider` の model ID 二重 literal を
   `MODEL_IDS` カタログ 1 箇所へ集約 (behavior-invariant)。U-MODELID-001..004 が合成一致 + 生 literal 不在を検査。
2. **TL cross-review (A-138)**: §5 PO-pending 4 件を live `ut-tdd codex --role tl --task-file … --execute`
   (codex-gpt-5.5、真の cross_agent) で裏取り。ITEM-1 adjust / ITEM-2 adjust / ITEM-3 agree / ITEM-4 agree。
3. **ITEM-4 handover slim-stub (`4e0ac24`)**: 同日 2 件目以降の §1/§2 を参照 stub に縮約 (header 数不変で
   bypass 契約維持)。U-HOVER-013。**本エントリが slim 化の初実例** (4 件目 = 25 行 vs 初回 360 行)。
4. **ITEM-2 skill suggest `--text` (`de54cf8`)**: 自由文 suggest を additive で追加 (flat-list 維持、`--plan`
   相互排他)。3-bucket は PO 残課題。
5. **ITEM-3 IMP-085 feedback-log lint (`85cd9a7`)**: FB ドメスティック化 (open/空/dangling) を doctor
   hard gate 化。PLAN-L7-82、cross_agent QA(codex-gpt-5.5) verdict=pass、evidence A-139。
6. **ITEM-1 taxonomy + backlog disposition (`058c731`)**: status field の current/future/carry 区分を
   requirements に明記 (TL-decidable)。IMP-082/083 → (b) Reverse FR 先、IMP-107 → next PLAN と記録。

検証: typecheck + biome(174) + Vitest **743/743** + doctor EXIT=0 (全 gate green、handover pointer 同期済)。

次 session の起点: ①下記 §5 の PO 判断。②PO 承認後の Reverse FR 起票 (IMP-082/083) + PLAN-L7-83 (IMP-107)。
③未 push (main=`63ad052` より 5 commit 先行) の push/merge 判断。

## §4 carry (未了・先送り)

AI-decidable な §4/§5 carry は本 session で実装完了 (上記)。残るは PO 判断前提のもの:

- **IMP-107 (placeholder_deps 2 類型 threshold rule)**: TL-cleared だが正しい rule = IT-ASSET-07 の threshold
  (waiting_layer 到達前=carry / 到達後=failure) で **layer-completion 連動が必要**。実 repo に正当な L6 type-1
  carry 複数ゆえ単純一般化は green を壊す = 誤実装すると false-confidence 再生。**next PLAN-L7-83** で慎重実装。
- **IMP-082/083 (descent/test-design substance gate)**: TL = (b) Reverse FR 起票が先 (Type② 検査カテゴリ追加)。
  PO が新 FR 族 (`FR-vmodel-descent-substance` / `FR-test-design-substance`) を正本要件に起票後、AI が impl。

## §5 未了 PO 判断 (TL 裏取り済の残差のみ、A-138 RESIDUAL_FOR_PO)

- **next_action を carry として PLAN 化する優先度** + `status --json` の正式フィールド名 (`next_action`/`nextAction`)
  の確定 (公開契約、ITEM-1)。区分 (future/carry) は TL-decidable で適用済。
- **skill suggest の 3-bucket 出力契約** (bucket 名 = required/recommended/optional 素案、振り分け基準、
  flat list との互換) = 製品契約変更 (ITEM-2)。`--text` additive は実装済。
- **handover slim-stub の人間可読フォーマット採否** (ITEM-4、機械契約は header 数不変で安全、format 嗜好のみ)。
- **IMP-082/083 の Reverse FR 族を正本要件に起票するか** (governance、新 FR = 新 required behavior)。

## §6 壊さない / 再発させない

- **判断系は self-review → 相手 provider クロスレビュー → 残差のみ PO** ([[feedback_cross_review_before_po_escalation]])。
  本 session は A-138 で 4 件まとめて DESK REVIEW 制約 front-load で逸れ無く裏取りできた (live codex 成功)。
- **model ID は `MODEL_IDS` (model-policy) が単一正本**。`TIER_TABLE`/`modelForProvider` に生 literal を再ハードコード
  するな (U-MODELID-003/004 が fail-close)。価格表 token-tracker は外部 pricing 由来の別正本で統合しない。
- **handover 同日 2 件目以降は slim** (PLAN-L7-82 でなく ITEM-4)。`# Session Handover` header は 1 エントリ 1 個を
  維持する限り `countHandoverEntries`/`doc_entry_count` の bypass 契約は不変。slim を消すと累積肥大が再発。
- **feedback-log の memory `[[name]]` は存在突合しない** (repo 外 private)。CI 前提に agent-private path を持ち込むな。
- **substance gate (IMP-107/082/083) を要件化/threshold 設計なしに急いで実装するな**。誤実装は gate 自身が警告する
  false-confidence を生む ([[feedback_coverage_not_substance]])。TL clearance は「実装可能」だが「rule 設計は別」。
