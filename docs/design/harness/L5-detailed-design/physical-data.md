...
| `plan_id_reservations` | `reservation_id TEXT`, `namespace TEXT`, `ordinal INTEGER`, `asset_id TEXT`, `lease_token_hash TEXT`, `reserved_at TEXT`, `expires_at TEXT`, `released_at TEXT?` | PK=`reservation_id`、active `(namespace,ordinal)` UNIQUE、token生値保存禁止 | namespace+ordinal / expiry index |
| `legacy_plan_migrations` | `migration_id TEXT`, `legacy_plan_id TEXT`, `asset_id TEXT?`, `decision TEXT`, `collision_group TEXT?`, `loss_fields_json TEXT`, `reason TEXT`, `review_plan_id TEXT?`, `source_digest TEXT` | PK=`migration_id`、legacy ID UNIQUE、ambiguous/loss時asset NULLかつreview PLAN必須 | decision/collision index |
| `workflow_transition_events` | `event_id TEXT`, `subject_id TEXT`, `subject_revision INTEGER`, `sequence INTEGER`, `command_id TEXT`, `from_state TEXT`, `to_state TEXT`, `resume_state TEXT?`, `actor TEXT`, `reason TEXT`, `source_commit TEXT`, `occurred_at TEXT`, `event_digest TEXT` | PK=`event_id`、`(subject_id,sequence)`/`command_id` UNIQUE、subject FK、例外時reason必須 | subject+sequence昇順 / command index |
| `evidence_records` | `evidence_id TEXT`, `subject_id TEXT`, `subject_revision INTEGER`, `evidence_kind TEXT`, `source_commit TEXT`, `digest TEXT`, `producer TEXT`, `produced_at TEXT`, `expires_at TEXT?`, `supersedes_id TEXT?` | PK=`evidence_id`、subject/revision FK、accepted producer/kind CHECK、expiry/supersedesのみnullable | subject+revision+kind index |
| `workflow_event_evidence` | `event_id TEXT`, `evidence_id TEXT`, `requirement_id TEXT` | PK=`(event_id,evidence_id,requirement_id)`、event/evidence FK | event / evidence索引 |
