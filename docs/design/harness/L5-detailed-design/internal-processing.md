...
| `roster list` | roster | FR-L1-46 | 未 (PLAN-X-03 spike 実証) |
| `roster check` | roster | FR-L1-46/48 | 未 (PLAN-X-03 spike 実証) |
| `ut-tdd asset` | roster | FR-L1-48 | 未 (L6 carry、`waiting_layer:L6`。内部資産 inventory/管理 command は roster の command パターン確定後に L6 で詳細化、Discovery spike 対象外) |
...
| `roster check` | allowlist 突合 report 生成 ∧ **missingFromRoster=0 ∧ nameMismatches=0 で ok/exit 0、乖離ありで fail-close/exit 1** ∧ state 不変。PLAN-X-03 で nonAllowlisted=4 (be-* / db-schema / devops-deploy = Codex 委譲組) は乖離でなく既知集合 |
