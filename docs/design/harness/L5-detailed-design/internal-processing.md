...
`cutover-evidence.v1`、`subject_revision`はproducer slice commitである。先行7 fieldを固定順JSON arrayへ
...
`SliceAdmissionReceipt`を封印し、transitionの`admission_digest`はそのevidence receipt digestと一致する。
