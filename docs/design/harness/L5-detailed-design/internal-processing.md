...
Node配布単位は`target別Node runtime image + manifest + compiled ESM + production dependencies + Rust companion + SBOM + signature`であり、片側更新を禁止する。`RuntimeBundleVerifier`は署名、target、Node runtime version/digest/license/SBOM identity、全file digest、Rust protocolを照合し、verified handleだけを`NodeEntrypointResolver`へ渡す。resolverはbundle root内のabsolute Node executableとESM argvだけを返し、system Node、PATH探索、runtime download、Bun/tsx/TS直実行fallbackを持たない。bootstrap build/CIのseed Nodeはproduction runtime authorityではなく、exact pin/digestをbuild receiptへ封印する準備用toolchainとする。
