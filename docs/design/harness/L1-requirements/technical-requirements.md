...
| `pair/` | W-model pair 対応 (L1↔L14 等) の freeze 状態 | `ut-tdd gate` |
...
| `recipe_store/` (A-52 audit C-1、Phase B) | Learning Engine の成功パターン蓄積 (FR-L1-19、`pattern_key` 付き event-sourced log) | L4 データ設計 sub-doc / Phase B 実装 |
| `accuracy_score/` (A-52 audit C-1、Phase B) | recipe 適用精度・skill 推薦精度 (FR-L1-19 / observability-metrics.md §トラブル計測) | L4 データ設計 sub-doc / Phase B 実装 |
