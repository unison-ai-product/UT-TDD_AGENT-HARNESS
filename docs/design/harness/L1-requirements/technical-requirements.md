...
| **harness state** | ファイルベース (`.ut-tdd/` 配下) + `.ut-tdd/harness.db` SQLite projection | 採用確定済 (functional-requirements.md / L4 data.md 準拠。旧「DB は L2/L4 で検討」は決定済につき PLAN-L7-453 M8 で更新)。file state が正本、DB は projection |
