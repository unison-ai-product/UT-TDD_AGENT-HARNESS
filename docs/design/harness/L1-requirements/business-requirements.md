...
| **BR-22** | **自前 runtime 内部資産体系を持つ** — UT-TDD は自身が使う/対象に提供する **subagent roster / skill pack / command** を UT-TDD 用の正本資産として持ち、HELIX 由来資産を「そのまま使う」のでなく **UT-TDD 用に再構築**する。guard (呼出統制) だけでなく資産そのものを統制対象とする (再構築の HOW = FR-L1-46〜49) | A-77 PO 指摘 (前提抜け) / Recovery PLAN-X-02 |
...
| BR-22 内部資産 (subagent roster / skill pack / command) の UT-TDD 化詳細 (FR-L1-46〜49 / 棚卸 = internal-asset-inventory.md / porting-map W6/W7/W10) | L3 FR / L4-L6 | Recovery PLAN-X-02、設計増分は L4-L6 |
