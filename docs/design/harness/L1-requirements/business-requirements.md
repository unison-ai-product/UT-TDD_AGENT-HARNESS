...
| **BR-22** | **自前 runtime 内部資産体系を持つ** — UT-TDD は自身が使う/対象に提供する **subagent roster / skill pack / command** を UT-TDD 用の正本資産として持ち、HELIX 由来資産を「そのまま使う」のでなく **UT-TDD 用に再構築 (harden / curate / CLI 化)** する。guard (呼出統制) だけでなく資産そのものを統制対象とする | A-77 PO 指摘 (前提抜け) / Recovery PLAN-REC-001 |
