...
| **state-db** (`src/state-db/`) | `.ut-tdd/harness.db` projection の `node:sqlite` adapter + registry-driven migration (PRAGMA user_version) + idempotent upsert。runtime 出し分けや Bun fallback は設けない。projection 充填は span ② で配線 (PLAN-L7-44/45) | `openHarnessDb()` / `migrate()` / `upsertRow()` / `harnessDbStatus()` | schema (harness-db registry) / kernel alias resolver / fs |
