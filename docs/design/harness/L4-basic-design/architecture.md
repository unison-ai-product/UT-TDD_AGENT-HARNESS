...
| **projection** (`src/projection/`) | DB非依存のprojection domain、application command、意味的read/store portを所有する。SQLやfilesystemをdomain/applicationへ漏らさず、具象adapterは`state-db`側で実装する | `ProjectionStore` / `PocEvaluationReadPort` / `projectPocEvaluations()` | (なし — domain/application/contracts内で閉じ、state-dbへ逆依存しない) |
