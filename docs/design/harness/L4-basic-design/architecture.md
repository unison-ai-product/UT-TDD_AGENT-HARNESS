...
| **plan-asset** (`src/plan-asset/`) | immutable PLAN identity/revision/evidence/reservationとv1 canonical adapter。short alias多義をfail-closeし、state-db/CLIへpure verdictを提供する | `PlanAsset.create/reconstruct/revise()` / `EvidenceRecord.isUsableFor()` / `PlanIdReservation.reserve()` / `adaptLegacyPlan()` / `loadProjectIdentityFromHead()` | node:crypto（domain）/ Git・fs（adapterのみ） |
