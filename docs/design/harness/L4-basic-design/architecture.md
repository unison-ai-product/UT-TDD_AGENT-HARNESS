...
| **git** (`src/git/`) | default branch の ref→SHA 解決だけを持つ副作用最小の git 参照 adapter (PLAN-L7-461)。ref 依存 check (`memory-sync` / `merged-plan-status`) と snapshot ref 注入・doctor envelope の観測面記録が**同じ規則で解決した値**を突き合わせられるようにするための SSoT。解決不能な面では null / 空 map を返し、存在しない ref を推測で埋めない (fail-close を壊さない) | `resolveDefaultBranchRef()` / `defaultBranchRefMap()` / `headSha()` | (なし — git CLI のみ) |
...
| **memory** (`src/memory/`) | `.ut-tdd/memory/*.md` を authored source とする Claude/Codex 共有 memory。`memory_entries` へ deterministic projection する。**読み路の入口は `MemoryService` (`src/memory/service.ts`) で、本文は正本ファイルから読み、DB は `content_hash` 照合の派生 index として扱う**。SessionStart は runtime を止めないが、DB 障害時は無音ではなく劣化 digest を出す (PLAN-L7-189 §5)。secret-like payload は write/parse 時に fail-close する (PLAN-L7-189)。 | `writeMemoryEntry()` / `loadMemoryEntries()` / `selectMemoryEntries()` / `renderMemorySurface()` | secret / state-db type / fs |
