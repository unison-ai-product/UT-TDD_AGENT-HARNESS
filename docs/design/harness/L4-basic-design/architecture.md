...
| **plan-asset** (`src/plan-asset/`) | immutable PLAN identity/revision/evidence/reservationとv1 canonical adapter。`.ut-tdd/ledger/harness-ledger.db`をcanonical event/receipt正本として所有し、short alias多義をfail-closeする | `PlanAsset.create/reconstruct/revise()` / `EvidenceRecord.isUsableFor()` / `PlanIdReservation.reserve()` / `PlanLedger.reserve/release/expire()` / `adaptLegacyPlan()` / `loadProjectIdentityFromHead()` / `migratePlanLedger()` | schema typed DDL / state-db SQLite port / node:crypto / Git・fs（adapterのみ） |
