...
| **execution** (`src/execution/`) | Execution Ledger の Forward 外遷移 (escape) 契約層 (PLAN-L6-83 / PLAN-L7-452)。境界分類、`RequestForwardEscape` validation (fail-close、冪等 payload digest)、drive model 三面一致、Issue body projection と reconcile を GitHub SDK 非依存の純粋関数 + port で提供する。episode 集約・永続化は L7-436 系列で拡張する。 | `classifyForwardBoundary()` / `validateForwardEscape()` / `projectForwardEscapeIssue()` / `reconcileIssueProjection()` | node:crypto (digest) のみ (state-db / GitHub SDK へ依存しない) |
