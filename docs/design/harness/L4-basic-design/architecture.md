...
| **state-db** (`src/state-db/`) | `.ut-tdd/harness.db` projection の SQLite adapter (bun:sqlite first / node:sqlite fallback、runtime 出し分け) + registry-driven migration (PRAGMA user_version) + idempotent upsert。projection 充填は span ② で配線 (PLAN-L7-44/45) | `openHarnessDb()` / `migrate()` / `upsertRow()` / `harnessDbStatus()` | schema (harness-db registry) / plan-asset alias resolver / fs |
...
| **plan-asset** (`src/plan-asset/`) | immutable PLAN identity/revision/evidence/reservationとv1 canonical adapter。short alias多義をfail-closeし、state-db/CLIへpure verdictを提供する | `PlanAsset.create/reconstruct/revise()` / `EvidenceRecord.isUsableFor()` / `PlanIdReservation.reserve()` / `adaptLegacyPlan()` | node:crypto（domain）/ Git・fsは将来adapterのみ |
