...
| **git** (`src/git/`) | commit と tracked path を shell 展開なしで実 Git object の regular blobへ解決する共有境界。`plan-admission` と `plan-asset` が互いを import せず同じ provenance 判定を使えるよう、Git command port と検証済み blob valueを所有する。working tree、index、remote URLからの補完は禁止する。 | `TrustedGitBlobResolver.resolve()` / `GitCommandPort` / `NodeGitCommandPort` / `TrustedGitBlob` | node:child_process（Node adapterのみ） |
| **execution** (`src/execution/`) | Execution Ledger の Forward 外遷移 (escape) 契約層 (PLAN-L6-83 / PLAN-L7-452)。境界分類、`RequestForwardEscape` validation (fail-close、冪等 payload digest)、drive model 三面一致、新規Issue projection、既存Issue本文を不変に保つadoption、reconcileをGitHub SDK非依存のdomain + portで提供する。E2 custodyと二系統のE3/E4 FSMはSQLite adapterでclose/reopen可能なdigest chainへ永続化する。episode 集約は L7-436 系列で拡張する。 | `classifyForwardBoundary()` / `validateForwardEscape()` / `projectForwardEscapeIssue()` / `adoptForwardEscapeIssue()` / `reconcileIssueProjection()` / `SqliteForwardEscapeJournal` | node:crypto / state-db (SQLite)。GitHub I/Oはadapterへ隔離 |
