...
| **projection** (`src/projection/`) | DB非依存のprojection domain、application command、意味的read/store portを所有する。SQLやfilesystemをdomain/applicationへ漏らさず、具象SQLite adapterは`state-db`側で実装する | `ProjectionStore` / `PocEvaluationReadPort` / `ModelEvaluationReadPort` / `projectPocEvaluations()` / `projectModelEvaluations()` | domain/application/contractsは内部依存のみ。repository config adapterだけがnode:fs/path + yamlへ依存し、state-dbへ逆依存しない |
