...
| `ut-tdd asset` / `ut-tdd builder` | FR-L1-48 | 未 | 内部資産 (skill pack / command / builder) カタログ (porting-map W11/W12/W16) |
...
> **定量テスト → 定性レビュー順序 (全駆動モデル普遍、IMP-077)**: 上表の各駆動モデルの状態遷移は **定量 verify step → 定性 review/サインオフ step** の順 (Discovery=S3 verify→S4 decide / Scrum=increment テスト→S3 レビュー / Reverse=③テスト設計状態確定→R4 / Incident=収束確認→postmortem / Refactor=テスト緑確認→commit / Retrofit=L8 回帰→exit / Add-feature=テスト確認→V 整合 / Research=候補比較→ADR)。共通機械アンカー = `review_evidence.tests_green_at ≤ reviewed_at` を doctor `checkReviewEvidence` が fail-close (concept §2.1.2.1 核心ルール 6)。未検証成果物をレビューさせない。

### §3.2 signal → mode routing (優先度モデル、FR-08)

検出 signal は `runtime(detect)` + `mode-routing.yaml` を介して駆動モデルへ自動 routing される。**失敗 routing の全順序 (total order、concept §2.6 / gate-design §1.1 正本)** — 複数の失敗系 signal が競合したときの解決順:
...
|---|---|---|
| `runtime(detect)` + `mode-routing.yaml` | signal 検出 → 優先度 routing (§3.2) | architecture.md §3.1 runtime / §4.1 |
| **workflow orchestration module** (将来) | 駆動モデルの phase/step 駆動 + Forward 合流 | architecture.md §3.1「将来 workflow module」 |
...
- **review_kind / cross_agent_review の記録着地先 (F-2)**: claude-only/codex-only で記録する `review_kind=intra_runtime_subagent` + `cross_agent_review=unavailable` は **PLAN frontmatter の `review_evidence` (reviewer/review_kind/verdict/scope) に着地** (data.md §6 Plan 不変条件)。**`doctor checkReviewEvidence` が hard 検証** (review-skip freeze を fail-close、IMP-071) = mode 別 review tier 縮退が doc-only でなく機械担保される。詳細 schema は L5 D-CONTRACT で確定。
...
- **cross-review の semantic 強制 (PO 問い、under-design 残 → IMP-076)**: concept §2.1.2.1 は cross-review の**実行時強制 3 種**を設計済 — ① `same_model_approval: forbidden` (cross_agent なのに worker≡reviewer の同一 (provider,model) なら承認無効化し exit) / ② `review_kind` ↔ execution mode 整合 (claude-only/standalone が `cross_agent` を僭称できない = self-review が cross-agent に化けない) / ③ checklist 逐条 pass/fail/n-a 記録 (§7.8.7 checklist)。**現状の機械強制は review_evidence の presence + review_kind まで (IMP-071)**。上記 3 semantic は **worker 識別子 (provider,model) の記録が前提**のため未実装 = **IMP-076 として機械着地予定** (review_evidence に worker field 追加 + checkReviewEvidence で same_model_approval / kind↔mode 検査)。concept §2.1.2.1 設計済の「実行時強制」未実装分の**明示 defer** (carry+IMP 紐付け = under-design でない、IMP-071 が presence/kind を閉じた続き)。
