...
- **担当 building block**: mode 検出 = `runtime` (`detectMode()`、§3.5 / architecture §5)。provider 引継ぎ (Claude↔Codex context+PLAN+budget、FR-L1-42) = `runtime/provider-handover.ts` + `ut-tdd handover provider export/status` (package は `.ut-tdd/handover/provider/` に保存)。runtime 委譲 surface = `ut-tdd codex` / `ut-tdd claude` dry-run contract plan + `--execute` 明示実行。
