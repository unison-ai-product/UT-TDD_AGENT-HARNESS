...
artifact_role: topic_setup_solo_team
...
**PLAN-L7-157 distribution addendum**: `buildCleanDistributionPlan(paths, tag, cleanRepo)` は R1/R2/R12/R13 の clean export contract である。既定の clean distribution repo は `unison-ai-product/UT-TDD_AGENT-HARNESS-Pack` であり、source development repo へ逆流させない。clean-repo + signed-tarball channel を emit し、Claude/Codex hook、subagent、command templates を含む LICENSE/package/src/adapter templates を含め、dogfood (`docs/plans`, `docs/design/harness`, `docs/test-design`, `.ut-tdd`) と UI (`src/web`) を除外し、required-file または denylist drift で fail-close する (U-SETUP-011)。`buildConsumerReadinessPlan(host/runtime/workspace/tag signals)` は R3/R5/R6/R8/R9/R10/R16/R17 の onboarding/readiness contract である。host を mutate せず、consumer `package.json` の `engines.node` (npm semver) と Git を正本に Node/npm + git/project-local `ut-tdd`/runtime preflight、CI self-sufficiency、rollback managed paths、tag-pin/public contracts、monorepo package-root placement、portability smoke scenarios を報告する (U-SETUP-012)。Node が missing、`engines.node` が missing/invalid、または Node version が range 外なら blocking とする。gh は GitHub setup 用 warning に留め、Claude/Codex runtime CLI が未ログインまたは未導入でも setup/readiness 自体は standalone mode の advisory として継続できる (judgment gate は人間レビュー必須として表示)。generated Claude/Codex adapter hooks は project-local `.ut-tdd/bin/ut-tdd.mjs` を invoke するため、project-local `ut-tdd` が解決できない場合 readiness は fail-close する。`checks`、`ci.requires`、`rollback.commands` は Node/npm 経路だけを提示し、Bun の check・導入案内・実行コマンドを含めない。install flow は `ut-tdd setup` 前に linked/package binary を確立しなければならない。local distribution acceptance smoke (U-SETUP-013 / AT-DIST-001) は planned clean artifact set を temporary repo に materialize するが、移行中の fixture では install/status/setup/typecheck に Bun を使う。distribution plan の実行側は `process.execPath` で Node を直接起動する。clean artifact は dogfood PLAN/design/test-design/runtime state を意図的に除外するため、source-repo full `doctor` は consumer-safe doctor profile でのみ扱う。
...
1台のPCに複数の consumer project が同居する前提では、global `bun link` / global `ut-tdd` を hook の正本にしない。`ut-tdd setup` は各 project に project-local の `.ut-tdd/bin/ut-tdd.mjs` だけを投影する（旧 `.ut-tdd/bin/run-bun.ts` は撤去済み）。Claude hook は Node.js 22.18 以上の無フラグ TypeScript execution を使う `node` exec-form で、`.ut-tdd/bin/ut-tdd.mjs` と hook 固有 subcommand を argv として直接渡す。Codex hook は文字列 serializer の能力境界により従来形式を維持する。wrapper は `process.execPath` で project root の `node_modules/ut-tdd/src/cli.ts`、次に setup を実行した harness checkout の `src/cli.ts` を直接起動し、shell や別ランタイムを経由しない。`.cmd`・shell・bare global fallback は使用せず、entrypoint が無ければ exit 127 で fail-close する。
...
- Node.js >=22.18 は Claude/Codex adapter と project-local wrapper の無フラグ TypeScript execution に必要である。生成 consumer の hook 実行に Bun は要求せず、UT-TDD harness binary は project dependency として解決する。
...
Claudeで使う hook の意味論は `executable="node"`、`argv[0]=".ut-tdd/bin/ut-tdd.mjs"`、残りを hook 固有
subcommand とする。`.ut-tdd/bin/run-bun.ts` は存在せず、`command` に argv、quote、redirect、pipe、`&&` 等を埋め込んではならない。
Windows では wrapper の子プロセス起動に `windowsHide: true` を指定する。
