...
| `parseScheduleEntries` | `ParseScheduleEntriesInput { bundle } -> ScheduleEntryDraftResult { entries[]; findings[] }` | PLAN-L6-39/383/PLAN-L7-471。post: `docs/governance/vmodel-upgrade-schedule.md` を第一入力として `schedule_entries` row draft (`schedule_entry_id`, `plan_id`, `plan_revision`, `layer`, `sub_doc`, `v_pair`, `predecessor_plan_ids`, `current_location`, `rag`, `status`, `blocked_reason`, `source_path`, `source_hash`, `indexed_at`) を返す。`plan_revision` は対応PLANの `admission_receipt.binding.revision`、legacy PLANではself-referenceするreview/admission証跡を除外したcanonical content tokenから解決する。専用工程表に掲載された `plan_id` は PLAN frontmatter fallback より優先し、未掲載 PLAN のみ fallback 生成する。対応PLANを解決できない工程行はwrite-zeroとする。空 current_location、未知 rag は finding 候補。invariant: `plan_revision` と文書全体fingerprintの `source_hash` は別namespaceであり、工程表はPLAN status / dependenciesを直接 mutate しない。 |
| `queueGithubProjection` / `markGithubProjectionApplied` / `markGithubProjectionFailed` | `QueueGithubProjectionInput { db; repositoryId; planId; planRevision; operation: project-item-upsert; payload: { owner; projectNumber; readiness; currentGate; headSha }; now? } -> outboxId` / `(db,outboxIds,now?) -> void` | PLAN-L6-85/PLAN-L7-471。post: payloadを閉じた5項目・固定順のcanonical JSONへ正規化し、SHA-256 `payload_digest`と同一rowへ保存する。未知field、空文字、不正Project番号はwrite前に拒否する。同一repository/PLAN/revision/operationかつ同一digestの再送だけstatus/attemptを保持する。digest変更時はdigestを含む新outbox identity、`pending`、attempt 0、空errorへ原子的に置換し、旧identityの完了通知をno-opにする。成功はexact outbox identityだけをappliedへ進め、失敗はattemptを加算してclosed reason `project-sync-failed`を保存しpendingを維持する。invariant: secret、credential、provider transcript、raw error、episode IDの推測値を保存しない。 |
| `syncRepositoryBindings` | `RepositoryBindingSyncInput { db; repositoryId; repoRoot; gh; now } -> RepositoryBindingSyncResult` | PLAN-L7-471。typed PR traceのPLANを`canonical PLAN source`、`schedule_entries.plan_revision`、trace revisionの三者でexact照合する。PLAN projection不在は`plan-projection-unavailable`、canonical source不在は`plan-source-unavailable`、revision不一致はstaleとしてprovider観測をwrite-zeroにする。数値admission revisionだけでなくlegacy canonical tokenにも同じfail-closeを適用し、工程表hashやtrace自己申告でrevisionを補完しない。 |
| `readGithubEvidence` / `deriveStoredForwardReadiness` / `rebuildExecutionReadiness` | `(db,repoRoot,repositoryId?) -> ForwardEvidence[] / ForwardReadinessRow[]` / `(db,repoRoot,now?) -> ForwardReadinessRow[]` | Project同期のderiveではrepository IDを必須入力としてbindingとProject itemを同一repositoryへ限定する。別repositoryの同一PLAN/revision/check/PRをProject mutationへ混入させない。一方、PK=`plan_id`の永続`execution_readiness_projection`はrepositoryで分割できないため、rebuildはrepository引数を持たず常に全体snapshotを保存する。scoped deriveでglobal projectionを削除・置換してはならない。 |
