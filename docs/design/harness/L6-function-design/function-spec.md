...
| `recordTestRunEvidence` | `TestRunEvidenceInput { command; runner; scope; started_at; completed_at; exit_code; evidence_path; cases? } -> RecordTestRunEvidenceResult { refs[] }` | implemented pseudocode: `src/workflow/contracts.ts` に実装済み。Node/Vitest/doctor/lint evidence を UT history projection に集約し、failure digests を redact し、raw provider transcripts は永続化しない。 |
...
runtime entrypoint は sealed TypeScript/Node artifact を正本とし、OS wrapper は thin に限る。`scripts/ut-tdd` は Linux/POSIX `sh` entrypoint である。これは `set -e` を有効化し、review 済み `dist/ut-tdd.mjs` を `node` で実行する。Bun/bunx/tsx/TS 直実行への fallback、Bash-only syntax、Python runtime dispatch、legacy runtime name を導入してはならない。
