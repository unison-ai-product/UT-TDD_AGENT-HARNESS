...
| `parseScheduleEntries` | `ParseScheduleEntriesInput { bundle } -> ScheduleEntryDraftResult { entries[]; findings[] }` | PLAN-L6-39/383/PLAN-L7-471。post: `docs/governance/vmodel-upgrade-schedule.md` を第一入力として `schedule_entries` row draft (`schedule_entry_id`, `plan_id`, `plan_revision`, `layer`, `sub_doc`, `v_pair`, `predecessor_plan_ids`, `current_location`, `rag`, `status`, `blocked_reason`, `source_path`, `source_hash`, `indexed_at`) を返す。`plan_revision` は対応PLANの `admission_receipt.binding.revision` から解決し、receiptを持たないlegacy PLANだけ `source_hash` へfallbackする。専用工程表に掲載された `plan_id` は PLAN frontmatter fallback より優先し、未掲載 PLAN のみ fallback 生成する。plan_id 未解決、空 current_location、未知 rag は finding 候補。invariant: `plan_revision` と内容fingerprintの `source_hash` は別namespaceであり、工程表はPLAN status / dependenciesを直接 mutate しない。 |
| `queueGithubProjection` | `QueueGithubProjectionInput { db; repositoryId; planId; planRevision; operation: project-item-upsert; payload: { owner; projectNumber; readiness; currentGate; headSha }; now? } -> outboxId` | PLAN-L6-85/PLAN-L7-471。post: payloadを閉じた5項目・固定順のcanonical JSONへ正規化し、SHA-256 `payload_digest`と同一rowへ保存する。未知field、空文字、不正Project番号はwrite前に拒否する。同一repository/PLAN/revision/operationは冪等upsertし、applied状態をpendingへ後退させない。invariant: secret、credential、provider transcript、episode IDの推測値を保存しない。 |
