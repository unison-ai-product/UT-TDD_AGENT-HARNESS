...
| `analyzePlanGovernance.routeModeKindLayer` | `PlanFrontmatter { plan_id; status; route_mode?; kind; layer? } -> PlanGovernanceViolation[]` | PLAN-L6-38。`routeModeKind` の layer 拡張 (設計判断(b): 全 mode 横展開)。post: `(route_mode, kind)` が L4 §3.1 の layer band 外なら `route_mode_kind_layer_mismatch` で fail-close。escape は draft-debt 台帳 entry の `promote_by` 有効期限内 + justification 記載時のみ免除し、期限超過は draft のままでも fail-close (internal-processing Appendix C.4)。 |
...
| `loadSpecIrSources` | `LoadSpecIrSourcesInput { repo_root; roots?: RepoRelativePath[]; include?: SpecIrSourceKind[] } -> SpecIrSourceBundle { sources[]; findings[] }` | PLAN-L6-39。pre: `roots` は repo-relative で、既定は docs / PLAN / test-design / schedule / activation profile の canonical root。post: source path、section anchor、frontmatter summary、content_hash、layer/sub_doc 候補を返す。source docs は rewrite しない。invariant: raw provider transcript、secret-like payload、PII-like payload は projection input へ載せず finding 化する。missing root は silent success ではなく warn finding。 |
| `parseSpecDefs` | `ParseSpecDefsInput { bundle: SpecIrSourceBundle } -> SpecDefDraftResult { defs[]; findings[] }` | PLAN-L6-39。post: L4 `SpecDef` を L5 `spec_defs` row draft へ変換し、`spec_id` / `source_path` / `section_anchor` / `layer` / `sub_doc` / `status` / `owner` / `source_hash` を deterministic に生成する。invariant: DB は authoring source ではないため、未知 layer/sub_doc、重複 ID、空 definition は finding/fail-close 候補にし、補完した仕様を創作しない。 |
| `parseSpecRelations` | `ParseSpecRelationsInput { bundle; defs } -> SpecRelationDraftResult { relations[]; findings[] }` | PLAN-L6-39。post: relation_kind は `defines` / `requires` / `verifies` / `pairs` / `derives` / `supersedes` の allowlist に限定し、L5 `spec_relations` row draft を返す。orphan relation、self-loop、未知 relation_kind は finding。`spec_relations` は仕様 IR 内の依存であり、横断 impact graph (`dependency_edges`) と二重正本化しない。 |
| `parseScheduleEntries` | `ParseScheduleEntriesInput { bundle } -> ScheduleEntryDraftResult { entries[]; findings[] }` | PLAN-L6-39。post: 工程管理表から `schedule_entries` row draft (`schedule_id`, `plan_id`, `phase`, `planned_start`, `planned_end`, `state`, `owner`, `evidence_path`) を返す。date/state 欠落、過去 due の未完了、plan_id 未解決は finding。invariant: 工程表は現在地把握の入力であり、PLAN status を直接 mutate しない。 |
| `parseActivationEntries` | `ParseActivationEntriesInput { bundle } -> ActivationEntryDraftResult { entries[]; findings[] }` | PLAN-L6-39。post: activation profile から `activation_entries` row draft (`activation_id`, `profile_id`, `drive`, `mode`, `enabled`, `reason`, `evidence_path`) を返す。enabled=true かつ reason 空、未知 drive/mode、profile 欠落は finding/fail-close 候補。invariant: 駆動モデル選択の厳格化は profile + routeFiling の組み合わせで行い、暗黙 default で有効化しない。 |
| `projectSpecIr` | `ProjectSpecIrInput { defs; relations; schedules; activations; candidates?; db } -> ProjectSpecIrResult { rows_by_table; findings[] }` | PLAN-L6-39。post: `spec_defs` / `spec_relations` / `schedule_entries` / `activation_entries` / `detector_route_candidates` を deterministic upsert し、同一入力の rebuild は row counts と IDs が安定する。invariant: projection は rebuildable であり authoring source ではない。source docs / PLAN / test-design / schedule / activation profile は書き換えない。 |
| `analyzeSpecIrIntegrity` | `AnalyzeSpecIrIntegrityInput { defs; relations; schedules; activations } -> SpecIrIntegrityResult { findings[]; quality_signals[] }` | PLAN-L6-39。post: orphan relation、未知 layer/sub_doc、activation reason 欠落、secret-like evidence/value、schedule/PLAN 不整合、raw markdown body 永続化を検出し、finding/quality_signal へ変換する。invariant: parse 失敗や欠損を silent skip しない。fail-close 対象は doctor/detector 層で route_signal へ昇格できる形に保つ。 |
| `deriveDetectorRouteCandidates` | `DeriveDetectorRouteCandidatesInput { findings; quality_signals; spec_ir; filing_target_ssot } -> DetectorRouteCandidateResult { candidates[]; findings[] }` | PLAN-L6-39。post: finding/spec/schedule/activation を join し、L5 `detector_route_candidates` row draft (`candidate_id`, `source_finding_id`, `route_signal`, `target_layer`, `target_sub_doc`, `confidence`, `evidence_path`) を返す。invariant: FilingTarget を創作せず、target snapshot は L4 function §3.2.1 / `routeFiling` SSoT から得る。SSoT 不在、route_signal unknown、target mismatch は non-ready finding。 |
