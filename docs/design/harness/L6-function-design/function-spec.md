...
| `preflightProviderExecution` | `ProviderExecutionRequest { invocation_id; provider; command_plan; deadline_at } -> ProviderExecutionPreflight { ok; execution_kind; capabilities; findings[] }` | L4 external-if §3.1。required capability集合はcaller入力にせずHARNESS policyの固定正本とする。起動方式固有の詳細を core へ漏らさず、`deadline`、stdin、terminal observation、process-tree custody/reap の全 capability を起動前に照合する。不足・unknown・追加の未登録capabilityはfail-closeし、別方式へsilent fallbackしない。U-ADAPTER-010/011。 |
| `executeProviderWithReceipt` | `(request: ProviderExecutionRequest, port: ProviderExecutionPort) -> Promise<ProviderTerminalReceipt>` | preflight Green の invocation だけを開始する。success / provider non-zero / timeout / cancel / adapter error の全 terminal path で、同じ `invocation_id` の terminal receipt を exactly once 返す。receipt は `terminal_kind`、`provider_exit_code?`、`started_at`、`completed_at`、`deadline_at`、`cleanup` を含む。U-ADAPTER-012/013。 |
| `finalizeProviderExecution` | `(receipt: ProviderTerminalReceipt) -> ProviderExecutionOutcome` | `cleanup.orphan_count === 0` かつ cleanup 状態が verified の場合だけ terminal success を許す。親 process の終了のみ、receipt 欠落、重複 receipt、未知 cleanup は fail-closeする。timeout/cancel 後も process tree 回収を完了してから outcome を確定する。U-ADAPTER-014/015。 |
