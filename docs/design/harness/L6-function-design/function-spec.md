...
| `readGithubEvidence` / `deriveStoredForwardReadiness` | `(db,repoRoot,repositoryId?) -> ForwardEvidence[] / ForwardReadinessRow[]` | Project/PR同期ではrepository IDを必須入力としてbindingとProject itemを同一repositoryへ限定する。別repositoryの同一PLAN/revision/check/PRをreadinessへ混入させない。repository ID省略は全体projection rebuild専用であり、外部同期CLIは必ず明示する。 |
