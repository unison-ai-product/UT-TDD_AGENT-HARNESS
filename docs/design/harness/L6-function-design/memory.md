...
> **L6 contract marker**: `writeMemoryEntry`, `parseMemoryFile`, `loadMemoryEntries`, `selectMemoryEntries`, `renderMemorySurface` は unit-test 粒度の contracts とする。DbC pre/post/invariant は §2-§3、L7 oracle family は U-MEMORY-001..005。
...
| `renderMemorySurface` | renderMemorySurface(entries: MemoryEntry[]) => string | 任意の entry 配列 | SessionStart / recall 用の人間可読 text | 空 entry は空文字列。長文 body は短縮 | U-MEMORY-005 |
| `evaluateMemoryPromotion` | `(events: SessionEvent[]) => { should_nudge: boolean; reason: string }` | 1 session分のsanitize済 event列 | commitまたはplan_switchあり、かつmemory write成功eventなしの時だけnudge候補を返す | 本文・git差分を読まない純関数。nudgeはmemory作成を強制しない | U-MEMORY-006 |
...
| 5 | entries 空 | `renderMemorySurface` が空文字列 | U-MEMORY-005 |
| 6 | commit/plan_switchあり・memory writeなし | `evaluateMemoryPromotion` がnudge候補 | U-MEMORY-006 |
| 7 | memory writeあり、または状態遷移なし | nudgeしない | U-MEMORY-006 |
