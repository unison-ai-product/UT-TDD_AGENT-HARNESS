...
- **machine-surface language**: machine-facing CLI/doctor/lint/gate/status messages may include Japanese explanation, but their decision token must be stable ASCII English (`OK`, `violation`, `warning`, `skipped`, `note`, `error`, `ready`, `not ready`). Japanese-only decision words in machine message lines are `machine-surface-language` violations.
