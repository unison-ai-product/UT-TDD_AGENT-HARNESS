...
| `analyzeL14CloseAudit` | analyzeL14CloseAudit(docs: L14CloseAuditDoc[], repoRoot?: string) => L14CloseAuditResult | `.ut-tdd/audit/A-143-l14-close-system-foundation-audit.md` is supplied as the L14 close audit source, and evidence paths are repo-relative. | returns parsed audit rows and violations for missing expected items, malformed rows, unknown status values, absent evidence paths, missing item-specific evidence, missing item-specific boundary markers, partial rows without gaps, and open boundary rows without next actions. Item-specific required evidence covers workflow definition, system foundation, Claude/Codex parity, clean distribution package, version-up nonbreaking, brownfield onboarding, cross-project workflow, L1/L2 mock roundtrip, L10 UX close, drive-model bookbinding, and green evidence integrity; `.claude/`, `.codex/`, `LICENSE`, adapter templates, CI workflow, and `.ut-tdd/evidence` paths are valid evidence roots when explicitly cited. | L14 close cannot be claimed by prose or local green commands alone; workflow definition, L10-L14 boundaries, distribution, brownfield onboarding, version-up, cross-project tests, L1/L2 mock feedback, release publication boundaries, parked version-up work, and green evidence correction evidence must remain explicit machine-checked rows. | U-L14CLOSE-001..008 |
