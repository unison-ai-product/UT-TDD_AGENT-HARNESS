...
pair_artifact: docs/test-design/harness/L10-ux-validation-test-design.md  # UXV 検証正本 = L10 UX 検証テスト設計 (旧 self-pair IMP-039/058 は PO 裁定なき作り込みとして撤去、PLAN-RECOVERY-09)
...
工程。L2 の右腕ペアの検証正本は [L10 UX 検証テスト設計](../../test-design/harness/L10-ux-validation-test-design.md)
(旧「mock 自体が pair を担い独立 doc を作らない」self-pair 規約は PO 裁定なき作り込みとして PLAN-RECOVERY-09 で撤去)。
...
- 上流: [L2 wireframe](../L2-screen/wireframe.md) + [L4 ui-standard](../L4-basic-design/ui-standard.md) (FE 設計標準) + L7 src/web 実装。
- 下流: G10 UX 承認 → L11 総合レビュー+UAT。

## §6 G10-WORKFLOW (移設)

G10-WORKFLOW の検証戦略・UXV テストケース定義は右腕層の正本
[L10 UX 検証テスト設計](../../test-design/harness/L10-ux-validation-test-design.md) へ移設した
(PLAN-RECOVERY-09。設計 doc に検証定義を同居させない — 両肺分離)。`g10-ux-workflow` doctor check の
parse 対象も同 doc である。
