...
| Phase | 状態 | 備考 |
|-------|------|------|
| Phase 1 (L0-L3) | **✅ freeze 済 (2026-06-04, G0.5/G1/G3 PO サインオフ, A-100)** | 改善/検証サイクル 4 巡完走 → L0-L3 設計 doc + L1/L3 PLAN を `status: confirmed` 化。残 PO 判断要 0。`roadmap.md` は living のため draft 維持 (本書は frozen 対象外) |
| Phase 2 (L4-L6) | **仕切り直し (未着手扱い)** | RECOVERY-02 (正規式モデル) で L4-L6 は仕切り直し。旧 G4/G5 COND PASS は park (gate-design §2 で要再評価へ rollback)。L4 から Forward 実開発する。L4/L5 テスト設計 doc も要起票 |
| GATE-A | 未 | — |
...
> **Phase 1 (L0-L3) は freeze 済** (2026-06-04)。次は **Phase 2 = L4-L6 の Forward 実開発** (改善/検証サイクルではなく設計の新規降下)。L4-L6 は正規式モデル (L4⇔L9 総合 / L5⇔L8 結合 / L6⇔L7 単体) で仕切り直す。freeze 後の L0-L3 規範変更は Reverse/Recovery を通す。
...
| 2026-06-04 | **Phase 1 (L0-L3) freeze** (PO「フィックス」= 改善/検証サイクル終了) | freeze (改善サイクルではなく確定) | **改善/検証サイクル 4 巡完走 (残 PO 判断要 0) を受け、PO が L0-L3 freeze (G0.5/G1/G3) を再確定サインオフ (A-100)**。RECOVERY-02 (正規式モデル) で fullback した L0-L3 を確定。実施: ① L1 設計 doc 5 + L3 設計 doc 4 (roadmap=living 除く) + L1/L3 PLAN 8 = **17 ファイルを `status: confirmed` 化** ② gate-design §2 台帳を再確定 (G0.5/G1/G3 = A-100 PASS、旧 A-41/A-60 は正規式前スコープ) ③ **L4-L6 (G4/G5) は仕切り直しのため park=要再評価へ rollback** (旧 A-67/A-91/A-70 は historical 保持) ④ §5 現在地を Phase1=freeze / Phase2=仕切り直し に更新。検証: vitest 177 pass / doctor exit 0 / pmo-sonnet 整合 + code-reviewer 前置。 | freeze アクション (新 IMP なし)。Phase 2 entry 条件 (= L3 要件定義 confirmed) を満たす。**次は Phase 2 = L4-L6 Forward 実開発** |
