...
# PLAN-REVERSE-24 (reverse): back-fill structured error-handling rule

## §0 Position

Records Reverse trace for IMP-095 so the implementation is not orphaned from governance design.
...
- Reverse trace is this PLAN plus the `requires` edge from PLAN-L7-25.

## §3 工程表

### Step 1: [並列] observed gap record

Record manual-review-only error handling as a coding-rule gap.
...
- [x] PLAN-L7-25 requires this Reverse PLAN.
