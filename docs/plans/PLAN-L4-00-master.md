...
| **screen** | ② 選択 | **skip/defer** | — (skip_sub_doc 記録) |

> **screen skip 理由** (PO 承認待ち): `harness core は CLI/library 優先。UI (PM/HM/GD 14 画面) は L1 screen-requirements で要求確定済だが、L2 モック検証 (G2) 前のため L4-screen は L2 完了後に別途起票する (defer)。BE/CLI/agent drive 主体の現段階では UI 設計確定は時期尚早`。
