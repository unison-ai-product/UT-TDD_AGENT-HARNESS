...
PLAN-L6-11 (vmodel-pair-freeze.md §7) の機能設計を実装する。`src/vmodel/lint.ts` に層群 freeze 集計 (loadPairDocs の status 拡張 + analyzeVerificationGroups + verificationGroupMessages) を追加し、doctor に `checkVerificationGroups` を note レベルで配線。③ ペア = L7-unit §1.14 U-VTRIG。
...
### Step 2: [直列] doctor checkVerificationGroups 配線 (note)
- 直列理由 = **downstream_dependency**。`src/doctor/index.ts` に note レベル surface (ok 非連動)。
...
- **src/doctor/index.ts** (情報源: 既存 checkPairFreeze 配線): checkVerificationGroups note。
