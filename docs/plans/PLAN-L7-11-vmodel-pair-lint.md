...
PLAN-L6-10 (vmodel-pair-freeze.md) の機能設計を実装する。`src/vmodel/lint.ts` stub を **設計層 pair freeze 検査** (`loadPairDocs` / `analyzePairFreeze` / `pairFreezeMessages` / `lintVmodel` 本実装) に置換し、`runDoctor` に `checkPairFreeze` を **warn-first** で配線。実 repo に当てて検出される孤児 (L3-functional/README 等の pair 欠落) は本 PLAN 内で doc 改善 (PO「チェック機構も改修」授権)。③ ペア = L7-unit §1.13 U-VPAIR。
...
### Step 2: [直列] doctor checkPairFreeze 配線 (warn-first)
- 直列理由 = **downstream_dependency** (Step 1 の関数に依存)。`src/doctor/index.ts` に `checkPairFreeze` を warn-first で追加 (I/O 失敗は skip=fail-safe)。既存 hard 条件 (backfill/scrum-reverse/propagation) の `runDoctor.ok` 連動は壊さない。
...
- **src/doctor/index.ts** (情報源: 既存 checkBackfill/checkScrumReverse 配線): `checkPairFreeze` warn-first。
