...
②実装↔④テストの双方向 trace edge (requirements §2.3 の edge 5-8 = 正常/異常/境界/エラーの docstring) 形式を確定候補として整理。G5 freeze 前に L8 詳細粒度と再照合する。
...
- G5 再 freeze は Step 9 review と L8 IT-* 詳細粒度の監査後に行う。
...
- [ ] edge case docstring 形式 (IMP-014、edge 5-8) が確定し G5 freeze 準備 (L8 詳細粒度監査後に再確認)
