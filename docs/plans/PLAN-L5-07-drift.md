...
- [ ] module-decomposition §4 に `asset-drift` rule を IMP-033 rule 型として結合記述 (新規 lint 手書きせず)
- [ ] auto-enroll (doc registry scan) + fail-close 経路 (doctor/gate) の結合
- [ ] 検査項目 4 種を inventory §1 / ADR-004 と trace
- [ ] DB 未充足検知 (placeholder_deps) 統合
- [ ] dependency-drift (ADR-002) と並置 (両方 IMP-033、二重定義なし)
- [ ] L8 IT-ASSET (drift) ペア + 未確定 placeholder_deps + 依存明示
- [ ] 判定関数 signature/regex を L6 carry (waiting_layer:L6) / engine 実装は L7
- [ ] §6 用語更新 / §7 機能要求更新
- [ ] self-review 通過
