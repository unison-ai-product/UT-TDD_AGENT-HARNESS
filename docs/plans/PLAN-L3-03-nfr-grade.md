...
> **TL レビュー反映 (2026-05-28、A-43 ledger)**: 全 15 項目を TL 視点で再仕分け。**PO 判断対象 = 1 件 (U-NFR3-1〜6 集約 = IPA Lv 全体方針 1 件)** のみ (IPA グレード Lv = 投資判断 / 受入基準のため PO 必須)。受入閾値 4 件 (U-NFR3-7〜10) は KPI integrated で自明 → 🆕。L4 / Phase B carry 4 件 (U-NFR3-11/12/14/15) は別 layer 専決 → ➡️。残 1 件 (U-NFR3-13) は agent-guard 整合で自明 → 🆕。

### 3.1 IPA グレード Lv 選定

| ID | ヒアリング項目 | 着地先 | status |
|----|--------------|--------|--------|
| **U-NFR3-1〜6 (集約)** | **IPA グレード Lv 全体方針** (可用性 / 性能・拡張性 / 運用・保守性 / 移行性 / セキュリティ / システム環境 の 6 大項目) | nfr-grade §1〜§6 | **❓ PO 判断必須** — Lv 選定 = 投資判断 / 受入基準のため PO 専決。TL 推奨案: **(a) 可用性 Lv2 / 性能・拡張性 Lv2 / 運用・保守性 Lv3 / 移行性 Lv2 / セキュリティ Lv3 / システム環境 Lv3** (推奨、社内内製ツール想定で過剰投資回避) / (b) 全項目 Lv3 (堅牢性重視) / (c) 全項目 Lv2 (MVP scope 重視) |

> **TL レビュー注記**: 各項目の TL 推奨根拠 (NFR-* との対応 + Lv 選定理由) は本 PLAN 起票時の §3.1 各行から (a) 案に集約。PO は (a)/(b)/(c) のいずれかを選択 (または個別 Lv override) してください。
...
| U-NFR3-15 | PII redaction NFR 化 | nfr-grade §6 carry / NFR 追加判断 | 🆕 **NFR-17 新規追加 = 「telemetry PII redaction」 (Phase B carry)**。理由: GDPR / 社内 PII 方針整合、Phase B telemetry 必須要件 |
