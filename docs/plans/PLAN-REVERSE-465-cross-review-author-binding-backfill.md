...
pair_artifact: docs/plans/PLAN-L6-13-cross-review-enforcement.md
agent_slots:
  - role: tl
...
PLAN-L7-465 は cross-review 契約 (PLAN-L6-13 / IMP-076) に **新しい判定面** を足す:
...
1. provider 族の導出規則 (commit trailer + wrapper session log) を L6 契約へ記述。
...
- AC-1: L6-13 に provider 族の導出規則と `unverified` の扱いが記載され、実装挙動と
  一致することを照合済み。
