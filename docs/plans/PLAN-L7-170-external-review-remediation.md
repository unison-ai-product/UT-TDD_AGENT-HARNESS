  ...
  - reviewer: codex
    ...
    green_commands:
      ...
      - kind: unit_test
        ...
        output_digest: "sha256:421a6cb444d84effc26ede45f76f6410d9edf26bd4af9c0e9bc70c2494dd9a50"
      - kind: unit_test
      ...
      - kind: typecheck
        ...
        output_digest: "sha256:e62ec6501d071c4e14fa16f758a775c48afe39865a66ef3dd7b87b3ae65ab9a0"
      - kind: typecheck
        command: "bun run typecheck"
        runner: bun
        scope: full
...
任意の将来改善として「signal 終了を JSON の `exit_code` に含める」のみ (本 scope 外)。worker=Claude
Opus 4.8、reviewer=Codex gpt-5.5 (model family 相異 → cross_agent 成立)。
...
- [x] staged diff が意図ファイル (4 修正 + 8 evidence digest PLAN + 本 PLAN) のみであることを確認

> 着地: 本 remediation は explicit stage (意図ファイルのみ) で 1 コミットにまとめ、commit message に
> "remediation + evidence restamp" を明示する (delivery 手順、PO の commit 指示時に実行)。
