...
`CODEX_REVIEW_TARGET_SESSION` が未設定または空文字の場合は、producer は inbox、hook surface、
backlog のいずれも書き込まず、`codex_review_target_session_unavailable` を返して fail-close
する。既定値、current process の PID、または任意の session の推測で補完してはならない。
...
観測を receipt/配送成功として扱わない。valid entry と invalid entry が混在する場合は、valid な
pending entry を surface しつつ、`invalidCount` に同じ inbox scan で検出した invalid entry 数を必ず
含める。これにより invalid entry を削除・再配送・receipt 化せずに consumer から可視化する。
...
から除外するが、invalid entry が一件以上あり valid entry が一件もない場合は、次の typed surface
を返し exit code `2` とする。
...
invalid entry の元 bytes は削除や別宛先への再配送で隠蔽せず、consumer が観測可能な typed state
として保持する。invalid-only の場合は上記 `status:"invalid"` shape と `invalidCount` を返し、valid+invalid
混在時は `status:"pending"` shape の `invalidCount` で可視化する。hook の stderr/stdout を review verdict、
receipt、merge authority の入力にしてはならない。
...
- claim marker は `claimedAt`、`leaseExpiresAt`、`targetSessionId`、entry identity を必須とする。
  `leaseExpiresAt` は `claimedAt` から **15 分以内**（`leaseExpiresAt <= claimedAt + 15 minutes`）に
  制限し、consumer は処理中に同じ project namespace 内で lease heartbeat を更新できる。terminal marker が
  無く lease が期限切れになった claim は、次の SessionStart/Stop で同一bytesを inbox へ atomic
  restore して再取得可能にする。期限前の active claim、foreign project の claim、terminal marker
  と identity が一致する claim は回収・削除しない。crash/kill 後もこの回収規則を通り、claim が永久に
  backlog を隠す状態を許さない。
