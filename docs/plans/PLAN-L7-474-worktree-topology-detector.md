...
agent_slots:
  ...
  - role: qa
    slot_label: "QA - U-WTTOPO-001〜011 を実装し fail-safe 分類 (dirty 最優先 / 観測不能除外) を検証"
  - role: tl
...
| `U-WTTOPO-011` | link 断で観測不能な worktree (collector が `dirty=false` / `mergedIntoMain=false` の既定値へ倒したもの) | `retirable` へ入れない。counts には残すが廃棄候補として提示しない (未コミット作業の誤廃棄を防ぐ fail-safe) |
