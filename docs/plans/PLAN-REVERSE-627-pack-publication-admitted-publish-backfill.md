...
agent_slots:
  - role: tl
    slot_label: Codex Sol - L6 before-state CAS / approval 境界と L7 admitted publish
      の同値性を非著者検証
  - role: qa
    slot_label: Terra - 36 guard、indeterminate、write-zero、replay、consume 後
      fail-close、token 境界の逆向き検証
generates:
...
admission_receipt:
  schema_version: v2
  receipt_id: certificate:6e600c144f17d750eaf3b52c06b2f0e0
  command_id: plan-draft:issue-627:admitted-publish-reverse:1
  admitted_at: 2026-09-17T04:42:42.760Z
  source_digest: sha256:f74841d08a3dda0f8dcc80142717a06d4c27e61bb5f64b97410deac235f6eec1
  decision_digest: sha256:db3a82cfa05adf299f40aeda7165958046ab86a2a2ea0048821e6f0c76684a0a
  receipt_digest: sha256:ea4444ede4c3ed304fe91e521cb07df84d24f7abc2d5bdceff68c77e5a168985
  binding:
    ...
    revision: 1
    content_digest: sha256:f74841d08a3dda0f8dcc80142717a06d4c27e61bb5f64b97410deac235f6eec1
  route:
    signal: reverse
    mode: reverse
  issue:
  ...
  origin:
    ...
    revision: 1
    digest: sha256:acb9b3d6e43d46ad6958723969e403912f9e0e0e66b5e016120628a0f0ed5f69
  transition:
  ...
  reentry:
    ...
    target_revision: 1
    phase: forward_merge
  escape_reason: "Issue #627 第三 slice の Reverse 対: PLAN-L7-627 rev 1 (36
    guard、candidate 59) が新設する seal 主体検証、consume 後 fail-close、token 権限一軸
    deny、pre-write drift、最小 publish receipt を上位 PLAN-L7-565 §1.1/§3/§5 と
    PLAN-L6-63 へ backfill する。admission.branch の work/reverse- prefix は
    plan-admission-branch-forbidden policy を満たす名目 branch であり、実 PR branch は
    work/add-feature-issue627-admitted-publish (PLAN-REVERSE-625 rev 3 /
    PLAN-REVERSE-626 の precedent)。"
---
...
consume と CAS token mint を #627 に残した。しかし上位契約には次が無い: (1) publish が受け取る admitted
record の seal 主体をどう検証するか (署名の無い record を自己整合のまま偽造・持ち込みできる)、(2) record
の strict schema と record digest / bundle digest / intent identity / approval binding の再導出一致、(3)
...
一軸 deny 語彙、(6) CAS 直前の main / PR head drift の typed deny、(7) 最小 publish receipt の member 集合。
これらが無いと #627 の main CAS が admitted record 以外の入力で発火し得る。

## R1: 上位不変条件

- publish の入力は #626 admitted record ただ 1 つであり、その seal 主体は admission record store の
  read-only resolve (identifiers → 唯一の record) と canonical bytes の一致で検証する。store 不在 / 不一致は
  deny、store 観測不能は indeterminate であり、署名・token・自己申告を seal の証明にしない。
- admitted record は identifiers / sealed / status の strict 3 群だけを持ち、record digest は canonical
  bytes から publish 側が導出する。observation bundle digest、publication intent identity、mutation approval
  binding は #626 と同じ導出関数で sealed 値から再導出し、記載値と byte 一致しなければ deny する。
...
- CAS authority token は全 approval consume 後にだけ fresh mint し、installation ID が record と一致し
  ...
  過剰、Contents write の欠落は不足として各々 deny し、token bytes を journal / receipt / result / error /
  argv に出さず、CAS 試行の成否に関わらず dispose する。
...
publish の store resolve、strict schema、再導出一致、configuration 一致、approval consume / fail-close、
token 境界、pre-write drift が同じ sealed operation の precondition であることを確認する。全 36 guard の
deny と port unavailable の indeterminate は、上位 L6 が要求する副作用前の write-zero (CAS 到達前) または
単一試行後の write-zero (CAS 到達後) へ写像される。完全一致 replay だけは同一 receipt bytes を再構成し、1
軸 drift は再利用ではなく新規 deny となる。

## R3-R4: 再合流

R3 で実装 PR の 59 candidate、対象テスト、exact-head CI、非著者 review を照合する。R4 で seal 主体検証、
strict schema / 再導出一致、configuration 一致、consume 後 fail-close、token 境界、pre-write drift、最小
receipt schema の不変条件を上位 staged-release contract (PLAN-L7-565 §1.1/§3/§5、PLAN-L6-63) へ backfill
する。Release / tag / asset / pointer、production ports、CLI、既存 adapter の再構成はこの Reverse の責務外
とする。
