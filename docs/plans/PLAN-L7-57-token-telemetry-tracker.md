...
- **OpenAI/Codex 単価表**: Codex の $ cost enrichment は OpenAI 公式 pricing を source 後に追加 (pmo-tech 調査)。
- **scan の CLI 配線**: `ut-tdd telemetry scan` 等で loadRuntimeSessionUsage→projectTokenUsage を定期実行する
  CLI/hook 配線は別 PLAN (env 固有の session-dir 解決を含む)。本 PLAN は parse/cost/projection の核を着地。
