...
> **L2 skip 質問への回答 (2026-06-02)**: L2 スキップラインの正本は **concept §3.7 (UI 有無で引く)**。UI 完全不在 (be-only/db無UI) のみ全 skip、UI 有れば画面要求3は drive 非依存で必須 (wireframe のみ drive 省略可)。機械検証 §G.3 が §3.7 に未追随 = V17。サービスによる選択は **L2 で既に発生** (UI 有無)、だが宣言的でなく archetype 未分化 = V19 (PO 設計判断)。
...
>
> **正本化(本PLAN R2-R4)/別PLAN carry**: V8-V11 (reverse ③ 機構・再入gate・generates契約・③所有hub) / V13 (R4→L3) / V15/V16 (要件 §G.6・§3.5 改訂 / hub 起票 / lint 実装)。
