...
- **正本化 carry note 追記**: L10-L14 は spike 拡張、正本 concept §3.1.5 は L8/L9 のみ である旨を表下に明記。

### B. 正本化 carry (REVERSE-01 R2-R4 で実施、正本先行)
- **concept §3.1.5 を「L8-L14」へ改訂**: L10-L14 差し戻し (特に L13→Incident 橋渡し・L11 の L1/L3 分岐) を正本に昇格。現状 spike 暫定。
...
- **ただし critic が実ギャップ 4 件を発見 (工程↔git の"接続点"未記述、製品が利用者チームに課す配線)**: 【high】**3 段階 freeze (Pair/Red/trace) ↔ PR 作成・merge timing の対応が無い** (どの freeze で PR-ready / merge-ready か) / mode 連鎖時の branch 切替手順 (poc/*→Reverse→feature/*) / L7 7step ↔ commit 粒度 / Reverse・Recovery の branch 処置。→ **REVERSE-01 正本化 carry** (利用者チーム向け工程↔git 接続の明文化、製品仕様であって自リポ開発手順ではない)。
