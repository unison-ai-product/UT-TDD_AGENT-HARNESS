...
1. **plan_id の mode legibility 範囲** — **①明文化を本 session 実施済 (§1.10.A)**: 「横断駆動 3種(Discovery/Reverse/Recovery)=mode token で ID-legible / layer-bound mode(Refactor/Retrofit/Incident-troubleshoot部/Add-feature/Research)=layer token + kind 識別」を意図的設計として確定記載。Scrum=kind=poc は Discovery と同名前空間で mode 入口識別 (scrum_type enum が将来 distinguisher 候補)。**残る PO 判断 = ②全 mode を ID-legible 化する token 拡張を行うか** (現状は①で確定、②は任意の将来拡張)。
