...
### A. 本 session 修正済 (安全な doc整合 / 副作用ゼロの fail-close)
- **incident.md 誤記**: Reverse fullback `F0-F4`(HELIX 旧) → `R0-R4` (UT-TDD 正、`confirmed_reverse_type=fullback`)。
- **scrum.md 発火 signal**: `requirement_continuous_refinement` 追記 (README §4 と整合)。Scrum↔Discovery は frontmatter 非区別 = mode(入口)識別 を明記 (既存設計の補強)。
- **Add-feature 起票 fail-close**: `frontmatter.ts` superRefine に `add-design→L3-L6 / add-impl→L7` ガード追加 (§1.3 主な layer を強制、起票時の impl 混在を防止、既存 PLAN 違反なし=副作用ゼロ)。
- **docs/research/ 実体化**: `.gitkeep` 先行作成 + repository-structure ツリー登録 (research.md 参照解決、scaffold-dirs 原則)。

### B. 正本改訂を伴う (R4→L3 routing で requirements/concept へ。modes は正本確定後に追随)
| 領域 | 不備 | 修正先 |
|------|------|--------|
| 発火点 (signal) | Discovery が prose のみ (named signal なし) / Research が signal 全欠落 / `interrupt` 横断 signal の受け皿 mode 未定義 (§2.6.5 にあるが §2.6.1/§7.8.1 routing 表に行なし) | concept §2.6.1 + requirements §7.8.1 に named signal + 判定木追加 |
| 収束点 (exit機械検証) | Scrum の「Reverse fullback 昇華完了」/ Incident の「恒久対策昇華 PLAN 起票」/ Recovery の resume_layer / Research の ADR→Forward 接続 が機械検証フィールド/ルールなし | requirements §3.x lint 強化 (フィールド最小追加) |
| 起票 (scrum_type) | requirements 行838 が `kind=poc PLAN は scrum_type 必須` と要求するが `frontmatter.ts` に未実装 | schema 実装 + §3.5 superRefine |
| 起票 (Research role) | README owner=tl だが requirements §1.8 必須role表に `kind=research→tl` 行なし | §1.8 行追加 + validator |
| 起票 (Incident 2-PLAN) | 1 incident → troubleshoot(token=L7) + recovery(token=RECOVERY) 分割運用が要件/台帳/mode に未明示 (dependencies.blocks で宣言) | incident.md + 台帳明記 |

### C. PO 設計判断 (escalation、unilateral 不可)
1. **plan_id の mode legibility 範囲**: 現状 ID で読めるのは Discovery/Reverse/Recovery (横断駆動 3種) のみ。**Refactor/Retrofit/Incident(troubleshoot部)/Add-feature/Research は layer token (L0-L14) になり ID から mode 判別不可** ([[feedback_drive_model_first_class_in_plan_id]] が「駆動モデルは ID で読めるべき」とした原則の射程)。①layer-bound mode は kind で識別 (現状) を正式設計として §1.10.A に明文化 / ②token 体系を拡張し全 mode を ID-legible 化、の二択。**Scrum は kind=poc で Discovery と完全同一** (V7 で drive 区別も消滅)、これを「mode 入口のみ識別」で許容するか別 distinguisher を持つか含む。
2. **forward_routing enum 拡張**: `L7`/`L8-L11`(fullback) を VALID_FORWARD_ROUTING に追加するか `gap-only` 代替か (V1 既出、R4 decision)。
3. **`runaway` signal**: `agent_runaway` の alias か独立 signal か (concept §2.6.1/requirements §7.8.1 に runaway、README §4 に欠落)。
