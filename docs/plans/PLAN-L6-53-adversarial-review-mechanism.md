...
**用語注記 (設計クロスチェック 2026-07-08)**: `src/team/advisor-policy.ts` の
`AdvisorConsultationMode = "adversarial"` (単一モデルへの frontier 相談検証) は本 PLAN
とは別の狭い機構であり、本 PLAN (ID 単位リスク加重抽出・攻撃者/防御者別プロバイダ分離・
ブラインドパケット・PASS-WEAK・冪等抜き打ち指名の一式) と混同しないこと。実装時に
`adversarial` という語を再利用する場合は既存モードと衝突しない命名にする。
