...
review_evidence:
  - reviewer: claude-fable-5
    ...
    verdict: approve
    scope: "frontier advisor による別 model family の最終設計判断。具体的な merge blocker なし (Go)。PR head の CI green、merge 直前の origin/main 再確認、24h TTL telemetry の意図維持、Claude 側での通常 merge を受入条件とした。"
    green_commands:
      - kind: unit_test
