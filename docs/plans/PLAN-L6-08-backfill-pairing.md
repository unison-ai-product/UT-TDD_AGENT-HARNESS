...
agent_slots:
  - role: tl
    slot_label: "TL — KIND_BACKFILL 要否マトリクスの妥当性 (どの駆動が設計へ戻す義務を負うか) / Reverse pairing の向き (reverse.requires→impl) / glossary 照合の表記ゆれ吸収 (normalizeTerm) / warn-first→fail-close 段階化のレビュー (claude-only は code-reviewer 代替)"
generates:
...
- **surface**: `ut-tdd doctor` が warn-first (reverseOrphans/glossaryGaps/conditionalPending)。lint engine 実装時に fail-close 化。
...
| warn-first 方針 | PO 提案 (handover discipline と同型、段階強制) |
