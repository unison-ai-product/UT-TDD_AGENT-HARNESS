...
### §4.3 doctor 検証 (surface, 暫定 warn-first)
...
- **Step 2 spike**: 情報源 = src/schema/team.ts (zod schema の既存実装パターン) + src/doctor/index.ts (check 追加パターン、warn-first 根拠コメント規約 IMP-093)。使い捨て前提のため最小スライス (roadmap 1 件 parse + span 実在 + gate 進捗 surface)。
...
**spike 実装 (Step 2)**: `src/schema/roadmap.ts` (zod schema + 構造整合 `validateRoadmapStructure`) + `src/lint/roadmap-registry.ts` (extract/parse/span 実在/gate 進捗/load) + doctor `checkRoadmap` (warn-first) + `tests/roadmap.test.ts` (U-ROADMAP-001〜010, 10 green)。typecheck/lint clean・vitest 342・doctor exit 0。
...
- **詰まり/finding**:
  ...
  3. **孤児 span の hard 化**: 現状 warn-first。本実装時に「宣言済 span が未起票」を fail-close するか PO 判断 (IMP-088 impl-plan-trace と連動候補)。
