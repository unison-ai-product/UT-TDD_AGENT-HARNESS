...
| **IMP-047** | 2026-06-04 | PO 指摘 (handover 忘れ) | PLAN完了/駆動サイクルの節目で handover+log を生成する規律が **agent 記憶依存で未強制** (§6.8.5 で定義済だが強制機構なし)。今回 PM が手動生成を忘れ PO 指摘 = workflow ギャップ。`handoverStale` lint / Stop-hook / `doctor checkHandover` surface / pre-push で「PLAN完了なのに handover 追記なし→warn/block」を強制化 | lint / policy | observed | §6.8.5 / src/handover handoverStale / doctor checkHandover / roadmap §1 |
| **IMP-048** | 2026-06-04 | ut-tdd handover dogfood | `ut-tdd handover` の §1-§2 auto-prefill が session-log digest の当日全 PLAN を含み、重複・`unknown` エントリのノイズが出る (digest が active plan/当該セッションへ未収束)。digest を active plan or セッション境界で絞る + dedup | doc / lint | observed | src/handover / src/runtime/session-log / ut-tdd handover |
