...
UT-TDD harness は HELIX 由来の **runtime 内部資産** = subagent (`.claude/agents/*.md` 19 個) / skill (`vendor/helix-source/skills/**` 107 個) / command を持つ。これらを「UT-TDD 用に作り替える」必要がある (BR-22、Recovery PLAN-RECOVERY-01 で前提抜けを是正)。
...
4. **drift lint (FR-L1-49)** が境界の番人: 正本 `.md` に HELIX 前提 (絶対パス `~/ai-dev-kit-vscode/` / `helix codex` 直叩き / 未 curate skill / model family 不整合) が残らないことを **fail-close** で検証する。IMP-033 cross-check rule engine の rule 型インスタンスとして実装 (新規 lint を手書きしない)。
...
- FR-L1-49 drift lint は「正本 .md に HELIX 前提が残っていないか fail-close」する設計であり、**markdown 正本 + TS 検証**と最も整合 (TS 生成方式だと生成元/生成物の差分管理が追加で要る)。
...
- **FR-L1-49 / IMP-033**: drift lint の検査項目 (HELIX 絶対パス / `helix codex` 直叩き / docs/skills 空 / roster↔guard model family 整合) を rule engine の rule 型として L6-L7 で実装。
...
- **subagent 19 件の HELIX 前提除去**: 絶対パス・`helix codex` 直叩きを UT-TDD 化 (drift lint の fail-close 対象を 0 にする)。
