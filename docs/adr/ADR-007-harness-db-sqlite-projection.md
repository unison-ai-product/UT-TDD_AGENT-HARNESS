...
ADR-001 (TS/Bun 全面再実装) は永続化を **`.ut-tdd/` file-based state (YAML/JSON)** とし、SQLite は **「必要時 `better-sqlite3`」として採用を deferral** していた (HELIX 版 `helix.db` schema の流用却下は維持)。
...
- **runtime**: Bun では `bun:sqlite` を第一候補、Node 互換が要る adapter のみ `better-sqlite3`。HELIX 版 `helix.db` schema は流用しない (ADR-001 維持)。
...
- **HELIX `helix.db` schema 流用** (却下、ADR-001 から継続): bash 依存・個人パス・schema 不整合。UT-TDD 独自 projection schema を新設する。
