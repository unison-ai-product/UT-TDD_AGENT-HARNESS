...
- **移行時点の読み方 (ADR-009)**: 本改訂はNode/Rust移行後のtarget配置を定義する。F0/W1および各cutover gateがGreenになるまでは、実repoに存在する`bun.lock`とBun scriptsを**現行migration debt**として併記し、Node targetが着地済みとは扱わない。target未着地と現行Bun debtの差分はPLAN-L7-458でfail-close管理する。
...
├── package.json                  # 移行中はBun現行scripts + Node candidate、cutover後はnpm/compiled ESM
├── tsconfig.json                 # TypeScript strict
├── bun.lock                      # [現行migration debt] Bun lane compatibility lock。cutover完了時に削除
├── rust-toolchain.toml           # [target] Resource Kernel用Rust toolchainのexact pin
├── package-lock.json             # [target] Node lane dependency graph のreview済み正本
...
├── bundles/                      #   [target/生成・非正本] target別署名済platform bundle staging (Git追跡しない)
│
├── docs/
│   ├── governance/               # ★ 現行正本 (本書群)
...
| OS entrypoint | `scripts/` | **薄い wrapper のみ**。review済みreceiptが封印したNode executable + compiled ESMを呼ぶだけで、core logicや別runtimeへのfallbackを持たない |
...
- **tracked**: `src/` `tests/` `docs/` (archive 含む) `scripts/` `package.json` `tsconfig.json` `vitest.config.ts` `.gitattributes` `.editorconfig`。移行中は`bun.lock`も現行migration debtとしてtrackedに残す。target着地sliceから`native/resource-kernel/` `package-lock.json` `rust-toolchain.toml`を追加し、最終cutoverだけが`bun.lock`を削除する。/ **監査証跡** `.ut-tdd/audit/*.md` `.ut-tdd/audit/reports/*.md` `.ut-tdd/evidence/` `.ut-tdd/handover/provider/` / **参照資料** `docs/reference/` (PO 決定 2026-06-10 tracked 化 / 2026-06-25 docs/reference へ移設、A-128 F-1 / IMP-127)
...
- **root config の下限**（避けられない）: 移行中は`package.json` / `bun.lock` / `tsconfig.json` / `.editorconfig`、target着地後は`package.json` / `package-lock.json` / `tsconfig.json` / `rust-toolchain.toml` / `.editorconfig`。
- **lint + format = Biome 1 枚 (`biome.json`)**。**eslint + prettier を別々に足さない**（plugin/ignore で 4-6 枚に増えるのを防ぐ）。`npm run lint` / `npm run format`。
...
→ target cutover後のroot configは **`package.json` / `package-lock.json` / `tsconfig.json` / `rust-toolchain.toml` / `.editorconfig` / `biome.json` / `vitest.config.ts` の 7 枚で頭打ち**に保つ。移行中の`bun.lock`は8枚目の恒久例外ではなく、PLAN-L7-458で撤去する可視化済み負債である。`rust-toolchain.toml`はResource Kernelの再現可能ビルドに必要な例外であり、浮動`stable`や個人環境のtoolchainを正本にしない。
