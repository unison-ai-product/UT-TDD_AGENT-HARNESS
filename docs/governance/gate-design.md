...
| G4 | ⏸ **要再評価 (park、正規式モデルで仕切り直し)** | (旧) A-67 + A-91 = historical | **A-100 で park**。L4-L6 は正規式モデル (L4⇔L9 総合) で Phase 2 にて Forward 実開発・再 audit する。旧 COND PASS (内部資産再audit含む) は historical 記録として保持 |
