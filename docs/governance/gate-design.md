...
> 上表の G0.5-G14 は **① 必須スケルトン (Forward spine、V-model)**。これだけでは不完全で、各工程は **② ケースバイケース駆動モデル**を合成する (concept §2.5 9-mode / metamodel / PLAN-X-01 PoC)。**本来 Reverse / Discovery 等に流す**経路を本節で明示する。
...
> 本節の駆動モデル詳細は concept §2.5/§4 + 各 `helix-process/*-workflow.md` (移植元) を正本根拠とし、UT-TDD では `ut-tdd` CLI の mode routing として実装 (FR-08、L7)。**§4 自動追加型クロスチェックエンジンは Forward/駆動 双方の成果物に適用**される (frontmatter の kind/workflow_phase 駆動でルール enroll)。
...
| G4 | ✅ COND PASS (全体再audit) | A-67 (FR-L1-01〜45) + **A-91 (内部資産 FR-L1-46〜49 含む全体再audit、2026-06-01)** | 初回 A-67 は内部資産増分**前**のスコープ。A-90 で 2 Master 並列解消 (L4-10→L4-00 従属) + cross-sub-doc gap-fill (C-1 data.md roster/skill 非entity / C-2 C12 集約 / I-1 external-if fs境界 解消) → **A-91 で全体 G4 再audit = Critical 0、4軸 PASS、CONDITIONAL PASS**。残条件: child PLAN-L4-11/12/13 起票済 (DoD反映) / ST-ASSET-04 placeholder_deps は doctor fail-close 追跡。PO 正式サインオフは G6 後にまとめて可 |
...
> **採番注記 (A-85 是正)**: 本記述は当初 "FR-L1-46 として起票" と書いていたが、**FR-L1-46 は A-77 で内部資産 subagent roster に採番済** (FR-L1-46〜49 = 内部資産、Recovery PLAN-X-02)。cross-check engine を独立 FR 化する場合は FR-L1-49 まで採番済のため **その次番号以降の空き ID** を用いる (ID 衝突回避。bare な未登録 ID トークンは upstream-coverage/g3-trace の孤児検出に掛かるため本文に直書きしない)。
