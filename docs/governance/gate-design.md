...
> 上表の G0.5-G14 は **① 必須スケルトン (Forward spine、V-model)**。これだけでは不完全で、各工程は **② ケースバイケース駆動モデル**を合成する (concept §2.5 9-mode / metamodel / PLAN-001 PoC)。**本来 Reverse / Discovery 等に流す**経路を本節で明示する。
