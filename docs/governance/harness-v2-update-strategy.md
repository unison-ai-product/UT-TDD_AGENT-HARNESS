...
- **姉妹編**: 実装・設計の実質品質 (構造衛生/契約明確性/検証実質/設計現役性) の底上げは `harness-v2-quality-uplift-strategy.md` (A-182、2026-07-03) が正本。本 doc = 運用・持続性、姉妹編 = 成果物品質 — 重複しない。
...
| PLAN-L7-303-digest-commit-anchor (v2 新規) | digest の commit anchor 化 → 199 件是正 → 段階 hard 化 | なし (L7-300 の増分化と独立)。**状態注記 (PLAN-L7-459 M4)**: 本 PLAN は version-up 規約外で confirmed/landed 済 — 現況の正本は `version-up-route-debt-2026-07-10.md` (immutable legacy debt 固定)。Wave 2 未着手案件として扱わない |
