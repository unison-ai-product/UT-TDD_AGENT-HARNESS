...
| UTV4-AC-068 | FR-056 | 初期画面ルール freeze 前に画面プロトチケットを 2 名以上へ分担発行する / fixture が本番 schema と異なる形を持つ / 異常系必須セットを欠く画面を検証完了にする / screen finding を prose のみで記録する | 順に deny (FR-028) / deny / deny / deny (typed 必須) |
