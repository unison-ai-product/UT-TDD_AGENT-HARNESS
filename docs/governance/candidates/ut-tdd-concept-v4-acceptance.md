...
| UTV4-AC-068 | FR-056 | 深度 profile を risk 信号 record なしに deep へ設定する / light profile で必須最小 4 要素のいずれかを省く /  初期画面ルール freeze 前に画面プロトチケットを 2 名以上へ分担発行する / fixture が本番 schema と異なる形を持つ / 異常系必須セットを欠く画面を検証完了にする / screen finding を prose のみで記録する | 順に deny (根拠 record 必須) / deny / deny (FR-028) / deny / deny / deny (typed 必須) |
| UTV4-AC-070 | FR-056 | 意味ロジックの判定器出力を Gold として登録する / 予測値を確定値と同じ表示規約で描く / 仮説 record に棄却条件が無い | 順に deny (独立 Gold 必須) / deny (表示規約違反 finding) / deny |
| UTV4-AC-069 | FR-057 | fixture 契約・異常系セット・finding・checklist のいずれかを欠く screen id を製本点 (a) で画面仕様へ束ねる / ハーネス自身のスプレッドシート view や digest をプロト工程なしに L5 詳細設計へ進める | 前者 compile が backflow_required / 後者 deny |
| UTV4-AC-071 | FR-058 | AI lane または compiler が要求 record を frozen にする / 人間 decision record も S4 confirmed evidence も無い frozen 遷移 / provisional 要求を view で確定値と同じ表示にする / AI 間の解釈齟齬を一方の AI が確定して下流へ流す | 順に deny / deny / finding (表示規約違反) / deny (discrepancy record 必須) |
| UTV4-AC-072 | FR-059 | budget record の無い PoC チケットを発行する / 許容規模超過後に S4 decision 無しで PoC チケットを追加発行する / PoC 期の AGENT / CI 設定をそのまま production へ持ち込む | 順に deny / deny (fence) / deny (設計結論として起こす) |
