...
L0 は「リポジトリ初期化」ではなく **企画工程**。企画書 PLAN に背景・目的・スコープ・投資対効果・成功条件・KGI/KPI・想定リスクを必須化する。**G0.5 企画突合**で、現在作業している AI とは別系統の `frontier-reviewer` による adversarial check を 1 回必須とする (チーム: po+tl が企画を確定 → tl が L1 起票)。リポジトリ初期化・Branch Protection 等の基盤整備は Phase 0 (要件定義書 §10) として工程外で扱う。
...
R4 で Gap を整理した後、UT-TDD Agent Harness の Forward 経路に接続。R4 outcome に応じて L1 (要求定義) / L3 (要件定義 FR+AC) / L4 (基本設計) / L5 (詳細設計) のどこに合流するかを決める (L0-L14 体系。旧 L2/L3 = 全体設計/詳細設計 は L4/L5 に remap。要件定義書 §3.4)。
