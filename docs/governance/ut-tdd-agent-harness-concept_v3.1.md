...
| **back-fill pairing** | 駆動モデルが「設計ドキュメントまで戻す」完全性。bottom-up build した impl を上位設計/governance へ Reverse 合流させ、§6 用語更新を L0 §10 へ back-merge する。`src/lint/backfill-pairing.ts` が「Reverse 無き impl」「glossary 未 merge」を検知 (`ut-tdd doctor`、fail-close、導入層 L6、IMP-051)。検査の 3 出力 = `reverseOrphans` / `glossaryGaps` / `conditionalPending`。複合ラベルの表記ゆれは `normalizeTerm` (先頭コア語) で吸収 |
