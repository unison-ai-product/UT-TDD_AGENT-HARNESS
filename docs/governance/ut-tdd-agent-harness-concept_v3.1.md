...
L0 は「リポジトリ初期化」ではなく **企画工程**。企画書 PLAN に背景・目的・スコープ・投資対効果・成功条件・KGI/KPI・想定リスクを必須化する。企画書は `kind=charter` (layer=L0、`parent_design` 不要 = root) で起票し、必須 role は `po` (要件定義書 §1.8)。**G0.5 企画突合**で、現在作業している AI とは別系統の `frontier-reviewer` による adversarial check を 1 回必須とする (単一 AI 時は専門サブエージェント review で代替)。G0.5 の fail-close 条件 (企画書 → L1 業務要求への trace) は要件定義書 §2.1.1 に確定する (チーム: po+tl が企画を確定 → tl が L1 起票)。リポジトリ初期化・Branch Protection 等の基盤整備は Phase 0 (要件定義書 §10) として工程外で扱う。
