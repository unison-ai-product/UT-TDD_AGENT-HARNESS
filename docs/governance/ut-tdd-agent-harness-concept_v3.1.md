...
### V-model: 左 (設計) ↔ 右 (検証) の対 (v3.1 で L0-L14 化)

V2 の L0-L14 は左右が 1:1 で対になる V-model を成す。左側で書いた設計には、同層で **③ テスト設計** を対にして凍結し、右側の対応工程で **④ テストコード** として実施する。
...
右側の検証工程で失敗した場合、差し戻し先を明示する: **L8 結合テスト失敗 → L5 詳細設計 または L7 実装へ**、**L9 総合テスト失敗 → L4 基本設計へ**。差し戻し記録は PLAN の carry log に残す。**右側工程で「ペア凍結されていないテスト設計」を新規起票することは V-model 違反** (テスト設計は必ず左側の対応層で凍結済みであること)。
