...
| **L3** 要件定義 | **画面/UI の機能要件 + 画面 AC** (`functional` 内に FE 観点を明示分離) | 画面の振る舞い・入出力・状態遷移を SyRS/AC として確定 | ISO 29148 SyRS + BDD GWT | **穴** (現 `L3-functional/functional` は BE 中心、FE 観点が未分離) |
| **L4** 基本設計 | **UI 設計標準 + UI 部品カタログ + design tokens** (`ui-standard`) | 再利用 FE 設計標準 = `data` (DB 設計標準) の FE 対応物 (§1b) | Nablarch UI標準(画面)/UI部品カタログ/共通コンポーネント設計標準 | ✓ `L4-basic-design/ui-standard.md` + `tokens.yaml` (本 PLAN) |
| **L5** 詳細設計 | **FE 内部設計** (`ui-detail` 候補): コンポーネント分割 / 状態管理 / ルーティング / 画面内部処理 | 画面の内部構造を IEEE 1016 SDD 相当で詳細化 | IEEE 1016 SDD / UML | **穴** (現 `L5-detailed-design` は BE module/物理 DB 中心) |
| **L6** 機能設計 | **per-screen 機能設計** (`screen-spec` 候補): 画面ごとの項目定義 / イベント / バリデーション / 画面内遷移 | 画面 1 枚を関数仕様粒度で確定 (= L7 単体テスト設計の対) | Nablarch システム機能設計書(画面) | **穴** (現 `L6-function-design` は BE 関数中心、per-screen 個別 doc 無し) |
...
> **定義された穴 (本節の成果 = 次に埋める対象、段階順で実体作成)**:
> 1. **L3 FE 機能要件**: 画面/UI の機能要件 + AC を BE functional から明示分離 (slot は `functional` 内の FE 節 or 新 sub_doc、後続で確定)。
> 2. **L5 FE 内部設計** (`ui-detail` 候補): コンポーネント分割/状態管理/ルーティング (schema slot 追加は後続)。
> 3. **L6 per-screen 機能設計** (`screen-spec` 候補): 画面 1 枚ごとの項目/イベント/バリデーション/遷移 = Nablarch システム機能設計書(画面) 相当 (schema slot 追加は後続)。
>
> L4 (`ui-standard`) は本 PLAN で slot 追加済 (`VALID_SUB_DOCS[L4]`)。L3/L5/L6 の slot 追加と各 doc 実体作成は
> 本定義を起点に段階順で行う (定義 → slot → 起票 → 作成 → 実装)。L8/L9 の FE 観点充足は右腕 test-design 側。
>
> ⚠ **候補 slug (`ui-detail` / `screen-spec`) は schema `VALID_SUB_DOCS` 未登録 = 起票に使用不可**。
> 別途 PLAN で schema 追加 (+ 要件 §G.1 + 本 §1c を 3 点同期) してから sub_doc 値として使う
> (`sub-doc-catalog-drift` gate が fail-close。L4 `ui-standard` が踏んだ手順と同じ)。
