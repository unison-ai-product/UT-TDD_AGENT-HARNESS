...
> 各 V/W-model 工程の **① 必須スケルトン (作成必須ドキュメント)** を業界標準 (日本式 IPA 共通フレーム + 国際標準 + Design by Contract) で裏付ける正本。
> メタモデルの ① 必須タスク/doc の grounding 資料 ([[ut-tdd-agent-harness-concept_v3.1]] §3 V/W-model と連動)。
...
| UT-TDD 層 | 日本 SI 工程 | 主成果物 (① 必須) | 国際標準 | W-pair (テスト設計) |
|---|---|---|---|---|
