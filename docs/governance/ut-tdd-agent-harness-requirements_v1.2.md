...
- **検証 (IMP-047 で機械 surface 化)**: PLAN 活動 (active_plan + digest) があるのに `CURRENT.json` が未生成 / stale / 別 PLAN を指す (drift) → `checkHandoverDiscipline` が warn を返し、**Stop-hook (`.claude/hooks/session-log.ts`、stderr、fail-open) と `ut-tdd doctor` (`checkHandover`) の 2 機構で機械 surface** する (PLAN-L7-06)。`plan-lint` / pre-push への配線は lint engine (`src/plan/lint.ts` stub) 実装時の carry。それまでは本機構 + 人手 binding が併存。
