...
- **工程層体系**: v1.2 で **V2 (HELIX-workflows) の L0-L14 + W-model を base に採用** (§1.4 / §2、構想書 v3.1 連動)
...
本書は構想書 v3.1 に対する **要件定義 (HOW を満たす条件)** を確定する。**実装詳細 (Python コード / YAML 全文 / hook script 本体)** は将来の個別 PLAN-XXX 詳細設計で詰める。
...
`kind=impl` (layer=L7 実装スプリント) の PLAN は `parent_design:` フィールドを **必須** とする。値は L6 機能設計 doc (① 設計) への path で、その doc に紐づかない実装は W-model 違反 (構想書 v3.1 §3.5 AP-5)。`parent_design` が不在または存在しない path の場合、`vmodel_validator` は **exit 1** (fail-close)。design 系 kind (L0-L6) では任意。

## 1.1.poc kind=poc variant (Scrum 専用、R-C1 fix)

```yaml
---
plan_id: PLAN-X-01-slug                       # layer=cross → ID layer token は X
title: "..."
kind: poc                                     # 固定
layer: cross                                  # 固定
...
```yaml
---
plan_id: PLAN-X-02-slug                       # layer=cross → ID layer token は X
title: "..."
kind: reverse
layer: cross
...
v1.2 で **V2 (HELIX-workflows) の L0-L14 + W-model** を base に採用。左 (設計 L0-L6) で書いた設計には同層で ③ テスト設計を対に凍結し、右 (検証 L8-L14) の対応工程で ④ テストコードを実施する。旧 L0-L11+小数層は本表に remap した (構想書 v3.1 §3 連動)。

| layer | 名称 | ① 設計 / ③ テスト設計ペア (W-model) | 旧 v1.1 layer からの remap |
|-------|------|--------------------------------------|----------------------------|
| `L0` | 企画 | 企画書 (G0.5 企画突合) | 旧 L0 基盤整備は Phase 0 (§10) へ分離 |
| `L1` | 要求定義 (業務要求 BR-*/NFR-*) | ① 業務要求 / ③ 運用テスト設計 (→L14 実施) | 旧 L1 (業務要求部分) |
| `L2` | 画面設計 | ① ワイヤーモック (mock がペア →L10) | (新規) |
...
- [ ] 形式が `^PLAN-(L(?:[0-9]|1[0-4])|X|M)-\d{2}(-[a-z0-9-]+)?$` に一致 (`ut-tdd plan lint` で正規表現検証)
  - 構造 `PLAN-<layer>-<NN>-<slug>`: **layer** = `L0`〜`L14` (該当工程) / `X` (cross = poc・reverse 等の横断) / `M` (master plan)、**NN** = layer 内 2 桁連番、**slug** = kebab。
  - 例: `PLAN-L1-01-business-requirements` / `PLAN-L7-02-plan-lint` / `PLAN-X-01-some-poc`。
  - 狙い: **ID 単体でフェーズ (L0-L14) が判別でき、phase ↔ PLAN のマッピングが容易**。archived の旧 flat `PLAN-001..004` とは別名前空間で衝突しない。
- [ ] **ID の layer token が frontmatter `layer` と一致** (`layer=cross` → ID は `X`、master → `M`)。不一致 → exit 1。
...
### 段階 A: Pair freeze (設計⇔テスト設計、W-model 左各層。L7 実装前)
...
## 2.7 受入条件 (V-model 工程、R-C3 fix で統一。v1.2 で L0-L14 W-model 化)

- [ ] W-model 左各層の pair freeze (①⇔③) 不在 → exit 1 (G1=L1 運用 / G3=L3 受入 / G4=L4 総合 / G5=L5 結合 / G6=L6 単体)
...
- [ ] 右側工程 (L8 結合 / L9 総合) でペア未凍結のテスト設計を新規起票 → P0 fail-close (W-model 違反、構想書 v3.1 §3.5 AP-7)
...
policy:
  ...
  single_runtime_review_checklist: docs/skills/review-checklist.md  # DOC/TST/COD/XR 明文化 checklist の正本
members:
  - role: tl
  ...
  for each PLAN file in §1.10 対象:
    ...
    case kind:
      design / add-design / research:
        → §2.2 段階 A (Pair freeze ①⇔③) を W-model 左各層 (G1/G3/G4/G5/G6) 対象で検証
      impl / add-impl:
...
- [ ] **ルール同一性 (MUST、構想書 §2.1.0)**: gate / V-model / checklist / enum / route の正本は `ut-tdd` core + governance docs に単一定義され、`.claude/CLAUDE.md` / `AGENTS.md` がルールを再定義・分岐していない (doctor が両 adapter のルール重複・drift を検出し、検出時 fail)
- [ ] **rule parity test**: 同一 PLAN / diff を claude-only と codex-only で処理した際、`ut-tdd gate` / `ut-tdd plan lint` / `ut-tdd vmodel lint` の **判定結果と exit code が一致**する (runtime 差で結果が変わらない)
- [ ] **hybrid 機能分散 (MUST、構想書 §2.1.0)**: `ut-tdd team run` が `hybrid` で判断系 / 実行系を別 runtime に割り当て、同一 role の同一 runtime 重複・同一作業の二重実行を exit 1 で弾く (§7.1 team run 検証 7)
...
interrupt / debt / drift-check / readiness と doctor 検出器 (relation-graph / doc-drift / connection-deficiency / regression) + test-perspective-gate を `ut-tdd doctor` / `ut-tdd plan lint` に束ねる。検出は `.ut-tdd/` state を参照し、`helix.db` には依存しない。test-perspective-gate は W-model 各ペアの **観点網羅** (抜け) と **レベル間非重複** (重複) を `--static-only` で fail-close 検証する。
...
- DOC-2: ③ テスト設計 doc が同層で存在し ① と pair freeze 済み (W-model 左)
...
- [ ] `ut-tdd gate <判断ゲート>` は `ut-tdd status` の `mode` を参照し、`hybrid` 以外では gate 結果 JSON に `cross_agent_review: unavailable` を必ず含める (含まなければ実装不備として exit 1)
- [ ] 単一エージェント (claude-only / codex-only) の判断ゲートは **② 専門サブエージェント review を hard 要件**とし、§7.8.7.1 の必須 checklist 項目 (DOC / TST / COD / XR / DEP / DUP / MOD) 全件の逐条実行記録 (各 pass/fail/n-a + 根拠) が無ければ **exit 1**。揃えば `passed: true` + `review_kind: intra_runtime_subagent` を記録する
- [ ] ③ naive self-review (実装者の自己読み直し) を判断ゲートの通過根拠として記録してはいけない (cross-agent / intra_runtime_subagent と区別)
- [ ] `same_model_approval: forbidden` を実行時強制: 承認者 (frontier-reviewer) と worker の `(provider, model)` が一致したら **① cross-agent としては無効化** (hybrid でも同一モデル割当を弾く)。② はそもそも同一モデル前提のため cross-provider 要件には数えない
- [ ] checklist のいずれかの項目が `fail` なら gate を止める。`n-a` は根拠必須 (根拠なし n-a は欠落扱いで exit 1)
- [ ] `orchestration_mode` が要求する agent が現 execution mode で不在の場合、silent fallback せず縮退 (§7.8.4 / 構想書 §2.1.2.1) を適用し、`degraded_from` / `degraded_to` を記録する
- [ ] **escalation 境界 (本番影響 / 認証 / 認可 / 決済 / PII / ライセンス / destructive)** に該当する変更 (XR-2 で検出) は execution mode を問わず人間サインオフ必須。未承認なら mode に関わらず exit 1 (② でも代替不可、§8 と整合)
