...
- **機械強制 (目標、IMP-086)**: doctor が `status: open` の FB / `domesticated to` 空の FB を surface し、参照先 (memory/IMP/doc) の実在も突合する。「フィードバックがドメスティック化されずに放置」を fail-close で塞ぐ (柱2 doc×機械、柱3 フィードバック機構)。
