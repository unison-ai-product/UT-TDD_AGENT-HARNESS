...
出典: concept v3.1 §2.5 (9-mode ecosystem) / §2.6.1 signal→mode (`user_feedback_iteration`) / requirements v1.2 §1.3 kind=poc / §1.5 workflow_phase S0-S4 / §1.6 drive=scrum / §1.8 role=aim / helix-process/scrum-workflow.md (翻案元)

---

## 1. 概要

Scrum は **作るものは概ね決定済だが、要件をユーザーとの反復で固めていく**モード。Discovery (「そもそも作れるか/何を作るか未確定」) とは入口が異なり、ユーザーフィードバックによる継続的な要件調整 (`user_feedback_iteration`) が trigger。Discovery と同じ `kind=poc` だが `drive=scrum` で識別する。
...
| S1 | Sprint Plan | スプリントバックログ選択・受入条件確定 | sprint backlog PLAN (kind=poc, drive=scrum) |
