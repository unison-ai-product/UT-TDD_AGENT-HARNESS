...
| ST-EXT-07 | provider execution seam (`external-if.md` §3.1) | execution kind を CLI/hook/fake port 間で差し替えても、capability preflight 不成立なら起動0、成立後は success/non-zero/timeout/cancel/error の全経路で terminal receipt exactly once、receipt 確定時 `orphan_count=0` となる。provider exit 0でも receipt/cleanup 不成立ならsystem outcomeはfail-closeする | U-ADAPTER-010〜015 + provider adapter integration。native OS custody / parent-loss / descendant escape は独立した `ST-RGK-*` で検証し、本STのmock Greenを代用しない |
...
- external-if.md §3.1 provider execution seam → ST-EXT-07 (capability preflight / terminal receipt exactly-once / orphan 0)。native Resource Kernel の OS 固有保証は ST-RGK-* へ分離
