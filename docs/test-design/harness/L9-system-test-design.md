...
> **status**: confirmed (G4/A-101 freeze — ST カテゴリ ⇔ L4 設計要素の被覆を凍結、孤児 0)。個別 ST ケース (Given-When-Then) は検証 band (L8-L14) / L9 本起票で展開し、未確定分は `placeholder_deps` で追跡 (ST-ASSET-04 等)。L3↔L12 と同型。
...
| ST-ASSET-05 | skills building block (FR-L1-47、architecture §3) | `docs/skills/` curated assets are non-empty as of A-116 (`review-checklist.yaml`), so the previous "current empty" precondition is closed. Full skill catalog / recommender / injector behavior remains L7 carry: catalog reads curated skill assets, layer-scoped injection is deterministic, and dependency direction stays schema-only. | vitest skills (L7) + dependency lint |
| ST-ASSET-06 | 内部資産 drift lint (FR-L1-49、architecture §4.1) | Current hard gate slice is implemented: HELIX personal path residue / legacy `helix codex|claude|plan|gate|handover` delegation residue / docs-skills vacancy / guard allowlist missing agent docs → fail-close. Full roster↔guard semantic drift remains ST-ASSET-07 carry. | vitest `asset-drift` rule (implemented) |
| ST-ASSET-07 | roster↔guard 整合 移行段階 (Critical-2、function §1.1) | roster 未実装期間は guard ハードコード allowlist 維持、roster 実装後に切替。整合検査は **placeholder_deps [実装状態解消型、physical-data §7]: {waiting_layer: L7, waiting_spec: roster module 実装 + guard 切替}**。Current doctor does not have a dedicated `placeholder_deps` rule; unresolved state is now explicit L7 carry in A-118 instead of being claimed as already fail-closed. | L7 roster implementation + follow-up placeholder-deps lint |
...
- **function.md §1.1 C12 内部資産 roster/command (FR-L1-46/48) → ST-ASSET-01〜03 / architecture §3 skills (FR-L1-47) → ST-ASSET-05 / architecture §4.1 drift lint (FR-L1-49) → ST-ASSET-06 (implemented hard-gate slice + remaining roster/skills carry) + ST-ASSET-04 (unmaterialized command/recommender system detail = L7/L9 back-fill carry)**
- external-if.md §3 境界 4 / §4 degradation → ST-EXT-01〜04
- **孤児 (設計要素で ST 未被覆) = 0** を L9 本起票で機械確認する。Current hard evidence is pair-freeze orphan 0 + implemented asset-drift slice. A dedicated `placeholder_deps` doctor rule is **not yet present**; unresolved ST-ASSET-04/07 details are reviewed and routed as L7/L9 carry in A-118 instead of being claimed as already fail-closed.
