...
| `CAND-NODEBOOT-207` | genesis付き5-state cutover receipt chain | `CUTOVER-EVIDENCE-REGISTRY-v1`とL7正式ID family `U-CUTOVER-{001–009}`をoracleとし、F0 producer commitsのdescendant closureを要求する。空chain、wrong-edge、stale/replay/non-ancestor、skip/reverse、digest不一致を拒否しprojectionはvalidated chainと一致 |
