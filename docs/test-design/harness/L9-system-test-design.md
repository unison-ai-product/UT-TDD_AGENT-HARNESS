...
| CANDIDATE-ST-WTLIFE-006 | planned の activation 未解決、owner不明、terminal欠落/不一致、inventory欠測を各1軸で入力する | activation 未解決の retire 要求は `activation_unresolved`、その他は `owner_unknown`/`terminal_missing`/`terminal_mismatch`/`inventory_unavailable` を保持し、TTLで Green にしない | negative |
...
| CANDIDATE-ST-WTLIFE-011 | canonical/durable HARNESS state、retention中の cache/scratch、retention到達後の local cacheを混在させ、`RetentionPolicyPort` の policy id/revision 欠落・drift と adapter の自己申告値/時刻を各1軸で入力する | `canonical_state`/`retention_active` を返し、policy authority以外の値を採用せず、正本 state は削除せず、許可された local cache のみ apply する | mixed |
