...
| `ST-RGK-12` / `AC-RGK-12` | required capabilityを一つずつ欠落、空集合、probe/journal/token barrierを各一箇所除去し、command名だけで`create/spawn/resume`をExecute外へ移したadapter matrixでadmission。Recovery leaseのvalid/stale/別nonceも併走 | control process/probe事実を別記し、全不足・token不正・token迂回caseが`managed_root_created=false`の`capability_failure`。Probe/Recoveryのlauncher・生成・resume call 0。valid leaseのterminate/prove-emptyはtoken期限後も完遂 | handshakeだけでexecute、Recoveryから生成/resume、cleanupへadmission tokenを要求して期限後cleanup不能、control/rootを単一boolean化、warning、skip、PID polling、soft limitへ暗黙縮退 |
