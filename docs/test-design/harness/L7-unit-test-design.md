...
| `CANDIDATE-RELMAN-011` | PF-2 materializer / #248 / `PLAN-L7-486` | 同じsource entry集合の2呼出し、入力順反転、`docs/skills→skills` remap、workflow template source mapping、`package.json` transform、destination path/mode/content、symlink target/`120000`、UTF-8 byte sort、uint32be/uint64be framing、destination衝突、unsupported mode、root外symlink、未知versionを各1件mutation | version 1 materializer後のdestination path/mode/contentから同一digest。source blobだけ同じでPack outputが違えばmismatch。衝突/mode/symlink不正はtyped invalid、未知versionは`unavailable`でfallbackしない。入力は不変、manifestはcontrol copyでdigest対象外 |
