...
| `U-RVATT-027` | repository snapshot lifecycle | `tests/live-review-projection.test.ts`。実application compositionへ既存merge-gate GitHub ports fixtureを注入し、dispatch→request→delegated verdict→receipt→同一HEAD wrapperを実行。request/receipt/HEADを各1点欠落・変異 | network 0で正常系だけallow。3負例はdenyし、wrapper成功receipt後のD2-D `bypass_merge`は0 |
...
| `CANDIDATE-RELMAN-012` | PF-3 resolver / #249 / `PLAN-L7-487` | control HEADで`stable=v1` / `canary=v2`、v1/v2 object、実`--filter=blob:none` partial cloneのmissing blob、2 MiB超かつNULを含むblob、`cat-file --batch` framingを個別mutation | 選択channelだけを対応revisionからmaterialize。control HEADとのSHA一致を要求しない。全Git childへ`GIT_NO_LAZY_FETCH=1`を固定し、欠落時はpromisor remoteが到達可能でも`unavailable`、remote request/materializer/reconstruction/copy 0。binary contentはdeclared sizeどおりbyte同一で渡す |
