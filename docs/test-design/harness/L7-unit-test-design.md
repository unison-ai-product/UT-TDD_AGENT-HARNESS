...
| `U-RVATT-024` | delegated-attestation-before-display | `tests/live-review-projection.test.ts` / `tests/cli-delegation.test.ts` / `tests/review-live-cli.test.ts`。interactive session/CLI optionのprovider自己申告、verdict欠落、identity mismatch、receipt write failureに加え、Codex/Claude著者を同族childへroute、未知family、反対族runtime不在を各1点変異。live-dispatchのwake targetもreviewer providerへ束縛し、Codex reviewerをClaude workspaceへ誤配送する変異を拒否。Codex project inboxはcanonical envelopeをFIFOでsurfaceし、consume後にterminal marker＋inbox projection除去を行い、後続requestをhead-of-line blockしない | 負例はreceipt/comment/memory 0。Codex著者→Claude child、Claude著者→Codex childのspawn provider/model/role/time/exitCodeに束縛したattestationだけreceipt 1→派生表示。Codex受信面が未提供ならClaude workspaceを参照せずtyped unavailableでcanonical requestだけをbacklog保持。Codex wakeはsurfaceだけでは完了扱いにせず、`review live-consume --envelope` 成功後だけterminalizeする |
