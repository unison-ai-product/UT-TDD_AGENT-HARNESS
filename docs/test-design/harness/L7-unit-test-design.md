...
| `CAND-NODEBOOT-029` | `scripts/ut-tdd.ps1`のBOM変異 — BOM欠落、BOM 2個以上、UTF-16 LE/BEへのencoding差し替え | いずれも`C_ps1`由来の受理4集合外としてfail-close (PLAN-L6-93 §5.2.1)。**POSIX側と同一規則を当てる実装は BOM欠落を通してしまう**ため、file別canonical byte列であることを固定する |
