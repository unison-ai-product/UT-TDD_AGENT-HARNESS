...
| U-SETUP-009a | `loadTemplates` / built-in adapter templates | `docs/design/harness/L6-function-design/skill-index.md` の runtime asset 配布境界に従い、配布 subagent template は `model:` frontmatter を持つ。代表 3 family として `pmo-sonnet`=sonnet、`pmo-haiku`=haiku、`pdm-tech-innovation`=opus を固定確認する。 |
...
- **skill-index.md §7 配布 repo の skill 配置 / runtime asset 境界 → U-SETUP-009a** (配布 adapter の `model:` frontmatter。representative family を unit test で固定。孤児 0)
