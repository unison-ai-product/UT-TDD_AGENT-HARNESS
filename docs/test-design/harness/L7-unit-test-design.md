...
| U-SETUP-009b/b2/b3 | `common/ut-tdd.mjs` wrapper (`emitSetup` output) | 生成 wrapper の resolution order (local `node_modules/ut-tdd` → 同梱 setup Pack CLI fallback) を固定する。**Issue #506 (2026-09-01) 記録**: U-SETUP-009b2 (`node_modules/ut-tdd/src/cli.ts` local-package 分岐) は Node が `node_modules` 配下の `.ts` 実行を拒否する (`ERR_UNSUPPORTED_NODE_MODULES_TYPE_STRIPPING`) ため `it.skip` — Bun 撤去の fixture 問題ではなく、wrapper の local-package 分岐が compiled/self-contained entry point を要求する実プロダクト課題であり、#420 / #463 (self-contained consumer Node runtime) に帰属する。U-SETUP-009b3 (setup Pack CLI fallback、node_modules 外の source path) は影響を受けず Green のまま。 |
...
| U-SETUP-012 | `buildConsumerReadinessPlan` | consumer `package.json` の `engines.node` (npm semver) と Git を正本に Node/npm + git/project-local `ut-tdd`/runtime CLI を preflight として診断する。Node missing、`engines.node` missing/invalid、または supported range 外 (below-range / above-range) は blocking。gh は GitHub setup 用 warning、runtime CLI 不在は `mode=standalone` の advisory とし、judgment gate は人間レビュー必須として表示する。生成 adapter hooks は `.ut-tdd/bin/ut-tdd.mjs ...` を呼ぶため、project-local CLI 未解決なら consumer hook 自走性を満たさず readiness を fail-close する。`checks`、`ci.requires`、`rollback.commands` は Node/npm 経路だけを提示し、Bun の check・導入案内・実行コマンドを含めない。rollback managed paths、tag-pin contract、CI self-sufficiency、monorepo package-root 判定、全 smoke scenario を返す。単軸 mutation として Bun check/導入案内/実行コマンドの再導入、range guard の削除、CI/rollback への Bun command 再導入を Red とする。 |
| U-SETUP-013 / AT-DIST-001 | `tests/distribution-acceptance.test.ts` | Local clean distribution acceptance smoke は planned clean artifacts を temp repo に materialize する。fixture の install/status/setup/typecheck は Node/npm で実行する (install は `npm ci`、CLI 実行は `process.execPath` 直接起動、typecheck は `npm run typecheck`。Issue #506 で実 Bun spawn を退役済み、real Bun spawn 0 件の契約を維持する)。`bun.lock` は direct-graph parity の検査対象として残る (PLAN-L7-527 §2.5)。Pack `v0.1.0` tag / GitHub Release 後も、この local smoke は署名 tarball publish / UAT / post-release telemetry を実行しない。source repo 用 full `doctor` は dogfood PLAN/design/test-design/runtime artifact を除外する clean distribution の受け入れ条件には含めず、consumer doctor profile が必要なら別 PLAN とする。Issue #405 の再発条件は、clean Pack から `package-lock.json` を除去すると `bun run test:pack` が Vitest 開始前の `npm ci` で fail-close すること。 |
...
| `U-PA-SEAL-004` | E.6 certificate/source authority/review authority digestを各1 bit改変 | `SealedLineageLocalMigration.migrate` のcertificate preflight | `seal-certificate-digest-mismatch` / `seal-source-authority-invalid` / `seal-review-authority-invalid`で拒否し、asset/revision/alias/admission/custody/seal/certificate/receiptのwrite 0 |
| `U-PA-SEAL-005` | source commit到達不能、source/projection path・blob OID・payload・custody・terminal不一致、HEAD TOCTOU | Git preflight port + tracked source/projection verifier | 各不成立を専用`seal-source-*` / `seal-projection-*` findingでfail-closeし、全8 tableのwrite 0 |
| `U-PA-SEAL-006` | tracked projectionの最大sequenceが同値で2件存在 | historical projection terminal resolver | terminalを推測で選ばず`seal-projection-terminal-mismatch`で拒否し、全8 tableのwrite 0 |
| `U-PA-SEAL-007` | Git preflight port未注入 | `SealedLineageLocalMigration.migrate` authority preflight | tracked authorityを補完せず`seal-git-preflight-unavailable`でfail-closeし、全8 tableのwrite 0 |
| `U-PA-SEAL-008` | review authority port未注入 | `SealedLineageLocalMigration.migrate` custody preflight | review authorityを自己申告値で補完せず`seal-review-authority-invalid`でfail-closeし、全8 tableのwrite 0 |
| `U-PA-SEAL-009` | seal成功後、Git/review authority portが一時的に例外を返す同payload再送 | durable command replay lookup | authority portを再観測せず既存resultを`replayed: true`で返し、行数を増やさない |
| `U-PA-SEAL-010` | custody observationが未知reasonまたは同一reason重複を含む | review authority observation validator | canonical reason集合へ推測変換せず`seal-review-authority-invalid`でfail-closeし、全8 tableのwrite 0 |
...
| `U-RVDISP-054` | 独立identity変異 | memoryId/pr/head/reviewRevision/reviewer-family/kind-verdict | identity軸はpending＋`identity_mismatch`、same-familyは`same_family_reviewer`、kind/verdict不整合は`schema_invalid` |
...
| `U-RVDISP-060` | FLAG verdict projection | identity一致した非author FLAG receipt | requestを`consumed`へ進め、`flagged`を別出力する。`identity_mismatch`へ誤分類しない |
| `U-RVDISP-061` | null receipt projection | receipt JSONが`null` | TypeErrorを発生させず、対応requestをpending＋`schema_invalid`としてfail-close |
| `U-RVDISP-062` | receipt ordering projection | identity一致だがrequest以前のreceipt | pending＋`receipt_before_request`としてtyped fail-closeし、`identity_mismatch`へ潰さない |
| `U-RVDISP-063` | strict request revision projection | basename identity digestと不一致の`rv1-<digest>` | `isStrictReviewRequest`で`filename_digest_mismatch`としてfail-close |

実行対応: `tests/review-dispatch.test.ts` (`U-RVDISP-001`〜`052`) と
`tests/review-projection-reconciliation.test.ts` (`U-RVDISP-053`〜`063`)。
...
| ID | 設計境界 | fixture / mutation | expected |
| --- | --- | --- | --- |
| `U-RVHEAD-001` | old HEAD FLAGのcurrent HEAD supersession | old HEAD FLAG + newer exact-current-HEAD non-author PASS、両requestとreceiptを保持 | `runPrMerge`はmergeし、analyzer/audit側にはold FLAG entryとfindingが残る |
...
| `U-RVATT-037` | terminal retry identity/orphan resistance | `tests/review-verdict-custody.test.ts`で valid terminal receipt、orphan receipt、validな`attempt_completed`を基準とするexactHead / verdictPath / receiptFileDigest / reviewer provider family / model非空 / exitCode / verdictDigest の各単独変異を別caseにし、retryを再実行。receipt欠落のcrash-windowでは正規providerと同族providerを分離する。各guard削除のmutation probeで対応caseをRedにする | orphan/identity-driftだけではterminal扱いにせずtyped retryを許可し、receipt/audit/verdict-tree bytesを上書きしない（request metadataの再発行は許可）。Codex著者のCodex provider completionなど同族eventはterminalに昇格せず、receipt欠落時も`attempt_outcome_indeterminate`で停止。terminal canonical receiptが存在する場合だけ`review_receipt_already_exists`とwrite 0 |
| `U-RVATT-038` | malformed custody audit fail-close | `tests/review-verdict-custody.test.ts`でreceipt fileを配置し、監査JSONLを不正JSONへ変異して`issueReviewRequest`を実行 | `attempt_outcome_indeterminate` を返し、reviewer CLIへ `SyntaxError` を漏らさず、request write 0、receipt bytes不変 |
...
| `U-GHPROJ-010〜015` | SQLite projection / binding custody | schedule投影、rebuild clear、out-of-order観測、stale HEAD、pending outbox、object付替え、全lifecycle bindingのrevision rollover | readinessは再構築され、binding/outboxは保持。PR HEAD不一致のCI/review/mergeはwrite 0。別PLANへのprovider identity付替えと古い観測への後退を拒否し、同一PLANの新revisionへは収束する |
| `U-GHPROJ-019〜026` | Project V2 reconcile | custom field key mangling、dry-run、create、同値再送、重複item、field欠損、binding保存、同期不整合 | mojibakeを含むprovider keyを正規fieldへ束縛する。dry-runはremote/DB mutation 0のままapplyと同じ目標field値を示し、同値はno-op、重複/契約driftと不整合applyはfail-close、item identityをdurable保存 |
| `U-GHPROJ-030〜038` | lifecycle証跡選択 / closure fail-close | 複数open PR、旧merged PR後着、旧Project revision、管理済み完了item、証跡なし完了status、偽造・失効merge receipt、明示repo root、stale Project head、repository越境 | 複数openは`不整合`、現行open HEADだけを採用、旧revisionを無視、管理済みitemは完了状態まで収束同期する。statusだけでは完了・後続解放せず、PR/main check IDの成功bindingを再照合できる正規receiptだけをmerge証跡に採用し、失効receiptやmerge row単独注入は不採用。review sourceはprocess cwdでなく注入repo rootから読み、stale Project headでPR headを上書きせず、Project管理済み判定をrepository内へ限定する |
| `U-GHPROJ-039〜052` | revision / Project outbox / admission custody | 正規payload、同一/変更digest、applying中enqueue、stale完了、同期失敗、repository越境、global projection、legacy PLAN、PLAN不在、partial/未追跡admission receipt、未着手PLANの空HEAD | payload validation後にdispatch対象全件をatomic claimする。Project projection/binding保存とapplied化はcurrent outbox identityを再照合した単一transactionであり、stale workerはwrite 0。失敗はclaim済みintentだけretry可能に戻す。legacyはcanonical tokenで安定する。partial receipt、tracked hash chain不在、receipt/content digest不一致はschedule/readiness/outboxを全てwrite-zeroとし、`source_hash`をrevisionへ流用しない。`headSha=""`だけは未着手PLANのProject field clearとしてqueueできる |
| `U-GHBIND-001〜020` | repository facts同期 / typed closure receipt | typed PR trace、全lifecycle、revision/HEAD/review custody、Project item、transaction中network、manual evidence、canonical admission、trace base SHA ancestry、source hash fallback | 完全trace、main向けPR、required check、異provider2 lane receipt、Project item、Issue closeが同じcanonical revision/HEADへ揃う時だけclosureする。admissionはtracked receipt hash chainとcanonical content digestまで照合し、legacyだけcanonical tokenを許す。repository-scoped compareで`trace.base_sha...headSha`が`ahead|identical`の場合だけbindingを書き、diverged/behind/provider errorはwrite-zero。PLAN/source/revision欠損、partial/未追跡receipt、`source_hash`代替、truncated snapshot、競合はfail-closeする。provider観測はtransaction前に完了し、書込競合は全rollbackする |
...
S3の副作用境界はpure predicate単体の呼出し回数では証明しない。実装testは既存PF5の
pointer write / publish / apply / restore portをinjected composition harnessへ接続し、deny・
indeterminate各経路のspy countとprior bytes/stateを観測する。S3 gate自身はportを直接呼び出さず、
`sideEffects: "none"`は型契約として保持する。staleまたは別subject（HEAD、PLAN revision、release
identity、artifact/evidence digest、channel）は`identity_mismatch`、欠落だけを`*_missing`へ分類し、
staleをmissingへ丸めない。既存PF5 apply結果は`unavailable`→`artifact_unavailable` deny、
`rollback_failed`/`applied=indeterminate`→`rollback_failed` indeterminateへ写像する。
...
| `U-RELMAN-003` | S3 promotion / `PLAN-L7-494` / #363 | canonical Linux/Windows/aggregate CI、QA Go、cross-family closing receipt、およびcanonical `ReviewRequest`からD1/D2/facts/claim/specへの各mappingを1軸ずつ欠落・stale・別subjectへ変異。D1、D2、claim/spec各laneの`PASS` / `PASS-WEAK`は相互同値を要求せず単独変異 | CI absent=`ci_missing`、QA absent=`qa_missing`、QA non-go=`qa_no_go`、review/lane absent=`review_missing`。stale/別subjectまたはsource mapping不一致は`identity_mismatch`でpromotion拒否。各verdictの有効な混在はallow。composition harnessのchannel pointer/write/publish spy 0、prior state不変 |
| `U-RELMAN-004` | S3 rollback / `PLAN-L7-494` / #363 | 同じmanifest・prior release・targetを2回rollback評価 | 同一pointer deltaとdigestへ収束し、二重apply 0 |
| `U-RELMAN-005` | S3 rollback / `PLAN-L7-494` / #363 | rollback実行計画を生成 | force push / tag付替え / commit / push command 0 |
...
| `U-RELMAN-018` | PF-4 adapter hardening / `PLAN-L7-489` / #331 | `tests/release-channel-adapter.test.ts` のidentity drift・port throw・typed reason | resolverのrelease identity 3値 (`releaseId` / `artifactSourceCommit` / `artifactSetDigest`) の変異は`unavailable(invalid_artifact)`、port throwは`unavailable(unavailable)`、resolverの`invalid_distribution_plan` / `invalid_artifact`は理由を保持して透過する。既存U-RELMAN-006の三値テストは変更しない |
| `U-PACKPUB-001` | publication manifest v2 pure domain / `PLAN-L7-499` / #380 | `tests/release-manifest.test.ts` のstrict v2 schema、非ASCII literal golden、identity単独・協調mutation、v1 read-only、UTF-8 canonical path、deep freeze | explicit artifact inventoryだけを受理し、inventory／release-record digestのbyte framingを独立literalで固定する。unknown/missing/type、digest・identity drift、重複・非正規path／mode／size、lone surrogateをtyped denyし、v1は読取可能でも新規publicationでは`v1_read_only`、成功snapshotはdeep immutable |
...
| `U-RELMAN-008` | S3 promotion / `PLAN-L7-494` / #363 | no-go未解除のままstableへpromotion | dependency不足で拒否、channel pointer不変、write/publish 0 |
...
| `U-RELMAN-010` | S3 promotion / rollback / `PLAN-L7-494` / #363 | valid manifest deltaに対しpromotion/rollback双方のD2 `ok` / `state` / `reasons`を各1軸で非ready化し、rollbackではD2自体も欠落 | promotion/rollbackを拒否し、実PF5 compositionのsnapshot/staging/apply/restore/pointer/publish 0、prior state不変 |
| `U-RELMAN-019` | S3 promotion / `PLAN-L7-494` / #363 | `releaseId`、`sourceRevision`、`artifactDigest`、materializer version、channelの各identityを1要素ずつ変異 | すべて`identity_mismatch`として拒否、channel pointer不変、composition harnessのwrite/publish spy 0 |
| `U-RELMAN-020` | S3 promotion / `PLAN-L7-494` / #363 | manifest、PF4、PF5、CI、QA、D1/D2/facts/claim/specを各1軸で変異する。caller expected bindingとD1/D2/facts/lanesを一括共変異してもcanonical `ReviewRequest`は変えないcoherent splice、receipt digest単独drift、CI/QA timestamp逆転、review nested array/field、sealed plan kind/destination/entries/entry shapeをtableで攻撃する。`observedAt`単独の経過時間は変異対象にしない | shapeは読取前に`invalid_input`、canonical request source mapping・digest・timestamp不一致は`identity_mismatch`。factsはOPEN、D1 reasons/breachesは空を要求する。non-attested attestationのidentity各fieldをstatusより先に照合し、別subjectをavailabilityへ丸めない。実PF5 compositionの全port 0、prior state不変 |
| `U-RELMAN-021` | S3 rollback / `PLAN-L7-494` / #363 | 直前attested rollback候補を0件、2件、attestation欠落、non-attested別identity、artifact可用性欠落、runtime `[null]`へ各1点変異し、全denyを実PF5 compositionへ渡す | throwせずtyped deny。別subjectは`identity_mismatch`、`[null]`は`invalid_input`。各caseで現行pointer・artifact bytes不変、snapshot/staging/apply/restore/pointer/publish全port 0 |
| `U-RELMAN-022` | S3 rollback / `PLAN-L7-494` / #363 | allow済みrollback selectionから既存PF5 `applySealedReleaseAggregate`へ接続し、apply後restore不能faultを注入 | `error=rollback_failed, applied=indeterminate`を`decision=indeterminate, reason=rollback_failed`へfail-close。apply/restore各1、pointer/publish 0、変化したstateを観測して成功扱い・prior不変の虚偽報告をしない |
| `U-RELMAN-023` | S3 promotion / `PLAN-L7-494` / #363 | product-defined channel順序の次段でないtarget、channel pointerの旧revision、unknown targetを各1点変異 | promotion拒否、channel pointer不変、write/publish 0 |
| `U-RELMAN-011` | PF-2 materializer / #248 / `PLAN-L7-486` | `tests/release-materializer.test.ts` の `U-RELMAN-011`。同じsource entry集合の2呼出し、入力順反転、`docs/skills→skills` remap、artifact空間からのworkflow source逆引き、`package.json` transform/decode/parse、destination path/mode/content、symlink target/`120000`、UTF-8 byte sort、uint32be/uint64be framing、dedupe前destination衝突、invalid plan/missing source/空artifact set、path形式、unsupported mode、root外symlink、allowlist到達済みcontrol manifest、version token (`"1"` / number `1` / `"v1"` / trim付き / 未知)を各1件mutation | workflowのcontent/modeはtemplate source entry由来。文字列`"1"`のversion 1 materializer後のdestination path/mode/contentから同一digest。source blobだけ同じでPack outputが違えばmismatch。plan/source/path/mode/symlink/package不正はtyped invalid、非`"1"`tokenは`unavailable`でcoerce/trim/fallbackしない。入力は不変、control manifestはplanに含まれてもPF-2が明示除外し出力にもdigestにも含めない |
| `U-RELMAN-012` | PF-3 resolver / #249 / `PLAN-L7-487` | `tests/release-artifact-resolver.test.ts`。control HEADで`stable=v1` / `canary=v2`、v1/v2 object、実`--filter=blob:none` partial cloneのmissing blob、2 MiB超かつNULを含むblob、`cat-file --batch` framing、caller Git env/replace refs、Git/materializer port例外を個別mutation | 選択channelだけを対応revisionからmaterialize。control HEADとのSHA一致を要求しない。全Git childへ`GIT_NO_LAZY_FETCH=1` / `GIT_NO_REPLACE_OBJECTS=1`を固定しrepository selector envを除去。欠落時はpromisor remoteが到達可能でも`unavailable`、remote request/materializer/reconstruction/copy 0。binary contentはdeclared sizeどおりbyte同一で渡す |
...
| `U-RELMAN-014` | PF-5 aggregate / #251 | `src/setup/release-aggregate-admission.ts` のfinal-tree manifest SSoT、clean Pack allowlist、selected-revision copy predicateを各条件（mapping cardinality、releaseId、sourceRevision、revision format、sourcePath、source/destination path format、destination allowlist）ごとに1点ずつ欠落 | aggregate admission guardが3 predicateをside effect前にAND判定し、欠落時はsealed planを発行せずresolver/materializer/copy/write count 0。Git commit境界は判定しない |
| `U-RELMAN-015` | PF-5 aggregate / #251 | `src/setup/release-aggregate-admission.ts` へschema invalid manifestを入力 | pure errorを受けてresolver/materializer/copy/write count 0。PF-1のpure `001`とは別に実測する |
...
| `CAND-NODEBOOT-025` | 各wrapperのbyte列を**そのfileの**受理4集合 (`C` / `C0` / `CRLF(C)` / `CRLF(C0)`、`C`は`C_posix` または `C_ps1`) の外へ出す — lone CR、NUL、末尾の追加空行、行末trailing whitespace、Unicode正規化差異 (NFC/NFD)、homoglyph、zero-width文字、UTF-8として不正なbyte列 | すべてbyte列不一致としてfail-close (PLAN-L6-93 §5.2.1 比較手順)。**個別規則を持たず4集合への帰属だけで判定する**ことと、終端LFが2個以上を受理しないことを固定する。**BOMはfile別契約なので本caseの刺激に含めない** (POSIXへのBOM混入は030、`.ps1`のBOM変異は029が持つ) |
...
| `CAND-NODEBOOT-027` | sealed build receiptとNode parity receiptが**存在するが束縛tupleが不一致**の状態で`build` scriptを削除 — 独立4case: (a) stale (旧`subject_revision`のreceiptを流用)、(b) wrong-revision (2 receiptが別revision)、(c) wrong-generation (同revision別`generation_id`)、(d) wrong-artifact (同generation別`artifact_digest`) | 4caseとも撤去をfail-close (PLAN-L6-93 §5.4)。**存在確認だけのoracleは4case全てを通してしまう**ことを、各caseを個別に落とすことで固定する |
...
> 主張しない。診断は advisory であり、CIのhard gateを変更しない。実装PRはtest citationと同じ
> commitで `CANDIDATE-*` を `U-*` へ昇格し、未実装の確定oracleを作らない。
...
| `U-WTTOPO-013` | PF4 / `PLAN-L7-478` | before/afterでhealthy件数は同じだがworktree Aが壊れ別worktree Cが追加 | normalized path/admin/HEAD identity集合digest不一致で移設acceptance拒否。findings 0かつ許可remap後の集合一致だけを受理する。`tests/worktree-topology-migration.test.ts` |
...
| `U-WTTOPO-015` | PF3 / `PLAN-L7-477` | empty factsとadvisory finding factsをdoctor consumerへ個別入力 | emptyは出力なし。findingは表示するがhard-gate/CI成功判定を変更しない。`tests/worktree-topology-doctor.test.ts` (`npm exec -- vitest --config vitest.pf3.config.ts run tests/worktree-topology-doctor.test.ts --reporter=dot`) |
...
digest/ref を入力へ戻してはならない。下表の Candidate/U 対応は同番号で固定し、実装時に
軸を削除したり別の番号へ移し替えたりしない。入力面を持たない軸は、対応する U oracle
で「生成値のみを受理するため caller mutation が存在しない」ことを明示する。

| Candidate ID (freeze) | 正規 U ID (実装) | 対象 / DbC oracle |
|---|---|---|
| CANDIDATE-D3B-001 | U-D3B-001 | valid exact D3a attempt + provider evidence。D3a request 由来の`author_family`と反対側へ導出した`reviewer_family`を含む canonical JCS payload から1 artifact/`d3b:<judgment_digest>`を生成し、同一入力のreplayは同一digestへ収束 |
| CANDIDATE-D3B-002 | U-D3B-002 | subject / family binding。repository/PR/head/request memory/digest/revision/attempt/nonce、author family、派生 reviewer family、実 spawn provider familyを各1軸変異する。same-familyは`same_family_reviewer`、その他は`identity_mismatch`で拒否し、artifact write 0 |
| CANDIDATE-D3B-003 | U-D3B-003 | provider evidence binding。evidence bytesを単独変異し、生成される`evidence_digest`とjudgment digestが変わることを検査。provider-evidence refのcaller差替えは入力schemaに存在しないため、CANDIDATE-D3B-007 / U-D3B-007のcaller-authority拒否で同じ軸を保持する |
...
## PLAN-L7-506 merged-plan-status landing guidance oracle (Issue #390)
...
| ID | fixture / mutation | expected |
|---|---|---|
| `U-MPSTATUS-506-001` | `phase=landing` の未 confirm PLAN violation | `(A) 分割` と `(B) 単一の実装 PR`、preflight `review_evidence` による implementation PR 前の confirm、exact PR HEAD の非著者 closing review PASS + canonical receipt、PR comment、`tests_green_at <= reviewed_at`、全 `green_commands` fields を案内し、receipt の PLAN への pre-merge 書き戻しを要求しない |
| `U-MPSTATUS-506-002` | `phase=merged` の既存 violation | legacy message と完全一致し、landing guidance を含まない |
| `U-MPSTATUS-506-003` | `ok=true` | 既存 OK message と完全一致 |
