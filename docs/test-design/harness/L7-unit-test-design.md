...
| `U-WTTOPO-011` | link 断で観測不能な worktree (collector が `dirty=false` / `mergedIntoMain=false` へ倒した既定値) | `retirable` へ入れない。分類 (counts) には残すが廃棄候補としては提示しない (未コミット作業の誤廃棄を防ぐ fail-safe) |

実行対応: `tests/worktree-topology.test.ts` (`U-WTTOPO-001`〜`011`)。設計元 =
