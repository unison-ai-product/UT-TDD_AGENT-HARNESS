...
| U-SETUP-013 / AT-DIST-001 | `tests/distribution-acceptance.test.ts` | Local clean distribution acceptance smoke は planned clean artifacts を temp repo に materialize する。移行中の fixture では install/status/setup/typecheck に Bun を使う (`bun.lock` と sealed snapshot runner 用 `package-lock.json` の両方を検査し、Bun の install/status/setup/typecheck 経路を実行する) が、distribution plan の実行側は `process.execPath` で Node を直接起動する。Pack `v0.1.0` tag / GitHub Release 後も、この local smoke は署名 tarball publish / UAT / post-release telemetry を実行しない。source repo 用 full `doctor` は dogfood PLAN/design/test-design/runtime artifact を除外する clean distribution の受け入れ条件には含めず、consumer doctor profile が必要なら別 PLAN とする。Issue #405 の再発条件は、clean Pack から `package-lock.json` を除去すると `bun run test:pack` が Vitest 開始前の `npm ci` で fail-close すること。 |
