...
| `U-TESTHYGIENE-043` | `global-setup-fence.test.ts` | teardown fence process | sealed detached HEAD snapshot改変後のglobal teardownはsnapshot runner子processを非0で終了し`test workspace fence violation`を出す |
