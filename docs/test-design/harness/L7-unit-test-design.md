...
| `U-RELMAN-010` | S3 promotion / rollback / `PLAN-L7-494` / #363 | valid manifest deltaに対しD2 `ok` / `state` / `reasons`を各1軸で非ready化し、rollbackではD2自体を欠落 | promotion/rollbackを拒否し、実PF5 compositionのsnapshot/staging/apply/restore/pointer/publish 0、prior state不変 |
...
| `U-RELMAN-020` | S3 promotion / `PLAN-L7-494` / #363 | manifest、PF4、PF5、CI、QA、D1/D2/facts/claim/specを各1軸で変異する。D1/D2/factsのPR、authorized entryのmemory/revision/family、receiptのplanId/author/reviewer family、laneを別subjectへspliceし、expected receipt digestだけは変異後へ合わせる。shape+identity、identity+missing、CI+QA missingの複合故障も注入する。`observedAt`単独の経過時間は変異対象にしない | source-aligned anchorで全spliceを`identity_mismatch`拒否し、reason precedenceを`invalid_input`→`identity_mismatch`→`ci_missing`へ固定。non-attested attestationも存在identityを先に照合し、別subjectをavailabilityへ丸めない。実PF5 compositionの全port 0、prior state不変 |
| `U-RELMAN-021` | S3 rollback / `PLAN-L7-494` / #363 | 直前attested rollback候補を0件、2件、attestation欠落、non-attested別identity、artifact可用性欠落、runtime `[null]`へ各1点変異 | throwせずtyped deny。別subjectは`identity_mismatch`、`[null]`は`invalid_input`。現行pointer・artifact bytes不変、実PF5 compositionの全port 0 |
