...
| `U-WTTOPO-007` | findingなしのclean merged、保持refから到達可能なclean detached、activeが混在 | mergedと到達可能detachedだけを`retirable`へ、activeは除外 |
| `U-WTTOPO-008` | findingなしのmain root worktree | `healthy`/identity集合/`mainCount=1`へ含め、liveness/`retirable`から除外 |
...
| `U-WTTOPO-012` | clean detached HEADがmain/保持対象refのどこからも到達不能、または到達可能性を観測不能 | review-requiredとして`retirable`から除外 |
| `U-WTTOPO-013` | before/afterでhealthy件数は同じだがworktree Aが壊れ別worktree Cが追加 | normalized path/admin/HEAD identity集合digest不一致で移設acceptance拒否 |
...
| `U-WTTOPO-016` | 同じWindows pathをdrive小文字・backslash・reparse経由で入力し、別fixtureで末尾separatorとcase-only別名を入力 | realpath後drive uppercase + `/` + root以外末尾除去へ収束。path全体はcase-foldせず別identityを誤併合しない |
...
| `U-WTTOPO-018` | identity入力順反転、合法remap、重複from、変換後collision、root外escapeを各mutation | `topology-v1` framed SHA-256は合法入力で同一。ambiguous/unsafe remapはacceptance比較前に拒否 |
