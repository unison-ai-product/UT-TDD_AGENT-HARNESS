...
| `U-DOCLEDGER-005` | 全4 kind正常replay、add(existing)、modify/delete/rename missing source、stale before、rename target占有/same path、sequence gap/duplicate、chain改竄、final path/blob不一致 | `replayDocumentDeltas` + final closure | 正常列だけeffective集合一致。違反はreason codeとpath/blob identity別の`doc-delta-unregistered`を全件stable順で返しexit 1。renameをGit heuristicで推測せず、明示renameなしはdelete+addとして報告。連続modify、add→modify/delete、rename→modify/delete、入力非変更、反復digest一致も固定 |
