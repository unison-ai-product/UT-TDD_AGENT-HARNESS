...
| `U-GHPROJ-039〜044` | revision / Project outbox payload custody | 正規5項目、未知field、空値、不正Project番号、同一digest再送、applied後のpayload変更、旧完了通知、同期失敗、legacy PLAN review証跡追加、PLAN不在工程行 | `project-item-upsert` payloadだけを固定順canonical JSONとSHA-256 digestで同時保存する。未知fieldや不正値はwrite 0。同一digestだけapplied/attemptを保持し、変更digestは新identityのpendingへ戻り旧完了通知ではappliedにならない。同期失敗はclosed reasonとattemptを記録しretry可能なpendingを維持する。legacy revisionはreview証跡を除外したcanonical tokenで安定し、文書全体hashと分離する。対応PLAN不在では工程表hashをrevisionに流用せずwrite 0 |
