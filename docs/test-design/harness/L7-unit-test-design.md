...
| `U-GHPROJ-039` | Project outbox payload custody | 正規5項目、未知field、空値、不正Project番号 | `project-item-upsert` payloadだけを固定順canonical JSONとSHA-256 digestで同時保存する。未知fieldや不正値はwrite 0で拒否し、secret/transcriptを汎用payloadとして保存できない |
