...
| `U-GHPROJ-019〜025` | Project V2 reconcile | custom field key mangling、dry-run、create、同値再送、重複item、field欠損、binding保存、同期不整合 | mojibakeを含むprovider keyを正規fieldへ束縛する。dry-runはremote/DB mutation 0のままapplyと同じ目標field値を示し、同値はno-op、重複/契約driftと不整合applyはfail-close、item identityをdurable保存 |
