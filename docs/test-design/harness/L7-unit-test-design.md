...
| `CANDIDATE-RELMAN-011` | PF-2 materializer / #248 / `PLAN-L7-486` | 同じsource entry集合の2呼出し、入力順反転、`docs/skills→skills` remap、artifact空間からのworkflow source逆引き、`package.json` transform、destination path/mode/content、symlink target/`120000`、UTF-8 byte sort、uint32be/uint64be framing、destination衝突、invalid plan/missing source/空artifact set、path形式、unsupported mode、root外symlink、control manifest、version token (`"1"` / number `1` / `"v1"` / trim付き / 未知)を各1件mutation | workflowのcontent/modeはtemplate source entry由来。文字列`"1"`のversion 1 materializer後のdestination path/mode/contentから同一digest。source blobだけ同じでPack outputが違えばmismatch。plan/source/path/mode/symlink不正はtyped invalid、非`"1"`tokenは`unavailable`でcoerce/trim/fallbackしない。入力は不変、control manifestは出力にもdigestにも含めない |
