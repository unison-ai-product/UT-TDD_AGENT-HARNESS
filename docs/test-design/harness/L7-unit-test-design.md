...
| `U-GHPROJ-039〜047` | revision / Project outbox payload custody | 正規5項目、未知field、空値、不正Project番号、同一digest再送、applied後のpayload変更、旧完了/失敗通知、同期失敗、repository越境、scoped derive後のglobal projection、legacy PLAN review証跡追加、PLAN不在工程行 | `project-item-upsert` payloadだけを固定順canonical JSONとSHA-256 digestで同時保存する。未知fieldや不正値はwrite 0。同一digestだけapplied/attemptを保持し、変更digestは新identityのpendingへ戻り旧完了通知ではappliedにならない。同期失敗はclosed reasonとattemptを記録しretry可能なpendingを維持するが、applied済みintentを再openしない。外部同期readinessはrepository境界を越えず、scoped deriveはPK=`plan_id`のglobal readiness projectionを削除・置換しない。legacy revisionはreview証跡を除外したcanonical tokenで安定し、文書全体hashと分離する。対応PLAN不在では工程表hashをrevisionに流用せずwrite 0 |
| `U-GHBIND-001〜015` | repository facts同期 / typed closure receipt | typed PR trace、branch/check/review/merge/Issue、trace欠損、revision欠損、stale HEAD、main以外へのmerge、無関係green check、同一・未知provider review、片lane欠落、旧HEAD lane receipt、PLAN sourceなしの直接DB row注入、PLAN projection/source不在、legacy source/token一致、typed trace外PLAN文字列、provider list truncation、途中write競合、Project item欠損、transaction中network、手動check/review注入、古い観測の再送、source全文hashとPLAN revisionの混同 | Issue番号を含む完全traceとmain向けPR、PR/main双方のrequired `harness-check` ID、canonical PLAN frontmatterにexactly once存在し同一revision/HEADへ固定された既知の異provider claim-blind/spec-blind両receiptの結合digestだけがclosure receiptへ収束する。revisionはadmission bindingまたはlegacy canonical tokenから解決し、GitHub approvalは正本にせずUT-TDD receiptを使う。PLAN projection/source不在、Issue省略、main以外、未知model、不完全・曖昧・DB単独の履歴、truncated snapshotは推測せずwrite-zeroとし、legacyはcanonical sourceとtokenが一致する場合だけbindingを書く。Project item欠損ではclosureを拒否する。全provider観測後の単一transactionで書き、競合時は全rollbackし、manual observeはcheck/review証拠を生成できない。古い観測のno-opは新規`bindingIds`として報告しない |
