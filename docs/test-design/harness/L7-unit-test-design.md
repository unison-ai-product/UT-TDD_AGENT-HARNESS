...
| `U-GHBIND-001〜012` | repository facts同期 / typed closure receipt | typed PR trace、branch/check/review/merge/Issue、trace欠損、revision欠損、stale HEAD、main以外へのmerge、無関係green check、同一・未知provider review、片lane欠落、旧HEAD lane receipt、PLAN sourceなしの直接DB row注入、typed trace外PLAN文字列、provider list truncation、途中write競合、Project item欠損、transaction中network、手動check/review注入、古い観測の再送、source全文hashとPLAN revisionの混同 | Issue番号を含む完全traceとmain向けPR、PR/main双方のrequired `harness-check` ID、canonical PLAN frontmatterにexactly once存在し同一revision/HEADへ固定された既知の異provider claim-blind/spec-blind両receiptの結合digestだけがclosure receiptへ収束する。revisionはadmission bindingから解決し、GitHub approvalは正本にせずUT-TDD receiptを使う。Issue省略、main以外、未知model、不完全・曖昧・DB単独の履歴、truncated snapshotは推測せずskipし、Project item欠損ではclosureを拒否する。全provider観測後の単一transactionで書き、競合時は全rollbackし、manual observeはcheck/review証拠を生成できない。古い観測のno-opは新規`bindingIds`として報告しない |
