...
| `U-GHPROJ-039〜051` | revision / Project outbox / admission custody | 正規payload、同一/変更digest、applying中enqueue、stale完了、同期失敗、repository越境、global projection、legacy PLAN、PLAN不在、partial/未追跡admission receipt | payload validation後にdispatch対象全件をatomic claimする。Project projection/binding保存とapplied化はcurrent outbox identityを再照合した単一transactionであり、stale workerはwrite 0。失敗はclaim済みintentだけretry可能に戻す。legacyはcanonical tokenで安定する。partial receipt、tracked hash chain不在、receipt/content digest不一致はschedule/readiness/outboxを全てwrite-zeroとし、`source_hash`をrevisionへ流用しない |
