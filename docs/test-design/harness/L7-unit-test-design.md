...
| U-MCPPROFILE-002 | disabled-by-default policy | every external or MCP profile has `defaultEnabled=false`; built-in Node/doctor profiles remain enabled. |
...
| `U-TESTHYGIENE-047` | `vitest-snapshot-runner.test.ts` | Node runtime resolution | sealed Node generation の absolute executable を snapshot install/rebuild/Vitest に使い、Bun/bunx/tsx/TS 直実行へ fallback しない |
