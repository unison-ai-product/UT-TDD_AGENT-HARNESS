...
| `U-PA-SEAL-016` | Issue authority port欠測、live read failure、wrong Issue number、body digest drift、PLAN frontmatterのissue number / episode mismatch。issue number mismatch は command 側だけを 102→103 に変異し、fake live authority は command に追従させる。 | sealed-lineage migrationを実行する。 | 各入力を`seal-issue-authority-invalid`で拒否し、全 table write 0。frontmatter照合が先に発火し、live authority の自己申告で迂回できない。 | tracked PLAN blobのadmission receiptとlive GitHub Issue observationを結ぶpreflight port。 | episode ID推論、手書きbody digest、read failure時の自己申告fallback、frontmatterとのissue number不一致を許さない。 |
...
| `U-PA-SEAL-021` | 最初のGit preflight後、review authorityまたはIssue authorityの観測中にHEADを移動するadversarial port。 | migrationのtransaction直前HEAD検証を実行する。 | `seal-source-head-toctou`で拒否し、全8 table write 0。 | Git HEAD custodyの最終消費境界。 | authority I/O中に失効したsource HEADでsealしない。 |
| ↳ tracked remote到達性 | authority観測後、source commitをtracked remoteから到達不能にするか到達性検証を失敗させるadversarial port。 | migrationのtransaction直前reachability検証を実行する。 | `seal-source-commit-unreachable`で拒否し、全8 table write 0。 | tracked remote custodyの最終消費境界。 | HEAD一致だけで到達不能なsource commitをsealしない。 |
