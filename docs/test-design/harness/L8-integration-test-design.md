...
### §1.5 IT-ASSET (内部資産 roster 由来、PLAN-L5-05 / PLAN-X-03 Discovery confirmed)
| IT-ID (候補) | 検証対象 | シナリオ |
|---|---|---|
| IT-ASSET-01 | `roster list` scan→registry (module-decomp §1/§5) | `.claude/agents/*.md` 全件が registry (id=filename stem) に入る (PLAN-X-03 spike = 19 件実証) / capability class ⊥ model family に決定論解決 |
