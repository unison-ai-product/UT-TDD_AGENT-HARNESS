...
| `CAND-CUTOVER-107` | claim/spec片lane、mode別same provider/model/runtime/session/identity/author、attestation shape mutation | hybridはprovider/runtime差、codex/claude-onlyはmodel差を必須化。standaloneはdistinct human 2名だけ。nested schemaVersion/algorithm欠落、flat producer/recordDigestを拒否 |
| `CAND-CUTOVER-108` | validated Q0 SliceAdmission+L6Confirmation→genesis→sealedへfresh review bundle+CutoverAdmission+kind-discriminated evidenceをnested保存 | PLAN-L6-93 exact confirmed/content/head/attestationとCutoverAdmission direct refをchain-only再検証。draft/wrong/stale/unsigned、未定義root/wrapper/alias拒否 |
