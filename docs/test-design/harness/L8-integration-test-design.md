...
| `IT-DOCLEDGER-01` | baseline `3d232e9c`のGit tree object、raw NUL path/blob OID stream、空projection | snapshot capture→init→materialize→projection rebuildを実行する | snapshot/ledger/projectionが同じidentity集合・digestを持ち、全path exactly once、phantom/duplicate/case-fold collision 0 | raw NUL hash、tree OID、snapshot digest、921件基準receipt、row diff 0 |
| `IT-DOCLEDGER-02` | baseline後のadd/delete/renameと未更新ledger | final closureを実行する | 各deltaを別identityで拒否し、renameをdelete/addの片側だけで閉じない | delta finding manifest、期待exit 1 |
