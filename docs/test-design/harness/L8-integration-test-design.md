...
| `IT-RGK-PHYS-015` | bundle内key、unknown authority、key substitution | 外部trust registryとのbinding不一致でcontrol process 0 |
| `IT-RGK-PHYS-016` | key rotation/revocation/expiry境界とclock fault | overlap内だけ受理し、失効・期限外・欠測はfail-close |
| `IT-RGK-PHYS-017` | algorithm downgradeと古い正規署名bundle | allowlist/floorで拒否しcurrent bundle不変 |
| `IT-RGK-PHYS-018` | activation commitとfloor writeの各barrier crash | 旧bundle+旧floorまたは新bundle+新floorだけを観測 |
