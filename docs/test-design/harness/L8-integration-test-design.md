...
| `U-PA-SEAL-021` | 最初のGit preflight後、review authorityまたはIssue authorityの観測中にHEADを移動するadversarial port。 | migrationとdry-runの最終authority検証を実行する。 | transaction開始直前のHEAD一致・tracked remote到達性再検証で`seal-source-head-toctou`、全8 table write 0。 | Git custody tokenの最終消費境界。 | authority I/O中に失効したsource HEADでsealしない。 |

実行対応: `tests/plan-asset/sealed-lineage-local-migration.test.ts`
(`U-PA-SEAL-011..021`)。これらは `src/lint/oracle-test-citation-baseline.ts` への追加対象ではなく、
