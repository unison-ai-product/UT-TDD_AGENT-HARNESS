...
| IT-AUTHORING-GROUP-01 | design/PLAN/test-designの3 tracked成果物と空のcommand-group journal | N-artifact publisherを実行しDBをclose/reopenする。 | header/member/phase chainと各content/receipt digestが一致し、全member後だけcommitted。 | authoring service -> ledger SQLite -> filesystem publisher boundary。 | DBから本文を創作せず、group/member keyは再開時も不変。 |
| IT-AUTHORING-GROUP-02 | 各member publish前後とphase append前後のfault fixture | fault後に同commandを再実行する。 | 記録済memberはskip、未記録memberは同じidempotency keyで再送され、異payloadは拒否。 | fault port -> publisher -> durable phase journal boundary。 | partial publishを完了扱いせず、recovery_requiredを監査可能に残す。 |
