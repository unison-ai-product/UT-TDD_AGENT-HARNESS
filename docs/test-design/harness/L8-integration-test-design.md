...
> **status**: draft (pair skeleton — L5↔L8 の対応候補を物質化。IT-* 詳細は L8 本起票で展開、L3↔L12 / L4↔L9 と同型)
> **granularity correction (2026-06-08)**: 本書は L5 契約と IT-* 候補の coverage map / pair skeleton であり、結合テスト設計として confirmed できる粒度には未達。confirmed 条件は、各 IT-* に Given/When/Then、fixture、module boundary setup、mock/adapter 条件、期待 assertion、negative/edge matrix が存在すること。
...
- **孤児 (契約で IT 候補未対応) = 0** を L8 本起票で機械確認。現時点では candidate mapping であり、詳細 case 設計の orphan=0 ではない。
...
- With these additions, L5 internal asset contracts have candidate L8 rows. Detailed L8 case design remains open until each row receives Given/When/Then, fixtures, boundary setup, assertions, and negative/edge coverage; algorithmic detail remains a documented L6 carry, and implementation materialization remains a documented L7 carry.
