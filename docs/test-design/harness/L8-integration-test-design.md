...
| `IT-DOCLEDGER-02` | baseline後のadd/modify/delete/rename、正常event chain、未更新/改竄ledger | reducer→final closureを実行する | 正常4 kindはfinal path/blob集合と一致。未登録、illegal遷移、sequence gap/duplicate、chain改竄を別identityで拒否し、明示renameなしをdelete/addの片側だけで閉じない | delta chain/reduction digest、finding manifest、期待exit |
