...
| `CANDIDATE-U-D3BCOMP-012` | `UT_TDD_CUSTODY_JUDGMENT_ARTIFACT` を (a) payload 内容 1 byte 改変、(b) 空白 / key 順序 / 末尾改行だけ改変 (JSON 同値)、(c) basename を別名にする | (a)(c) JCS 再計算 digest ≠ basename の digest → `judgment_digest_mismatch`。(b) canonical bytes 不一致 → `judgment_artifact_noncanonical`。(d) payload に `judgment_digest` field を挿入した artifact → `judgment_artifact_noncanonical` (producer は派生値を artifact bytes に含めない、`PLAN-L7-562` U-D3B-006)。draft 0。改変前は draft の `judgmentDigest` / `providerEvidenceRef` が再計算値・basename と一致。file bytes の sha256 を digest にする実装、JSON 同値なら通す実装、payload 内の `judgment_digest` field を要求・参照する実装は Red |
