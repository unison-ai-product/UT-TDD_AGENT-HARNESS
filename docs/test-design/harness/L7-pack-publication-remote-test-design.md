...
| `CANDIDATE-PACKPUB-003-M1` | canary pointer objectまたはbefore control-manifest snapshotのdrift、before-state CAS precondition drift | `mismatch`/typed deny、**remote write 0**、pointer after-snapshot append 0 |
...
実装PRが `CANDIDATE-PACKPUB-003-A..R` を1対1の `U-PACKPUB-REMOTE-*` へ昇格し、各
