...
含み、before snapshotを上書きせずappendする。release Pack commit/tree と pointer append後の
Pack commit/tree は別々の一意な identityとして明示する。fixture生成時に source worktree、directory walk、Pack checkout、開発DB、PLAN本文、
環境変数から entry を補完しない。
...
| `PackRepositoryPort` | protected main/before snapshot観測、branch commit、PR作成、PR merge CAS/観測 | main SHA、branch/PR、release/pointer Pack commit/tree、merge state |
...
| `CanaryPointerPort` | after snapshot生成、protected main PR/CAS append、read-back | pointer object、before/after snapshot digest、pointer Pack commit/tree、release ID |
...
```text
planned / sealed re-attestation + before snapshot observe + operation approval/nonce consume
...
| `CANDIDATE-PACKPUB-003-M` | release_visible後のReleaseが未可視・別identity、またはafter snapshotのpointer object/ before-state/Pack commit/tree read-back不一致 | `mismatch`/`indeterminate`、canary append/公開 0 |
...
6. receipt と durable execution state は sealed intent、遷移ごとの approval/nonce、idempotency key、
   before/after identity、operation順序へ束縛される。
...
9. supersede-forward rollbackは別intentの参照境界として記録し、この sliceで既存objectを破壊しない。
...
実装PRが `CANDIDATE-PACKPUB-003-A..Q` を1対1の `U-PACKPUB-REMOTE-*` へ昇格し、各
