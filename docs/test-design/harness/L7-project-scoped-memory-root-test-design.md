...
- Red→Green: 期待値をhook引数から注入しない旧compositionではv4を正当にconsumeできず、
  ...
  entryをtyped fail-closeできる。
- exact targeted snapshot: 3 files / 9 passed / 37 skipped。
  `CANDIDATE-U-PMEMROOT-007`（Memoryおよびreview composition）、`U-RVATT-025`、
  `U-MEMTERM-001`、`U-MEMTERM-003`、`U-MEMWAKE-001補遺`、`U-MEMWAKE-007`を選択。
...
- `npm exec -- tsc --noEmit`、対象4ファイルのBiome checkもGreen。legacy root migration/quarantine、
  Pack parity、R2以降は未完了であり、この証跡では主張しない。Opus non-author closing reviewは保留。
