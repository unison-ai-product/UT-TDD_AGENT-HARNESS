...
| 正規 ID | 実測 | 所有テスト |
|---|---|---|
| 参照: `U-PACKBUN-003` | setup が生成した consumer tree 全体を再帰走査し Bun 到達 0 件 | `tests/setup-bun-removal.test.ts` |
| 参照: `U-PACKBUN-004` | 生成経路5軸の単独復活が期待 finding 集合と完全一致して Red | `tests/setup-bun-removal.test.ts` |
| 参照: `U-PACKBUN-006` | BAN lint の凍結サンプルを rule 単位で fail-close、source build を保持 | `tests/ban-lint-detection-power.test.ts` |
