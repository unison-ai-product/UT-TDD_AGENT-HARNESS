...
`b17a8ec7`、workerは `gpt-5.6-luna` である。
...
| team | `.ut-tdd/teams/example-review-team.yaml` | `docs/templates/team/example-review-team.yaml` | 1 |
...
| `CANDIDATE-PACKTPL-002` | `distribution sync-stage --json` 相当の materializerを実行 | source→artifact projectionが表と一致し、UTF-8/LF、mode、content bytesが保持され、`.ut-tdd` source pathを出力しない |
...
| `CANDIDATE-PACKTPL-006` | `docs/templates/**` 全体許可、または `.ut-tdd/**` 直接許可へ一軸変更 | source-only corpus、runtime state、`.ut-tdd/teams` source、unmanaged templateがartifact setへ入るため deny findingで Red |
...
| runtime state | `.ut-tdd/**`（team sourceを含む） | source pathは0件、許可されるのは `docs/templates/team/example-review-team.yaml` の一件 |
...
projected team YAMLは source `.ut-tdd/teams/example-review-team.yaml` を直接配布せず、
`docs/templates/team/example-review-team.yaml` として parseする。`name`、`strategy`、serialization、
member role/engine/task/依存順序が現行 `teamDefinitionSchema` と role/engine registryに適合し、
source `.ut-tdd` pathへの fallbackを要求しないことを oracleにする。
