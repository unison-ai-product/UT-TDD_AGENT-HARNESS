...
## U-RVATT-040 independent mutation matrix

| Candidate | Stimulus / mutation | 独立oracle |
| --- | --- | --- |
| `CANDIDATE-U-RVATT-040-A` | non-zero attemptで `attempt_execution_failed` appendを削除 | audit現物が0件となりRed。project関数の戻り値はoracleに使わない |
| `CANDIDATE-U-RVATT-040-B` | 既存canonical receiptを置いた後、成功retryのwriteをcreate-exclusiveからoverwriteへ変更 | 既存bytes/digest不変とtyped conflictを直接検査しRed。audit検査に依存しない |
| `CANDIDATE-U-RVATT-040-C` | 成功retry後に旧attempt fileまたはfailed/superseded eventを削除 | 各path/event identityの残存検査がRed。canonical receipt件数だけでは通らない |
| `CANDIDATE-U-RVATT-040-D` | failed attemptのoutcome appendを失敗/欠落させたまま次attemptを開始可能にする | `attempt_outcome_indeterminate` と新attempt write 0の検査がRed。成功receipt生成には到達させない |
...
5. attempt-1/2 files、failed outcome、superseded eventが残り、canonical receiptが成功内容から不変であることを再読する。
