...
| `U-PACKBUN-002` | readiness checksを取得し、supported / below-range / above-range Node fixtureを個別評価 | Bun checkと導入案内は0。Node rangeとGitのtyped checkが存在し、range guard削除またはBun文言復活の各単軸変異でRed |
