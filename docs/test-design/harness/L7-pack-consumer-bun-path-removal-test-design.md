...
S1-bの`U-PACKBUN-003` / `004` / `006`、S1-cの`U-PACKBUN-005`、Issue #504の`U-PACKBUN-008`は正規IDへ昇格済みであり、未実装候補は
`CANDIDATE-U-PACKBUN-001` / `002`である。`U-PACKBUN-007`はIssue #500のRed→Green実測と同じ実装revisionで正規昇格する。その他の未実装oracleを正規IDにせず、各sliceの実装PRが
Red testと同時に昇格する。
...
| `U-PACKBUN-005` | `.github/workflows/harness-check.yml` から `oven-sh/setup-bun@v2` を除去した状態で Pack/consumer acceptance fixture を実行する | fixture が Green。Bun の install / download / invocation trace が 0。実装 oracle は `tests/github-ci-policy.test.ts` の実 workflow parse (`jobs.harness-check-linux/windows` の各 step の `uses`/`run`/`name` に `oven-sh/setup-bun@` / bun・bunx 実行子が 0 件であること) であり、実 GitHub Actions 上の fixture 全量実行そのものは対象外 (構造検査による代理測定)。tests 側の実 `bun` spawn (`tests/distribution-acceptance.test.ts` / `tests/setup.test.ts` U-SETUP-009b 系) は Issue #506 / PR #508 で退役済み (実 spawn 0 件、allowlist entry も削除済み)。残存する `BUN_SPAWN_DEBT_ALLOWLIST` は `src/cli/distribution.ts` の 2 件のみで、`PLAN-L7-462` の別 exit criteria (Issue #134) に帰属する |
...
| `U-PACKBUN-008` | Issue #504: `forbidden_bun_execution` の false-negative 2 類型を Pack workflow への単独 step 追加 mutation として検査する。EVA-1 = `npx bun@<version>` 系 (`npx bun@1.3 run build` / `npx bun@latest install` / `npx --yes bun@1.2.4 --version`)、EVA-4 = installer pipe (`curl …bun.sh/install | bash` / `curl …install.sh | sh` / `wget …| bash`)。negative control 3 件 (`npm run bundle` / `cabundle` / pipe 無しの bun.sh 言及) を併置する | 各 EVA mutation は `forbidden_bun_execution` violation で Red (旧 matcher では 6 件とも素通り = Red-first 実測、PR #510)。negative control は violation 0 件。required step は全ケースで健全のまま |
...
| S1-c (source CI) | #472 | 参照: `U-PACKBUN-005` |
