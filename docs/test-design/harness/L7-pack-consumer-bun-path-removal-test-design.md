...
| `U-PACKBUN-005` | `.github/workflows/harness-check.yml` から `oven-sh/setup-bun@v2` を除去した状態で Pack/consumer acceptance fixture を実行する | fixture が Green。Bun の install / download / invocation trace が 0。実装 oracle は `tests/github-ci-policy.test.ts` の実 workflow parse (`jobs.harness-check-linux/windows` の各 step の `uses`/`run`/`name` に `oven-sh/setup-bun@` / bun・bunx 実行子が 0 件であること) であり、実 GitHub Actions 上の fixture 全量実行そのものは対象外 (構造検査による代理測定)。`tests/distribution-acceptance.test.ts` / `tests/setup.test.ts` U-SETUP-009b 系が実 `bun` バイナリを spawn する既知 debt (`runtime-portability.ts` の `BUN_SPAWN_DEBT_ALLOWLIST`、2 箇所) は本 oracle の対象外であり、`PLAN-L7-462` の別 exit criteria (Issue #134) に帰属する — 未解消のままである点に注意 (§S1-c 実装限界注記) |
