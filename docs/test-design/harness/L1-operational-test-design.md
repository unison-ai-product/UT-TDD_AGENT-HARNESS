...
| **OT-25** | SCR-11 (doctor 結果ビュー) | SCR-11 doctor 結果ビューが screen sub-doc に定義されており、FR-L1-18 (横断検出) と pair で情報要素 (検出種別 / severity / next_action) / 操作要素 (再実行 / ignore / 詳細展開) / 更新頻度 / 状態種別が記載されていることを確認 | SCR-11 が screen sub-doc に存在 / FR-L1-18 との pair 明記 / 情報・操作要素が記載 |
| **OT-26** | screen 6 遷移シナリオ (Recovery 復帰 + Discovery 追加) | screen sub-doc §遷移 に 6 遷移シナリオ (既存 + Recovery 復帰遷移 + Discovery 追加遷移) が記載されており、SCR-01〜07/11 の画面間接続が遷移図と整合していることを確認 | 遷移シナリオ 6 件が存在 / Recovery / Discovery 遷移が SCR-11 を含む遷移図に存在 |
...
> **新規 OT-18〜OT-31 追加 = 計 31 件** (既存 17 + 新規 14 = 31 件確定、2026-05-28 Step 1 整合更新)
...
- SR-* (SCR-01〜07) → **OT-15** (L2 モック lift で PO 判定。SCR-08 統合後 SCR-01〜07/11 の 7 画面)。
- SCR-11 (doctor 結果ビュー、新規) → **OT-25** (FR-L1-18 pair 確認)。
- 6 遷移シナリオ (Recovery 復帰 + Discovery 追加) → **OT-26** (遷移整合確認)。
- 画面個別の受入観点は L10 UX 磨き + L11 UAT で詳細化。**孤児 SR = 0**。
...
> **件数確定: 既存 OT-01〜17 (17 件) + 新規 OT-18〜31 (14 件) = 計 31 件** (2026-05-28 Step 1 整合更新)
