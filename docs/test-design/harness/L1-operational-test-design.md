...
- 全 FR-L1-* (**46 件** = FR-L1-01〜35 + 37/39/40/41/42/44/45/46/47/48/49) / OT 対応必須 (孤児 = 0。軽量原則で代表 OT へ集約可)。**FR-L1-36/38/43 は上記 46 件から除外** (BR-21 派生の L3 forward carry。L14 時点では OT-18 で宣言確認のみ、FR 詳細化は L3) → G1-trace の孤児誤検知対象外
- 全 SR-* (**14 画面** = PM-01〜05 + HM-01〜08 + GD-01) / OT 対応必須 (孤児 = 0)
...
| **OT-36** | HM-01 (機能一覧: FR-L1 46 件 × implementation_status 全件表示) | HM-01 機能一覧ビューで FR-L1-01〜35/37/39/40/41/42/44/45/46/47/48/49 の 46 件が implementation_status (not-implemented / design-only / implemented / deprecated) 付きで全件表示されることを確認。孤児 FR・status 欠落行が 0 件であることを観測 (not-implemented) | FR-L1 46 件が全件表示 / implementation_status 列が存在 / 孤児・欠落 0 |
...
| **OT-45** | functional §1 対応画面列 (G1-trace、P0 19 件) + screen §5 trace マトリクス + business §1.2 BR 一覧 | G1-trace 業務 ⇔ 画面 双方向 trace 整合を機械検証する。R1: 全 BR/UX/BR-21/BR-22 (13 件) が最低 1 画面紐付き / R2: 全 14 画面が最低 1 BR/UX/FR-L1 紐付き / R3: FR-L1 P0 19 件全件画面紐付き / R4: screen sub-doc `requires` に business + functional 列挙。整合確認は `ut-tdd trace --g1` で実行 (not-implemented) | R1〜R4 全条件 pass / 機械検証 fail 時は該当孤児を表示 + 「screen §5 trace マトリクス を更新してください」next_action 提示 |
...
> **新規 OT-18〜OT-31 (14 件) + OT-32〜OT-44 (13 件) + OT-45〜OT-46 (2 件) 追加 = 計 46 件** (既存 17 + 新規 29 = 46 件確定、2026-05-28 全面再編更新 + G1-trace 拡張 + 2026-06-02 BR-22 fullback)
...
- 詳細な機能要件レベルの受入条件は L3 で FR-* + AC-* として詳細化 (本書は L1 軽量、量閉じ確認のみ)。**孤児 FR-L1 = 0** (FR-L1-01〜35 + 37/39/40/41/42/44/45/46/47/48/49 = 計 46 件、全被覆)。
...
- HM-01〜HM-08 (HM 画面群 8 画面):
  - HM-01 機能一覧 → **OT-36** (FR-L1 46 件 × implementation_status 全件表示)。
...
- G1-trace 業務 ⇔ 画面 双方向 trace 整合 → **OT-45** (FR-L1 P0 19 件全件画面紐付き / BR/UX 13 件全件最低 1 画面 / 14 画面全件最低 1 要求紐付き 機械検証)。
...
> **件数確定: 既存 OT-01〜17 (17 件) + 新規 OT-18〜31 (14 件) + 新規 OT-32〜44 (13 件) + OT-45〜46 (2 件) = 計 46 件** (2026-05-28 全面再編更新: 14 画面 PM/HM/GD 再採番 + 横断原則 OT 追加 + G1-trace 拡張、2026-06-02 BR-22 fullback)
