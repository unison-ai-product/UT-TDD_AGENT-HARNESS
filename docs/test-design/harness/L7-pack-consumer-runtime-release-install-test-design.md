...
| --- | --- | --- |
| `U-PACKRT-001` | producer を tag の source revision で実行する。作業ツリーに未 commit 変更と untracked ファイル (allow prefix 配下を含む) を置いた状態でも実行する。同じ revision を 2 回実行する | 出力は exact 5 asset (`<tag>.tar.gz` / `.tar.gz.sha256` / `.ut-tdd.mjs` / `.consumer-runtime.json` / `.consumer.sha256`)。未 commit 変更・untracked の内容はどの asset にも入らず、2 回の `consumer-runtime.json` と `.consumer.sha256` の bytes がそれぞれ一致する |
...
| `U-PACKRT-003` | producer を絶対パスにユーザー名・空白・日本語を含む作業ディレクトリで実行する。5 asset の raw bytes と `consumer-runtime.json` 内の sealed receipt bytes を走査する。別に、receipt の `node.path` / `npm.cli_path` が user home 配下を指す toolchain で実行する | 前者: 5 asset と receipt bytes のどこにも作業ディレクトリ・user home 配下 path・ユーザー名・環境変数値が現れない (bytes 走査)。receipt は `PLAN-L6-93` の封印 bytes と一致し (無加工)、reviewed toolchain version 等の固定契約値だけが許容される。旧 `manifest.json` を出力しない。後者: fail-close し、asset を 1 件も出力しない |
| `U-PACKRT-004a` | producer の Node generation builder に reviewed 以外の Node/npm 版を注入する | reviewed Node/npm 版の照合で fail-close し、出力先に部分 asset を 1 件も残さない |
| `U-PACKRT-004b` | producer の Node generation builder に生成失敗を注入する | generation failure を伝播して fail-close し、出力先に部分 asset を 1 件も残さない |
| `U-PACKRT-004c` | producer の staged asset 移動に失敗を注入する | move failure を伝播して fail-close し、出力先に部分 asset を 1 件も残さない |
...
| `U-PACKRT-011` | 一時 git repo で artifact commit C1 と、`release/manifest.yaml` だけを追加した release commit C2 を作り、C2 に tag を打って producer を実行する。変異: (a) tag を C1 に打つ (manifest 不在)、(b) manifest の `artifactSourceCommit` を C2 の祖先でない commit (C1 から分岐した別 branch の commit) にする、(b2) C2 を merge commit にし、`artifactSourceCommit` を second parent 側だけにある commit にする (first-parent 祖先でない)、(c) C1..C2 に `release/` 外の file 変更 (`src/` 配下 1 file) を混ぜる、(d) manifest を schema 違反にする | 正常系は exact 5 asset を出力し、`release.source_revision` = `generation.subject_revision` = attestation の `artifactSourceCommit` = C1、`release.tag` = 指定 tag。(a)〜(d) はいずれも出力前に typed error で fail-close し、出力先に asset を 1 件も残さない |
