...
| `CANDIDATE-U-PACKRT-007` | (a) installer として実行するモジュールを改変し、`.consumer.sha256` も改変後の値に書き換えて整合させる (実行中のモジュールが `generation.compiled_esm_digest` と異なる)。(b) receipt の `compiled_cli.sha256` と `generation.compiled_esm_digest` を各 1 軸で食い違わせる。(c) coherent multi-asset forgery: 改変 installer に合わせて `generation.compiled_esm_digest`・receipt (`compiled_cli.sha256`・`receipt_digest`・`generation_id`)・`.consumer.sha256` を全て整合的に再計算する | (a)(b) は自己 digest 照合で deny。(c) は installer 単独では検出できないことを test で固定する (攻撃耐性を主張しない証拠、PLAN §6.1)。同じ test で、(c) の asset の sha256 が producer の出力した digest 記録 (PLAN §5.7) と一致しないことを示し、検出が publish 記録との照合に委ねられることを明示する |
...
| `CANDIDATE-U-PACKRT-009` | 同じ release で installer を 2 回実行する。2 回目の前に `active.json` を 1 byte 変える、bundle を 1 file 消す。別に、install 済みの consumer root を別の path へ複製して複製先で再実行する | 1 回目の後の再実行は、保存済み `consumer-receipt.json` と導出値の照合を経て新 write 0 で成功 (committed)。改変後の再実行は既存 `PLAN-L7-516` の reconcile 規則どおり indeterminate / deny とし、黙って上書きしない。複製先での再実行は typed deny `consumer_runtime_receipt_mismatch` で、保存済み receipt を組み直さない (再導出値で receipt を作ると照合が恒真になることへの mutation を含める) |
