...
| `U-PACKRT-004` | producer の Node generation builder に reviewed 以外の Node/npm 版を注入する。独立に generation failure と staged asset 移動 failure も注入する | 各軸で fail-close し、出力先に部分 asset を 1 件も残さない。Node/npm 版は reviewed 版照合、generation failure は failure 伝播、move failure は move failure 伝播でそれぞれ検査する |
