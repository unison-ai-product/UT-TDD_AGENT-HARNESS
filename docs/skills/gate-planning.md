...
applies_to:
  layers: [L1, L2]
  drive_models: [Forward, Discovery, Scrum]
upstream: vendor/helix-source/skills/workflow/gate-planning
---
...
UT-TDD の G0.5 通過条件に準拠し、以下を確認する。
...
出力先候補:
- `docs/design/D-TECH-STACK.md`
- `docs/adr/ADR-TECH-STACK.md`
...
運用メモ:
- `docs/skills/tech-selection.md` の評価マトリクスを利用
...
主連携:
- `docs/skills/tech-selection.md`
- `docs/skills/design-doc.md`

PoC 連携:
- UT-TDD Scrum ワークフロー（`ut-tdd status` / `.ut-tdd/` 配下 state）が導入済みの場合は同フローを優先
- 未導入環境では本スキルの PoC テンプレートを正本として運用

補助連携:
- `docs/skills/api-contract.md`（契約観点）
...
- `docs/design/` または `docs/adr/` に D-TECH-STACK が存在
- 企画項目カバー率 100%
- G1.5 の PoC 結果報告書が存在
