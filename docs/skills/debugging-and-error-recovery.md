...
upstream: vendor/helix-source/skills/agent-skills/debugging-and-error-recovery
---
...
|   +-- Compare Node/bun versions, OS, environment variables
...
bun test --testPathPattern="specific-file"
```
...
In the UT-TDD Recovery drive model, this skill provides the systematic triage for `troubleshoot` kind plans. A reproduction test (Step 5) becomes a V-model artifact: it maps to a test design entry at the unit test layer and its corresponding test code. When a bug is confirmed, a `fix` kind PLAN is created. The recovery evidence (repro test, root cause, fix, regression guard) is logged in `.ut-tdd/audit/`.
