...
applies_to:
  layers: [L1, L2, L3, L4]
  drive_models: [Reverse, Refactor, Retrofit]
upstream: vendor/helix-source/skills/workflow/reverse-analysis
---

# Reverse Analysis Router

UT-TDD Reverse は「既存コードという観測事実」から設計と意図を逆方向に復元し、差分を Forward UT-TDD に安全に接続するための流れである。本スキルは全体像と入口を提供し、各フェーズの実行詳細は専用スキルへ委譲する。
...
flowchart TD
  ...
  RG3 --> R4["R4 Gap & Routing"] --> FWD["Forward UT-TDD (L1-L4)"]
...
| フェーズ | 担当スキル |
|---|---|
| R0 Evidence Acquisition | reverse-r0 |
| R1 Observed Contracts | reverse-r1 |
| R2 As-Is Design | reverse-r2 |
| R3 Intent Hypotheses | reverse-r3 |
| R4 Gap & Routing | reverse-r4 |
| RGC Gap Closure | reverse-rgc |
...
既存の Reverse UT-TDD の標準経路。コード、DB、設定、運用実態から観測契約を抽出し、R1 で契約、R2 で As-Is 設計、R3 で意図仮説、R4 で Forward 接続を行う。
...
- upgrade の RGC skip: gap closure は upgrade 単独では閉じず、Forward 側の完了として評価するため。
