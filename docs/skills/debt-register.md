...
applies_to:
  layers: [L4]
  drive_models: [Forward, Add-feature, Refactor, Retrofit, Recovery]
upstream: vendor/helix-source/skills/workflow/debt-register
---

# 負債台帳（debt_register）スキル

G4 通過条件の負債台帳生成・更新スキル。
...
- G4 通過条件の「未解決 debt は台帳化」を満たす
- 暗黙知の負債を見える化し、返済計画に接続する
- `high impact` の放置を防ぎ、リリースリスクを明示する
...
## 6. G4 通過判定基準（UT-TDD V-model ゲート）
