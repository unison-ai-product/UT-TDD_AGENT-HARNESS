...
import { loadStrictCanonicalMemorySnapshot, writeMemory } from "../src/memory/service.ts";
import {
...
describe("PLAN-L7-533 PR-1 completion fence core", () => {
  ...
  it("U-PMEMFENCE-024: malformed canonical memory denies the complete corpus without runtime writes", () => {
    const root = fixture().primary;
    complete(root);
    const before = runtimeSnapshot(root);
    writeFileSync(memoryPath(root, "seed.md"), "not a canonical memory document\n");
  ...
  it("U-PMEMFENCE-025: a non-regular canonical memory path is source_unsafe", () => {
    const root = fixture().primary;
    complete(root);
    const before = runtimeSnapshot(root);
    const seed = memoryPath(root, "seed.md");
    rmSync(seed);
    mkdirSync(seed);
  ...
  it("U-PMEMFENCE-026: a mutation during strict snapshot read is snapshot_unstable", () => {
    const root = fixture().primary;
    let mutated = false;
    const result = loadStrictCanonicalMemorySnapshot(root, {
      lstat: (path) => lstatSync(path),
      realpath: (path) => realpathSync(path),
      readdir: (path) => readdirSync(path),
      readText: (path) => {
        const content = readFileSync(path, "utf8");
        if (!mutated && path.endsWith("seed.md")) {
          mutated = true;
          writeFileSync(path, `${content}drift\n`);
        }
        return content;
      },
    });

    expect(result).toEqual({ ok: false, reason: "snapshot_unstable" });
  });
