...
function runDelegation(args: string[]): Promise<{ stdout: string; stderr: string }> {
  ...
  return program.parseAsync(["node", "ut-tdd", ...args]).then(
    () => ({ stdout, stderr }),
    (error) => {
      throw error;
    },
...
describe("review attestation (U-RVATT)", () => {
  ...
  it("U-RVATT-003: 同一内容の receipt replay は content-addressed な1ファイルへ収束する", () => {
    ...
    try {
      writeFileSync(verdictFile, "VERDICT: PASS\n", "utf8");
      const first = projectReviewVerdict({ repoRoot: root, request: request(), attestation: attestation(), verdictFile });
      const replay = projectReviewVerdict({ repoRoot: root, request: request(), attestation: attestation(), verdictFile });
  ...
  it("U-RVATT-005: review_lane role の --review-head 欠落は review_head_required で fail-close する", async () => {
    ...
    try {
      const result = await runDelegation([
        "codex", "--role", "blind-reviewer", "--task", "review slice", "--review-pr", "731", "--review-revision", "review-rvatt-1",
      ]);
  ...
  it("U-RVATT-006: attestation.head と PR observation.head が異なれば D1 は merge_ready にしない", () => {
    ...
    try {
      ...
      const projected = projectReviewVerdict({ repoRoot: root, request: request(), attestation: attestation(), verdictFile });
  ...
  it("U-RVATT-008: verdict file に echo された契約があっても実 verdict を採用する", () => {
    ...
    try {
      ...
      const result = projectReviewVerdict({ repoRoot: root, request: request(), attestation: attestation(), verdictFile });
      expect(result).toMatchObject({ ok: true, receipt: { verdict: "PASS", blockingFindings: [] } });
  ...
  it("U-RVATT-009: 発行 request は receipt と memoryId/pr/exactHead/reviewRevision を突合できる", () => {
    ...
    try {
      ...
      const issued = issueReviewRequest({ repoRoot: root, request: request() });
      const projected = projectReviewVerdict({ repoRoot: root, request: request(), attestation: attestation(), verdictFile });
      expect(issued).toMatchObject({ ok: true, request: { memoryId: "memory-rvatt", pr: 731, exactHead: head, reviewRevision: "review-rvatt-1" } });
      expect(projected).toMatchObject({ ok: true, receipt: { memoryId: "memory-rvatt", pr: 731, head, reviewRevision: "review-rvatt-1" } });
