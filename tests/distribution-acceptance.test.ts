...
import { resolveNpmCli } from "../scripts/run-vitest-snapshot";
import {
...
function runNpm(cwd: string, args: string[], env: NodeJS.ProcessEnv = process.env) {
  return runNode(cwd, [resolveNpmCli(), ...args], env);
}
...
function writeLocalUtTddShim(root: string): string {
  ...
  if (process.platform === "win32") {
    ...
    writeFileSync(path, '@echo off\r\nnode "%~dp0..\\dist\\ut-tdd.mjs" %*\r\n', "utf8");
    return path;
  }
  const path = join(binDir, "ut-tdd");
  writeFileSync(path, '#!/bin/sh\nexec node "$(dirname "$0")/../dist/ut-tdd.mjs" "$@"\n', {
    encoding: "utf8",
    mode: 0o755,
  });
...
describe("clean distribution local acceptance smoke", () => {
  ...
  it("U-SETUP-013 / U-SETUP-014 / AT-DIST-001: clean artifact installs and exposes the same core CLI surfaces", () => {
    ...
    try {
      ...
      const install = runNpm(cleanRoot, ["ci", "--ignore-scripts"], env);
      expect(install.status, install.stderr || install.stdout).toBe(0);
      const build = runNpm(cleanRoot, ["run", "build"], env);
      ...
      expect(packPackageJson.scripts["test:source"]).toBe("node scripts/run-vitest-snapshot.ts");

      const status = runNode(cleanRoot, ["dist/ut-tdd.mjs", "status", "--json"], env);
      ...
      const distribution = runNode(
        cleanRoot,
        ["dist/ut-tdd.mjs", "distribution", "plan", "--tag", "v0.1.0", "--json"],
        env,
      );
      ...
      const setup = runNode(cleanRoot, ["dist/ut-tdd.mjs", "setup", "--solo"], env);
