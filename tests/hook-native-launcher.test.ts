...
// PLAN-L7-522 S1-b (Issue #470): `common/run-bun.ts` は撤去された。hook は生成 wrapper
// `common/ut-tdd.mjs` を node で直接起動するので、U-HOOKEXEC-001 / 008 の argv・stdin 転送と
// shell-free 契約は wrapper 自身に対して測る。launcher 固有の assertion だけが obsolete であり、
// 契約そのものは live である (oracle を消さずに retarget する)。
