...
// 既存 untraced 9 件は baseline (known-debt)、IMP-087 の 4 件は REVERSE-40 へ back-fill 済。
...
describe("loadImplPlanTraceInput real repo (U-IPT-004/005)", () => {
  it("U-IPT-004: 実 repo の orphan は 0 (4 back-fill + 9 baseline 適用後、fail-close 回帰網)", () => {
    const r = analyzeImplPlanTrace(loadImplPlanTraceInput(process.cwd()));
    expect(r.orphans).toEqual([]);
  });

  it("U-IPT-005: baseline は 9 件 (IMP-087 の 4 は back-fill で trace = baseline に含めない)", () => {
    expect(IMPL_PLAN_TRACE_BASELINE.size).toBe(9);
    expect(IMPL_PLAN_TRACE_BASELINE.has("src/plan/parent-drive-mismatch-baseline.ts")).toBe(true);
