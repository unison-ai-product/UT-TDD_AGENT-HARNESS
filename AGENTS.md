...
複数 AI を使える場合は、判断と実行を分ける。設計判断・判断ゲート (G0.5/G2/G4-G7) レビュー・R4 合流判定は、現在作業している AI とは別系統の `frontier-reviewer` に依頼する。実装・テスト追加・文書整形は `worker` に委譲し、軽量分類や要約は `fast-checker` に寄せる。同一 `runtime + model` が作成と承認を兼ねる構成は避ける。単体 mode では専門サブエージェント review を必須とする (構想書 §2.1.2.1)。
